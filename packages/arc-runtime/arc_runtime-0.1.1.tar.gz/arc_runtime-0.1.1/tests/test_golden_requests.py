        ...
        for test_case in self.test_cases:
            with self.subTest(name=test_case['name']):
                ...
                if expected_fix is None:
                    self.assertIsNone(
                        actual_fix,
                        f"No fix should be applied for {test_case['name']}"
                    )
                else:
                    self.assertIsNotNone(
                        actual_fix,
                        f"Fix should be applied for {test_case['name']}"
                    )
                    self.assertEqual(
                        actual_fix,
                        expected_fix,
...
class TestGoldenRequestsIntegration(unittest.TestCase):
    ...
    def test_yaml_bundle_loadable(self):
        ...
        self.assertGreaterEqual(len(data['test_cases']), 3, "Should have at least 3 test cases")
