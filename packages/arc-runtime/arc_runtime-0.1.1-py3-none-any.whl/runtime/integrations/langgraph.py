...
class ArcStateGraph(StateGraph if HAS_LANGGRAPH else object):
    ...
    def invoke(self, input_data: Dict[str, Any], config: Optional[Dict[str, Any]] = None) -> Any:
    ...
    def _execute_node_with_tracking(
        ...
        try:
            ...
            logger.debug(
                f"Executed node '{node_name}' in {latency_ms:.2f}ms"
            )
    ...
    def add_conditional_edges(
        ...
        def wrapped_path_func(state: Dict[str, Any]) -> str:
            ...
            if self.arc_context:
                logger.debug(
                    f"Routing decision from '{source}': {decision}"
                )
