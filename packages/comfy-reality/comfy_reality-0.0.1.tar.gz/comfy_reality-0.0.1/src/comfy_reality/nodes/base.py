...
class BaseARNode:
    ...
    def validate_image_tensor(
        ...
        if not isinstance(tensor, torch.Tensor):
            raise ARValidationError(
                f"{name} must be a torch.Tensor, got {type(tensor)}", name
            )
        ...
        if channels is not None:
            # Check channel dimension (2nd dimension for PyTorch CHW/BCHW format)
            if tensor.dim() == 4:  # BCHW format
                actual_channels = tensor.shape[1]
            elif tensor.dim() == 3:  # CHW format
        ...
        if tensor_min < min_val or tensor_max > max_val:
            raise ARValidationError(
                ...
                f"got range [{tensor_min:.3f}, {tensor_max:.3f}]",
                name,
            )
    ...
    def validate_mask_tensor(
        ...
        if not isinstance(tensor, torch.Tensor):
            raise ARValidationError(
                f"{name} must be a torch.Tensor or None, got {type(tensor)}", name
            )

        # Mask should be 2D or 3D (batch, height, width) or (height, width)
        if tensor.dim() not in [2, 3]:
            raise ARValidationError(
                f"{name} must have 2-3 dimensions, got {tensor.dim()}", name
            )

        # Check values are in [0, 1] range for masks
        if tensor.min() < 0.0 or tensor.max() > 1.0:
            raise ARValidationError(
                f"{name} values must be in range [0.0, 1.0], "
                f"got range [{tensor.min():.3f}, {tensor.max():.3f}]",
                name,
            )
    ...
    def validate_numeric_parameter(
        ...
        if valid_values is not None and value not in valid_values:
            raise ARValidationError(
                f"{name} must be one of {valid_values}, got {value}", name
            )
    ...
    def validate_string_parameter(
        ...
        if max_length is not None and len(value) > max_length:
            raise ARValidationError(
                f"{name} must be <= {max_length} characters, got {len(value)}", name
            )
    ...
    def tensor_to_numpy(self, tensor: torch.Tensor | None) -> np.ndarray | None:
        ...
        try:
            # Remove batch dimension if present (ComfyUI format)
    ...
    def numpy_to_tensor(
        self, array: np.ndarray | None, add_batch_dim: bool = True
    ) -> torch.Tensor | None:
    ...
    def validate_return_types(self, result: tuple, expected_types: tuple) -> None:
        ...
        if not isinstance(result, tuple):
            raise ARValidationError(
                f"Return value must be tuple, got {type(result)}", "return_value"
            )
