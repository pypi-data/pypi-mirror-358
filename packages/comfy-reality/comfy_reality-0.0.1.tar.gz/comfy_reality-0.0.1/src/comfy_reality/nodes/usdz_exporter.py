...
class USDZExporter(BaseARNode):
    ...
    def export_usdz(
        ...
        try:
            ...
            self.validate_image_tensor(
                image, "image", channels=(3, 4), value_range=(0.0, 1.0)
            )
            ...
            self.log_processing_info(
                "USDZ Export", filename=filename, scale=scale, material=material_type
            )
            ...
            image_np, mask_np = self._optimize_for_ar(
                image_np, mask_np, optimization_level
            )
            ...
            result = (
                (str(usdz_path),) if success else ("ERROR: Failed to create USDZ file",)
            )
    ...
    def _optimize_for_ar(
        self, image: np.ndarray, mask: np.ndarray | None, level: str
    ...
    def _create_usdz_file(
        ...
        if not USD_AVAILABLE:
            return self._create_fallback_usdz(
                image_np, output_path, scale, material_type
            )
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                # Create geometry
                mesh_prim = self._create_mesh_geometry(
                    stage, "/Root/Mesh", scale, coordinate_system
                )

                # Create and apply material
                material_prim = self._create_usd_material(
                    stage, "/Root/Material", material_type, "texture.png"
                )
                ...
                # Package as USDZ
                success = UsdUtils.CreateNewUsdzPackage(str(usd_file), str(output_path))
    ...
    def _create_fallback_usdz(
        self, image_np: np.ndarray, output_path: str, scale: float, material_type: str
    ) -> bool:
    ...
    def _save_texture(
        self, image_np: np.ndarray, mask_np: np.ndarray | None, output_path: str
    ) -> None:
        ...
        if mask_np is not None:
            ...
            if len(mask_np.shape) == 2:
                ...
                alpha_data = (
                    ...
                    else mask_np.flatten()[: height * width].reshape(height, width)
                )
        elif channels == 4:
    ...
    def _create_mesh_geometry(
        self, stage: "Usd.Stage", path: str, scale: float, coordinate_system: str
    ) -> "UsdGeom.Mesh":
        ...
        uv_attr = primvars_api.CreatePrimvar(
        ...
        # Set normals
        if coordinate_system == "y_up":
            normal = (0, 1, 0)  # Up in Y
        else:
            normal = (0, 0, 1)  # Up in Z

        mesh.CreateNormalsAttr().Set([Gf.Vec3f(*normal)] * 4)

        return mesh

    def _create_usd_material(
        self, stage: "Usd.Stage", path: str, material_type: str, texture_filename: str
    ) -> "UsdShade.Material":
        ...
        texture_reader.CreateInput("file", Sdf.ValueTypeNames.Asset).Set(
            texture_filename
        )
        texture_reader.CreateInput("st", Sdf.ValueTypeNames.Float2).ConnectToSource(
            material.CreateInput("st", Sdf.ValueTypeNames.Float2)
        )
        ...
        shader.CreateInput("opacity", Sdf.ValueTypeNames.Float).ConnectToSource(
            texture_reader.CreateOutput("a", Sdf.ValueTypeNames.Float)
        )
        ...
        for prop_name, prop_value in material_props.items():
            if prop_name in ["roughness", "metallic"]:
                ...
                shader.CreateInput(prop_name, Sdf.ValueTypeNames.Color3f).Set(
                    Gf.Vec3f(*prop_value)
                )
    ...
    def _add_physics_properties(self, mesh_prim: "UsdGeom.Mesh") -> None:
        ...
        prim.CreateAttribute("physics:collisionEnabled", Sdf.ValueTypeNames.Bool).Set(
            True
        )
        prim.CreateAttribute("physics:kinematicEnabled", Sdf.ValueTypeNames.Bool).Set(
            False
        )
        prim.CreateAttribute("physics:rigidBodyEnabled", Sdf.ValueTypeNames.Bool).Set(
            True
        )
