...
class MaterialComposer:
    ...
    def compose_material(
        ...
        if roughness_map is not None:
            material["roughness_map"] = self._process_texture(
        ...
        if occlusion_map is not None:
            material["occlusion_map"] = self._process_texture(
        ...
        if USD_AVAILABLE:
            material["usd_definition"] = self._create_usd_material_definition(
                material, properties
            )
    ...
    def _create_usd_material_definition(self, material: dict, properties: dict) -> dict:
        ...
        if material_type == "unlit":
            ...
            usd_definition["inputs"]["emissiveColor"] = usd_definition["inputs"][
                "diffuseColor"
            ]
            usd_definition["inputs"]["diffuseColor"] = {
                "type": "color3f",
        ...
        usd_definition["arkit_properties"] = {
            ...
            if properties.get("transparency", False)
            else "opaque",
