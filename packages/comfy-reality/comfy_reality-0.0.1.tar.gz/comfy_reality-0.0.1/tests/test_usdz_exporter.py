...
class TestUSDZExporter:
    ...
    def test_tensor_to_numpy(self, exporter):
        ...
        # Test 4D tensor (with batch)
        tensor_4d = torch.rand(1, 256, 256, 3)
        result = exporter.tensor_to_numpy(tensor_4d)
        assert isinstance(result, np.ndarray)
        assert result.shape == (256, 256, 3)
        
        # Test 3D tensor
        tensor_3d = torch.rand(256, 256, 3)
    ...
    def test_different_image_formats(self, exporter):
        ...
        # Test HWC format
        image_hwc = torch.rand(512, 512, 3)
        result = exporter.export_usdz(image_hwc, "test_hwc")
        assert isinstance(result, tuple)
