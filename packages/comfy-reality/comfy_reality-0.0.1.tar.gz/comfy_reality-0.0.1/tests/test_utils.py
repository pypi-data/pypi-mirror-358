...
from comfy_reality.utils.image_utils import (
    preprocess_image, postprocess_image, resize_image, 
    create_alpha_channel, apply_mask
)
from comfy_reality.utils.usdz_utils import (
    create_usdz_file, optimize_for_ar, validate_usdz_compatibility,
    create_material_properties, create_geometry_data
)
from comfy_reality.utils.validation import (
    validate_workflow, validate_usdz, validate_image_tensor,
    validate_mask_tensor, validate_ar_requirements
)
...
class TestValidation:
    ...
    def valid_workflow(self):
        ...
        return {
            "nodes": [
                {"id": "1", "type": "AROptimizer"},
                {"id": "2", "type": "SpatialPositioner"},
                {"id": "3", "type": "USDZExporter"}
            ],
            "connections": [
                {
                    "from_node": "1", "to_node": "2",
                    "from_output": "optimized_image", "to_input": "image"
                }
            ]
        }
    ...
    def test_validate_workflow_invalid_node_type(self):
        ...
        invalid_workflow = {
            "nodes": [{"id": "1", "type": "InvalidNodeType"}],
            "connections": []
        }
