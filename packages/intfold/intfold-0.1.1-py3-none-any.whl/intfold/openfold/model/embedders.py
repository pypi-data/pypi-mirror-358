...
class InputEmbedder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.linear_s_inputs = Linear(c_s_inputs,c_s,init='linear',bias=False) 
        self.linear_z_i = Linear(c_s_inputs, c_z,init='linear',bias=False) 
        self.linear_z_j = Linear(c_s_inputs, c_z,init='linear',bias=False) 

        self.linear_token_bonds = Linear(1, c_z, init='linear',bias=False) 
...
class RecyclingEmbedder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.linear_z = Linear(c_z, c_z,init='linear',bias=False) 
        self.layer_norm_z = LayerNorm(self.c_z) 
        self.linear_s = Linear(c_s, c_s,init='linear',bias=False) 
...
class MSAEmbedder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.linear_mf = Linear(c_msa_feat, c_m,init='linear',bias=False)
        self.linear_s_inputs = Linear(c_s_inputs, c_m,init='linear',bias=False)
    ...
    def forward(self, batch,s_inputs,msa_mask,inplace_safe=False):
        ...
        # Create a padded indices tensor using -1 as the invalid value
        indices = torch.stack(
            [F.pad(indice, (0, max_depth - len(indice)), value=-1) for indice in random_indices], dim=0
        )  # Shape: [batch_size, max_depth]
        
        # Mask invalid indices (-1) to avoid gathering unwanted features
        valid_mask = indices >= 0
        indices = indices.masked_fill(~valid_mask, 0)  # Replace invalid indices with 0 for gather compatibility

        # Gather the selected indices for `mf`
        gathered_mf = torch.gather(
            mf,
            1,
            einops.repeat(indices, 'b n -> b n m d', m=mf.shape[-2], d=mf.shape[-1])
        )  # Shape: [batch_size, max_depth, num_token, num_msa_feat]
        # Mask out invalid entries in the gathered tensor
        mf = gathered_mf.masked_fill(~valid_mask.unsqueeze(-1).unsqueeze(-1), 0)

        # Gather the selected indices for `msa_mask`
        gathered_msa_mask = torch.gather(
            msa_mask,
            1,
            einops.repeat(indices, 'b n -> b n m', m=msa_mask.shape[-1])
        )  # Shape: [batch_size, max_depth, num_token]
        # Mask out invalid entries in the gathered tensor
