...
from intfold.openfold.utils.tensor_utils import add, one_hot, tensor_tree_map, dict_multimap
...
class ConditionedTransitionBlock(nn.Module):
    ...
    def __init__(self, c_a, c_s, transition_n):
        ...
        self.linear_a = Linear(self.c_a, self.transition_n * self.c_a * 2, init="relu",bias=False)
        self.swish = nn.SiLU()
        
        self.linear_s = Linear(self.c_s, self.c_a, init="zeros",bias=True,bias_init=-2.0)
        self.linear_b = Linear(self.c_a * self.transition_n , self.c_a, init="linear",bias=False)
...
class AtomToTokenPooler(nn.Module):
    def __init__(
        ...
        self.linear_q = Linear(c_atom, c_token,init="linear",bias=False)
        self.relu = nn.ReLU()
...
class RelativePositionEncoding(nn.Module):
    ...
    def __init__(self, c_z: int, r_max: int, s_max: int, mapping=True, **kwargs):
        ...
        if mapping:
            self.linear_relpos = Linear(self.no_bins, c_z,init='linear',bias=False)
...
class DiffusionConditioning(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.linear_z = Linear(c_z + self.no_bins, c_z, init='linear', bias = False)
        ...
        self.linear_s = Linear(c_s + c_s_inputs, c_s, init='linear',bias = False)
        
        self.sigma_data = sigma_data
        self.fourier_embedding = FourierEmbedding(c_fourier)
        self.layer_norm_f = LayerNorm(c_fourier, bias=False)
        self.linear_f = Linear(c_fourier, c_s, init='linear', bias = False)
...
class AtomAttentionEncoder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.linear_ref_pos = Linear(3,c_atom, init='linear',bias = False)
        self.linear_ref_atom_name_chars = Linear(4 * 64,c_atom, init='linear',bias = False)
        self.linear_ref_element = Linear(128,c_atom, init='linear',bias = False)
        self.linear_ref_charge = Linear(1,c_atom, init='linear',bias = False)
        self.linear_ref_mask = Linear(1,c_atom, init='linear',bias = False)
        
        self.linear_d = Linear(3, c_atompair, init='linear', bias = False)
            # A5 Line 5
        self.linear_d_inv = Linear(1,c_atompair, init='linear',bias = False)
            # A5 Line 6
        self.linear_v = Linear(1,c_atompair, init='linear',bias= False)
            # A5 Line 9
        self.window_size_row = window_size_row
        self.window_size_col = window_size_col
        ...
        if self.initial==False:
            ...
            self.linear_s = Linear(c_s, c_atom, init='zeros',bias=False)

                # A5 Line 10
            self.layer_norm_z = LayerNorm(c_z, bias=False)
            self.linear_z = Linear(c_z, c_atompair, init='zeros', bias=False)
        
                # A5 Line 11
            self.linear_r = Linear(3, c_atom, bias=False)
            # A5 Line 13
        self.linear_c_row = Linear(c_atom, c_atompair,init="relu", bias=False)

        self.linear_c_col =  Linear(c_atom, c_atompair,init="relu", bias=False)
        
        self.linear_mlp_p_1 = Linear(c_atompair, c_atompair,init="relu",bias=False)
        self.linear_mlp_p_2 = Linear(c_atompair, c_atompair,init="relu",bias=False)
        self.linear_mlp_p_3 = Linear(c_atompair, c_atompair,init="zeros",bias=False)
    ...
    def forward(
        ...
        # A5 Line 7
        if self.initial == False:
            # Process the single condition first (according to Deepmind's Implementation)
        ...
        if self.initial == False:
            ...
            # break into windows and repeat token length to atom length
            ...
            # p_trunk = einx.get_at('b [i j] dap, b nw w1 w2, b nw w1 w2 -> b nw w1 w2 dap', p_trunk, row_indices, col_indices)
        ...
        if self.initial == False:
            # Next Process the query activation
            seq_len = s_trunk.shape[-2]
            q =  self.linear_r(r)
...
class AtomAttentionDecoder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        # A6 Line 1
        self.linear_a = Linear(c_token, c_atom,init= "linear", bias=False)
        ...
        self.linear_q = Linear(c_atom, 3,init="zeros", bias=False)
...
class DiffusionModule(nn.Module):
    ...
    def __init__(
        ...
        self.proj_s = nn.Sequential(
            ...
            Linear(self.c_s, self.c_token,init="linear",bias=False)
        )
    ...
    def forward(
        ...
        # A20 Line 4
        a = add(a, self.proj_s(s), inplace=False) # broadcasting
        # A20 Line 5
