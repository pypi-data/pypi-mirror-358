...
from intfold.openfold.utils.tensor_utils import one_hot
from intfold.openfold.utils.atom_token_conversion import repeat_consecutive_with_lens, aggregate_fn
...
class DistogramHead(nn.Module):
    ...
    def __init__(self, c_z, no_bins, **kwargs):
        ...
        self.linear = Linear(self.c_z, self.no_bins, init="zeros")
...
class pAEHead(nn.Module):
    def __init__(self, c_s, no_bins, **kwargs):
        ...
        self.linear = Linear(self.c_s, self.no_bins, init="zeros")
...
class pLDDTHead(nn.Module):
    def __init__(self, c_s, max_num_atoms, no_bins, **kwargs):
        ...
        self.layer_norm = LayerNorm(self.c_s)
        self.linear = Linear(self.c_s, max_num_atoms * self.no_bins, init="zeros")
...
class ResolvedHead(nn.Module):
    def __init__(self, c_s, max_num_atoms, **kwargs):
        ...
        self.c_s = c_s
        self.layer_norm = LayerNorm(self.c_s)
        self.linear = Linear(self.c_s, max_num_atoms * 2, init="zeros")
...
class ConfidenceHead(nn.Module):
    def __init__(self, config):
        ...
        self.linear_s_inputs_row = Linear(self.c_s_inputs, self.c_z, init="linear",bias=False)
        self.linear_s_inputs_col = Linear(self.c_s_inputs, self.c_z, init="linear",bias=False)
        
        self.linear_d = Linear(self.no_bins,self.c_z, init="linear",bias=False)
