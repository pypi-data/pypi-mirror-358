...
from intfold.openfold.model.dropout import DropoutRowwise, DropoutColumnwise
...
class AF3Attention(Attention):
    ...
    def __init__(
        ...
        self.linear_q = Linear(
            self.c_q, self.c_hidden * self.no_heads, bias=True, init="linear"
        )
        
        self.linear_k = Linear(
            self.c_k, self.c_hidden * self.no_heads, bias=False, init="linear"
        )
        # without bias
        if conditioned:
            self.linear_o = Linear(
                self.c_hidden * self.no_heads,  self.c_q, bias = False, init="linear"
            )
        else:
            self.linear_o = Linear(
                self.c_hidden * self.no_heads,  self.c_q, bias = False, init="zeros"
            )

        self.linear_g = None
        if self.gating:
            self.linear_g = Linear(
                self.c_q, self.c_hidden * self.no_heads,bias=False ,init="zeros"
            )
...
class AdaLN(nn.Module):
    ...
    def __init__(
        ...
        self.linear_s_gamma = Linear(c_s, c_a,init="zeros", bias=True)
        
        self.sigmoid = nn.Sigmoid()

        self.linear_s_beta = Linear(c_s, c_a ,init= "zeros" ,bias=False)
...
class BiasAttention(nn.Module):
    ...
    def __init__(
        ...
        self.linear_v = Linear(
            self.c_v, self.c_hidden * self.no_heads, bias=False, init="linear"
        )
        self.linear_o = Linear(
            self.c_hidden * self.no_heads, self.c_v, bias=False, init="zeros"
        )

        self.linear_g = Linear(
            self.c_v, self.c_hidden * self.no_heads, bias=False, init="zeros"
        )
...
class Transition(nn.Module):
    ...
    def __init__(self, c, transition_n):
        ...
        self.linear = Linear(self.c, self.transition_n * self.c * 2, init="relu",bias=False)
        self.swish = nn.SiLU()

        self.linear_o = Linear(self.transition_n * self.c, c, init="zeros",bias=False)
...
class AttentionPairBias(nn.Module):
    ...
    def __init__(
        ...
        self.linear_z = Linear(c_z, self.no_heads, bias=False, init="linear")
    ...
    def forward(
        ...
        Returns:
            [*, I, C_a] output tensor
        """ 
        use_deepspeed_evo_attention = False #! THIS IS UNSTABLE
        ...
        # Unsqueeze the single representation and input mask
        if single_mask is None:
            # [*, I]
            single_mask = a.new_ones(
                a.shape[:-1],
...
class MSAPairWeightedAveraging(nn.Module):
    ...
    def __init__(
        ...
        self.linear_z = Linear(
            self.c_z, self.no_heads, bias=False, init="linear"
        )
    ...
    def _prep_inputs(self,
        ...
        if not use_deepspeed_evo_attention:
            # This is different for the msa row-wise attention, since the weight is a global one rather than a per-row one
            mask = torch.max(mask, dim = -2, keepdim = True).values # [*, 1, N_token]
