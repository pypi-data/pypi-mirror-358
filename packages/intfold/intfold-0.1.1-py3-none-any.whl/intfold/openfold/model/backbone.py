...
class BackboneTrunk(nn.Module):
    ...
    def forward(self, feats):
        ...
        # uniform sampling between [1, self.recycling_iters + 1]

        if self.training:
            if torch.distributed.is_initialized():
                # Only rank 0 generates the random number
                if torch.distributed.get_rank() == 0:
                    num_iters = torch.randint(1, self.recycling_iters + 2, (1,),device=feats['seq_mask'].device)
                else:
                    num_iters = torch.zeros(1, dtype=torch.int64,device=feats['seq_mask'].device)  # Placeholder for other ranks
                
                # Broadcast the generated value to all ranks
                torch.distributed.broadcast(num_iters, src=0)
                num_iters = num_iters.item()
            else:
                # Fallback for non-DDP mode
                num_iters = torch.randint(1, self.recycling_iters + 2, (1,)).item()
        else:
            num_iters = self.recycling_iters + 1
