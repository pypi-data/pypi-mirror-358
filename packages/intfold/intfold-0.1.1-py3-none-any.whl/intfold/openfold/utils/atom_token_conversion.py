...
def aggregate_fn_advanced(original_seqs, attention_mask):
    ...
    num_atoms = attention_mask.sum(dim=(1, 2))  # Shape: [batch_size]
    ...
    range_tensor = torch.arange(max_atoms, device=attention_mask.device) # Shape: [max_atoms]
    range_tensor = einops.repeat(range_tensor, 'm -> b m', b=batch_size)  # Shape: [batch_size, max_atoms]
    output_attention_mask = range_tensor < num_atoms.unsqueeze(1)  # Shape: [batch_size, max_atoms]
...
def concat_previous_and_later_windows(
    ...
    *,
    ...
    # However, the first 2 windows and last 2 windows contains outofbound values
    # We need to use the third window to replace the first 2 windows and the third last window to replace the last 2 windows
    
    # Do it at the dim_seq
    t = torch.cat((
        first,first,
