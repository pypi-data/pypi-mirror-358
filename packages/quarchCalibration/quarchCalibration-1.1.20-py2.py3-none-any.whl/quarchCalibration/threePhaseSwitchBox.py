...
class ThreePhaseSwitchBox:
    ...
    def discover():
        ...
        # Loop through network interfaces to broadcast the scan on each
        for ip in ipList[2]:

            logging.debug(os.path.basename(__file__) + ": Broadcasting on : " + ip)

            # Bind the broadcast out socket
            tsock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        ...
        for response in responses:
            ...
            if strdict["NAME"] == "ET-2260":
                ...
                logging.debug(os.path.basename(__file__) + ": Found Mux: " + strdict['IP'])
