...
# 28-06-2025 :Уточнение конфига. Добавлены тени и толщина рамок. Обработка Text и Boundary.
#
#------------------------------------------------------------------------------
# TODO:
...
# 7. Вывод значений тэгов. Если ShowTags = True
#+8. Цвет контура 154 132 132
#+9. Boundary
#+10. ApplicationComponent ArchiMate_
#+11. Текст
#+12. Shadow
# 13. Типы проводки линий и привязка линий
# 14. Картинки
#+15. Толщина контура
# 16. Custom style
# 17. Shape для Component - чтобы выглядел как в EA - это сборка из двух компоннетов
# 18. В конфиге описать все компоненты Archimate и убрать условности из кода
#------------------------------------------------------------------------------
#region  import
...
class Config:
    ...
    def find(self, type, stereotype, attr, default=None):
        ...
        if stereotype.startswith("ArchiMate_Application"):   
            # Для этих стереотипов параметры едины для всех типов и они в конфиге описаны без указания типа
            type = "None"
            stereotype = "ArchiMate_Application"

        if stereotype.startswith("ArchiMate_Business"):   
            # Для этих стереотипов параметры едины для всех типов и они в конфиге описаны без указания типа
            type = "None"
            stereotype = "ArchiMate_Business"
...
class DioElement:
    ...
    """
    __slots__ = ('guid', 'name', 'x', 'y', 'width', 'height', 'fillcolor', 'shadow', 'strokeColor','strokeWidth', 'zord', 'square',
    ...
    def __init__(self, dObj, dElem, cornerX, cornerY):
        ...
        self.name = escape_xml_chars(dElem.Notes if type in ( "Note", "Text") else dElem.name)
        self.name.replace("\n", "<br>")   # Переводим переносы строк в HTML-теги
        ...
        if dObj.BorderColor != -1:
            ...
            self.strokeColor = CONF.find(type, stereotype, "bordercolor", '9A8484')

        self.strokeWidth = dObj.BorderLineWidth    # Толщина рамки компонента
        if not 1 <= self.strokeWidth <= 5:
            print(f'Толщина рамки компонента {self.strokeWidth} не в диапазоне от 1 до 5')
            self.strokeWidth = 1
        
        self.shadow = CONF.find(type, stereotype, "shadow", 0)  # Для тени нет свойства, берем ее из конфига
        ...
        # Получим значения тегов и преобразуем его в многострочную строку
        tagged_values = dElem.TaggedValues
        tags_str = None
        if tagged_values.Count > 0:
            # Собираем каждую пару "Name=Value" в список строк. Тэги с пустыми значениями игнорируем 
            tags_str = '\n'.join([f"{tv.Name}={tv.Value}" for tv in tagged_values if tv.Value])
            # print(f"Теги: {tags_str}")

        # Пока мне лениво, поэтому значения тэгов просто добавим к названию элемента,
        # для последующей ручной доработки в Drawio
        if tags_str and dObj.ShowTags:
            self.name += f"\n{tags_str}"
            self.name.replace("\n", "<br>")   # Переводим переносы строк в HTML-теги
...
def drawioGenerate(diagram):
    ...
    for key in sorted_elements:
        ...
        cell = ET.SubElement(
            ...
            style=f"{el.shape}shadow={el.shadow};strokeWidth={el.strokeWidth};whiteSpace=wrap;html=1;verticalAlign=top;" +
    ...
    for key in connectors:
        ...
        edge = ET.SubElement(
            ...
            style=f"{conn.endArrow}html=1;rounded=1;{conn.dashed}{conn.edgeStyle}" +
                  f"{conn.startArrow}{conn.strokeColor}{conn.lineWidth}exitPerimeter=0;entryPerimeter=0;",
            edge="1",
            parent="1", # type: ignore
...
def explore(item):
    ...
    for dObj in item.DiagramObjects:        # Выборка всех элементов диаграммы
        ...
        print(f'        GUID:{dObj.InstanceGUID} Left:{dObj.Left} Right:{dObj.Right} Top:{dObj.Top} Bottom:{dObj.Bottom} BackgroundColor:{dObj.BackgroundColor} Z:{dObj.Sequence} FontSize:{dObj.FontSize} TextAlign:{dObj.TextAlign} BorderColor:{dObj.BorderColor} BorderLineWidth:{dObj.BorderLineWidth}')
        print(f'        ShowTags:{dObj.ShowTags}')
