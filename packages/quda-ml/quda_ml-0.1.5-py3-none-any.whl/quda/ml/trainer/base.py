...
class BaseIncrementalTrainer(abc.ABC):
    ...
    def save_model(self, model, artifact_path: str):
        ...
        if isinstance(model, lightgbm.Booster):
            ...
            return mlflow.lightgbm.log_model(model, artifact_path, signature=signature,
                                             input_example=self.input_example)
        elif isinstance(model, torch.nn.Module):
            return mlflow.pytorch.log_model(model, artifact_path, signature=signature, input_example=self.input_example)
        else:
