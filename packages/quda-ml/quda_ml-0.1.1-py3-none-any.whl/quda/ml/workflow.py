...
class WorkFlow(BaseEstimator):
    ...
    def __init__(self,
        ...
        # 配置文件中解析出来的参数: 包括 workflow 配置参数，数据集配置参数，模型配置参数，训练参数，fit 运行配置
    ...
    def from_conf(cls, config_path: str, config_name: str) -> 'WorkFlow':
        ...
        # trainer_params
        trainer_params = instantiate(cfg.trainer_params)
    ...
    @classmethod
    def run(cls, config_path: str, config_name: str) -> 'WorkFlow':
        """
        ...
        """
        wf = cls.from_conf(config_path=config_path, config_name=config_name)
        fit_config = wf._cfg.get("fit")
        if not fit_config:
            ylog.warning("Missing WorkFlow.fit params.")
            return
        return cls.fit(**fit_config)
...
def init_config():
    ...
    try:
        ...
        ylog.error(f"[QUDA-ML] - {target_conf_dir} created.")
