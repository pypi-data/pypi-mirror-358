...
def hydra_wrapper(config_path, config_name):
    ...
    if not os.path.exists(abs_config_path):
        ylog.error(f"Missing {abs_config_path}")
        return
...
class WorkFlow(BaseEstimator):
    ...
    @classmethod
    def _new_workflow(cls, cfg: DictConfig):
        # 构建 data pipeline
        data_steps = []
    ...
    @classmethod
    def from_conf(cls, config_path: str, config_name: str) -> 'WorkFlow':
        """
        配置驱动:从配置文件中构建工作流
    ...
    def fit(self,
        ...
        with mlflow.start_run() as top_run:
            ...
            for i, train_batch in enumerate(
                    [iterables[i:i + batch_size] for i in range(0, len(iterables), batch_size)]):
                ...
                with mlflow.start_run(nested=True, log_system_metrics=True) as partial_run:
                    ...
                    ylog.info(f"Preparing on batch_{i + 1} ({i + 1}/{len(iterables)}) - {beg} >>> {end}")
                    params["data.beg"] = beg
                    params["data.end"] = end
                    # 加载训练数据
                    ...
                    if len(target) > 1:
                        ylog.warning(f"Multiple targets found: {target}, use {target[0]}")
                    else:
                    ...
                    ylog.info(f"Training on batch_{i + 1} ({i + 1}/({len(iterables)}) - {beg} >>> {end}")
                    ...
                    # 保存模型
                    model_dir = quda.tb_path("models")
                    model_dir.mkdir(parents=True, exist_ok=True)
                    artifact_path = model_dir / f"checkpoint_{i + 1}"
                    model_info = self.trainer.save_model(model, artifact_path, )
                    self.trainer.model = mlflow.pyfunc.load_model(model_info.model_uri)

                    model = None

    @classmethod
    def _run_config(cls, cfg: DictConfig) -> 'WorkFlow':
        """
        Returns
        -------

        """
        wf = cls._new_workflow(cfg)
        fit_config = cfg.get("fit")
        if not fit_config:
            ylog.warning("Missing WorkFlow.fit params.")
            return
        return wf.fit(**fit_config)

    @classmethod
    def run(cls, config_path: str, config_name: str, ):
        """
        ...
        if is_notebook():
            with initialize(version_base=None, config_path=config_path):
                cfg = compose(config_name=config_name)
            return cls._run_config(cfg)
        hydra_main = hydra_wrapper(config_path, config_name)
        if hydra_main is not None:
            return hydra_main(cls._run_config)()
