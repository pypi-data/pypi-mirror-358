...
class Location(object):
    ...
    def __init__(self, req: HttpRequest) -> None:
        ...
        try:
            nat_client = HttpClient(req=req, name=settings.PLACE_SERVICE)
            nat_res = nat_client.get(path=f"/nations/?code={self.country_code.upper()}")
            if nat_res.ok:
                nat_json = nat_res.json()
                nation = nat_json['data'][0]
                self._data['id'] = nation['id']
                self._data['phone'] = nation['phone']
                self._data['capital'] = nation['capital']
                self._data['languages'] = nation['languages']
    ...
    def id(self) -> Union[UUID, None]:
        return UUID(self.data['id']) if 'id' in self.data else None
    
    @property
    def phone(self) -> Union[str, None]:
        return str(self.data['phone']) if 'phone' in self.data else None
    
    @property
    def capital(self) -> Union[str, None]:
        return str(self.data['capital']) if 'capital' in self.data else None
    
    @property
    def languages(self) -> Union[str, None]:
        return str(self.data['languages']) if 'languages' in self.data else None
