...
def make_scalar(obj):
    if obj is None:
        ...
        ts = int(obj.value)
        return ir.make_scalar_timedelta_ns(ts)
...
def make_column_name_element(args):
    ...
    if isinstance(args, (int, float, str, bytes)) or args is None:
...
def make_column_name(args):
    ...
    if isinstance(args, (int, float, str, bytes, tuple)) or args is None:
...
def is_column_name(x):
    ...
    def is_supported_type(obj):
        return isinstance(
            obj, (int, float, str, bytes)
        )  # bool is included in int
