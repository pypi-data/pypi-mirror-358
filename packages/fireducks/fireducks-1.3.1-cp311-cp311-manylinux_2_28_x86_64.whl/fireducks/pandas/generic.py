    ...
    def clip(self, *args, **kwargs):
        ...
        def check_fallback_for_clip(decoded):
            reason = decoded.is_not_default(
                exclude=["lower", "upper", "axis", "inplace"]
            )
            if reason is not None:
                return reason
            ...
            if lower is not None and upper is not None:
                ...
                if irutils.irable_scalar(lower) and irutils.irable_scalar(
                    ...
                    decoded.lower, decoded.upper = min(lower, upper), max(
                        lower, upper
                    )
        ...
        if lower is not None and upper is None:
            ret_value = self._mask_impl(self < lower, lower, axis, accept_classes)
        elif lower is None and upper is not None:
            ret_value = self._mask_impl(self > upper, upper, axis, accept_classes)
        else:
            tmp = self.mask(self < lower, lower, axis=axis)
            ret_value = tmp._mask_impl(tmp > upper, upper, axis, accept_classes)
