(function () {
    ...
    (function ($) {
        ...
        function initializeDateTimePicker() {
            $(document).find(".vDateField").tempusDominus({
                display: {
                    viewMode: "calendar",
                    theme: "light",
                },
            ...
            $(document).find(".vTimeField").tempusDominus({
                display: {
                    theme: "light",
                    viewMode: "clock",
                },
            ...
            $(document).find(".datetime").each(function () {
        ...
        $(document).on("change", ".card-tools.delete input[type=checkbox]", function () {
            ...
            if ($(this).is(":checked")) {
                ...
                card.find(".card-header").next().slideUp();

                card
                    .attr("data-bs-toggle", "tooltip")
                    .attr("title", "Marked for deletion. Click 'Save' to delete permanently.")
                    .tooltip({trigger: 'hover'})
                    .tooltip('show');
            } else {
                card.removeClass("deleted");
                card.find(".card-header").next().slideDown();
                card.tooltip('dispose');
                card.removeAttr("data-bs-toggle title");
            }
        })

        $(document).on("click", ".card .card-header", function (e) {
            if ($(e.target).closest('.card-tools.delete').length) return;
            const card = $(this).closest(".card");
            if (card.hasClass("deleted")) return;
