...
class LaTeXCalculate(float):
    ...
    LaTeXError = type('LaTeXError', (Exception,), {
        '__name__': 'LaTeXError',
        '__qualname__': 'LaTeXError',
        '__module__': __name__
    })
    
    def __new__(self, LaTeX_string: str):
        if "**" in LaTeX_string:
            raise self.LaTeXError("operator '**' is not defined. Did you mean: '^'?")
        elif "//" in LaTeX_string:
            raise self.LaTeXError("operator '//' is not defined")
        ...
        try:
            ...
            raise self.LaTeXError("empty LaTeX") from None
    ...
    def Separate(self, LaTeX_string):
        ...
        # 检测括号是否闭合
        if bracket_level < 0:
            raise self.LaTeXError("extra close brace or missing open brace")
        elif bracket_level > 0:
            raise self.LaTeXError("extra open brace or missing close brace")
    ...
    def Check(self, tag):
        ...
        if tag.count([r"\lfloor", "function"]) > tag.count([r"\rfloor", "function"]):
            raise self.LaTeXError(r"extra \lfloor pr missing \rfloor")
        elif tag.count([r"\lfloor", "function"]) < tag.count([r"\rfloor", "function"]):
            raise self.LaTeXError(r"extra \rfloor or missing \lfloor")

        # 检查ceil嵌套
        if tag.count([r"\lceil", "function"]) > tag.count([r"\rceil", "function"]):
            raise self.LaTeXError(r"extra \lceil pr missing \rceil")
        elif tag.count([r"\lceil", "function"]) < tag.count([r"\rceil", "function"]):
            raise self.LaTeXError(r"extra \rceil or missing \lceil")
    ...
    def Calculate(self, tag):
        ...
        for j in range(len(tag)):
            ...
            if attribute == "decimal_point":
                if j-1 == -1 or tag[j-1][1] != "number":
                    last_is_number = False
                else:
                    last_is_number = True
                    
                if j+1 == len(tag) or tag[j+1][1] != "number":
                    next_is_number = False
                else:
                    next_is_number = True
                    
                if not last_is_number and not next_is_number:
                    raise self.LaTeXError("no numbers before and after the decimal point")
                    
                elif not last_is_number:
                    raise self.LaTeXError("no numbers before the decimal point")
                
                elif not next_is_number:
                    raise self.LaTeXError("no numbers after the decimal point")
            ...
            # 需要替换
                ...
                # 自动添加乘号防止报错
                if j-1 != -1 and (tag[j-1][1] == "number" or tag[j-1][0] in ["}", "e"]):
                    calculate_string.append("*")
                calculate_string.append("math.pi")

            elif item == "e":
                if j-1 != -1 and (tag[j-1][1] == "number" or tag[j-1][0] in ["}", r"\pi"]):
                    calculate_string.append("*")
            ...
            # 未定义
                ...
                raise self.LaTeXError(f"undefined name '{item}'")
        ...
        try:
            ...
            if res == "":
                raise self.LaTeXError("empty LaTeX") from None
            raise self.LaTeXError(f"wrong character '{res[e.offset-1]}'") from None

        except:
            raise self.LaTeXError("LaTeX calculate error. Please check your LaTeX") from None