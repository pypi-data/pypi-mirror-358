...
class LaTeXCalculate(float):
    ...
    def __new__(self, LaTeX_string: str):
        if "**" in LaTeX_string:
            raise self.LaTeXError("operator '**' is not defined. Did you mean: '^'?") from None
        elif "//" in LaTeX_string:
            raise self.LaTeXError("operator '//' is not defined") from None
    ...
    def Calculate(self, tag):
        ...
        for j in range(len(tag)):
            ...
            if attribute == "decimal_point":
                if tag[j-1][1] != "number" or tag[j+1][1] != "number":
                    raise self.LaTeXError("no numbers before or after the decimal point")
            ...
            # 需要替换
                ...
                # 自动添加乘号防止报错
                if tag[j-1][1] == "number" or tag[j-1][0] == "}":
                    calculate_string.append("*")
            ...
            # 未定义
                ...
                raise self.LaTeXError(f"undefined name '{item}'") from None
        ...
        try:
            ...
            raise self.LaTeXError("LaTeX parse error. Please check your LaTeX") from None