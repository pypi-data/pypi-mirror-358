...
def _filter_fields(data_list, fields_param=None, workspace_id=None):
    """
...
def get_stories_or_tasks(workspace_id: int, options: dict = None) -> str:
    ...
    # 如果用户指定了 fields 参数，确保包含 v_status 字段
    if fields_param:
        if isinstance(fields_param, str):
            _tmp_fields = [f.strip() for f in fields_param.split(',') if f.strip()]
            if 'v_status' not in _tmp_fields:
                _tmp_fields.append('v_status')
            fields_param = ','.join(_tmp_fields)
        elif isinstance(fields_param, list):
            if 'v_status' not in fields_param:
                fields_param.append('v_status')
    # ret 可能是 {"data": [{...}, ...]} 或 list
    if isinstance(ret, dict) and 'data' in ret and isinstance(ret['data'], list):
        ret['data'] = _filter_fields(ret['data'], fields_param, workspace_id)
    elif isinstance(ret, list):
        ret = _filter_fields(ret, fields_param, workspace_id)
