...
class Cynk(eval.ZynkLEval): # C Transpiler for Zynk
    ...
    def visit_block(self, expr, origin=True):
        stack = inspect.stack()
        print("[DEBUG] visit_block call stack:")
        for frame in stack[1:4]:
            print(f"  [DEBUG] {frame.function} at {frame.filename}:{frame.lineno}")
        # Solo crear entorno si origin es True
        if origin:
            print(f"[DEBUG] emit_nenv en visit_block (origin={origin})")
            self.emit(self.tables.emit_nenv())
        for stmt in expr.body:
            self.eval(stmt)
        if origin:
            print(f"[DEBUG] emit_ret_env en visit_block (origin={origin})")
    ...
    def visit_while(self, expr, origin=True):
        if origin:
            print(f"[DEBUG] emit_nenv en visit_while (origin={origin})")
            self.emit(self.tables.emit_nenv())
        self.eval(expr.condition)
        ...
        self.emit("}\n")
        if origin:
            print(f"[DEBUG] emit_ret_env en visit_while (origin={origin})")
            self.emit(self.tables.emit_ret_env())
    def visit_for(self, expr):
        print(f"[DEBUG] emit_nenv en visit_for")
        ...
        print(f"[DEBUG] emit_ret_env en visit_for")
