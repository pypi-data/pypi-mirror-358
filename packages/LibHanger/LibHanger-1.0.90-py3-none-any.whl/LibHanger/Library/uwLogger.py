...
def getIndent(depth: int) -> str:
    ...
    depth : int
        ネストの深さ
    """
    return logger_config.LogDepth * depth + " "
...
def getLogPrefix(prefixEmoji: str, prefixString: str):
    ...
    prefixString : str
        ログプレフィックスとして出力する文字列
    """
    return logger_config.LogPrefixFormat.format(prefixEmoji, prefixString.ljust(6) if prefixString else "(unknown)")

def getMethodStartPrefix():
    """
    メソッドStartログのプレフィックスを取得する
    """
    return getLogPrefix(logger_config.LogMethodStartEmoji, logger_config.LogMethodStartString)

def getMethodEndPrefix():
    """
    メソッドEndログのプレフィックスを取得する
    """
    return getLogPrefix(logger_config.LogMethodEndEmoji, logger_config.LogMethodEndString)

def getErrorPrefix():
    """
    Errorログのプレフィックスを取得する
    """
    return getLogPrefix(logger_config.LogErrorEmoji, logger_config.LogErrorString)

def getErrorCaption():
    """
    ErrorログのCaptionを取得する
    """
    return getLogPrefix(logger_config.LogErrorCaptionEmoji, logger_config.LogErrorCaptionString)

def getWarningPrefix():
    """
    Warningログのプレフィックスを取得する
    """
    return getLogPrefix(logger_config.LogWarningEmoji, logger_config.LogWarningString)

def getProcPrefix():
    """
    Procログのプレフィックスを取得する
    """
    return getLogPrefix(logger_config.LogProcEmoji, logger_config.LogProcString)
