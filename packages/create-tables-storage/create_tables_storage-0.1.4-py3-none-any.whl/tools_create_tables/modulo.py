...
class CreateTables:
    ...
    def _mount_container(self):
        ...
        if str(self.mount_point) in mounted:
            print(f"Container '{self.container}' already mounted at {self.mount_point}")
            return

        configs = {
            f"fs.azure.account.key.{self.container}.dfs.core.windows.net": self.auth_key
        }
        try:
    ...
    def _load_tables(self):
        ...
        for entry in self.tables_json_path:
            ...
            try:
                ...
                print(f"Loaded Delta table '{name}' from {path}")
            except Exception as e:
                print(f"Error loading table '{name}' from {path}: {e}")
        ...
        for entry in self.tables_json_path:
            ...
            try:
                if DeltaTable.isDeltaTable(self.spark, str(target)):
                    print(f"Skipping {bq_table}: Delta table exists at {target}")
                    continue
            except Exception:
                pass
            ...
            print(f"Writing to Delta at {target}...")
            ...
            count = df.count()
            print(f"Migrated {bq_table} ({count} rows) → {target}\n")
