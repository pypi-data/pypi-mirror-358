...
from psql_partition.settings import (
    postgres_prepend_local_search_path,
    postgres_reset_local_search_path,
)
from psql_partition.type_assertions import is_sql_with_params
...
class PostgresSchemaEditor(SchemaEditor):
    ...
    def delete_schema(self, name: str, cascade: bool) -> None:
        ...
        sql = self.sql_delete_schema if not cascade else self.sql_delete_schema_cascade
    ...
    def clone_model_structure_to_schema(
        ...
        if django.VERSION < (4, 1):
            ...
            for sequence in sequences:
                ...
                quoted_sequence_fqn = f"{quoted_schema_name}.{quoted_sequence_name}"
    ...
    def clone_model_constraints_and_indexes_to_schema(
        ...
        with postgres_prepend_local_search_path(
            ...
            if model._meta.unique_together:
                self.alter_unique_together(model, tuple(), model._meta.unique_together)
            ...
            for field in model._meta.local_concrete_fields:  # type: ignore[attr-defined]
                ...
                if field.primary_key:
                    with postgres_reset_local_search_path(using=self.connection.alias):
                ...
                if field.remote_field:
                    ...
                    new_field = self._clone_model_field(field, db_constraint=False)
    ...
    def reset_table_storage_setting(self, table_name: str, name: str) -> None:
        ...
        self.execute(
            self.sql_reset_table_storage_setting % (self.quote_name(table_name), name)
        )

    def reset_model_storage_setting(self, model: Type[Model], name: str) -> None:
    ...
    def _clone_model_field(self, field: Field, **overrides) -> Field:
        ...
        cloned_field = field.__class__(*cloned_field_args, **cloned_field_kwargs)
