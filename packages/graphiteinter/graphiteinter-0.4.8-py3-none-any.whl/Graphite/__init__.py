...
#acabei de criar abaixo
    ...
    def createtable(TableId, rows, columns, posx, posy, sizex, sizey, color):
        ...
        if GraphiteInter._root:
            ...
            tree = ttk.Treeview(table_frame, columns=tuple(f"#{i}" for i in range(1, columns + 1)), 
            ...
            if rows > 10:  # Se mais de 10 linhas, adicionar scrollbar
            ...
            GraphiteInter._tables[TableId] = {
                ...
                'rows': rows,
                'columns': columns,
                'data': [[""] * columns for _ in range(rows)]  # Matriz de dados
            }
    ...
    def tableRow(tableId, rowNumber, content):
        ...
        if tableId in GraphiteInter._tables:
            ...
            if isinstance(content, list):
                ...
                while len(content) < table_data['columns']:
                    content.append("")
                content = content[:table_data['columns']]  # Truncar se for muito grande
            else:
                # Se não for lista, converter para lista
                content = [str(content)] + [""] * (table_data['columns'] - 1)
            
            table_data['data'][rowNumber] = content
            
            # Atualizar a visualização da tabela
            tree = table_data['tree']
            items = tree.get_children()
            if rowNumber < len(items):
                tree.item(items[rowNumber], values=content)
            
            print(f"✅ Linha {rowNumber} da tabela '{tableId}' atualizada")
...
def savetolist(valor):
    """
    Recebe um valor e retorna uma lista contendo esse valor.
    Pode ser usada como: a = savetolist(valor)
    """
    return [valor]

def clearlist(lista):
    """
    Esvazia a lista passada como argumento (in-place) e retorna a lista vazia.
    Pode ser usada como: clearlist(minha_lista)
    """
    lista.clear()
    return lista
