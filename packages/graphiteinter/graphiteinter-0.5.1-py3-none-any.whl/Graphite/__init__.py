...
#acabei de criar abaixo
    ...
    def createtable(TableId, rows, columns, posx, posy, sizex, sizey, color):
        """
        Cria uma tabela na interface com todas as bordas visíveis e colunas ajustadas para exibir todo o conteúdo.
        """
        if GraphiteInter._root:
            if TableId in GraphiteInter._tables:
                raise ValueError(f"Já existe uma tabela com o ID '{TableId}'.")

            table_frame = tk.Frame(GraphiteInter._root, bg=color, width=sizex, height=sizey, highlightbackground="#222", highlightthickness=2)
            ...
            style.map(style_name, background=[('selected', '#e0e0ff')])
            style.layout(style_name, style.layout("Treeview"))
            style.configure(style_name + ".Heading", borderwidth=2, relief="solid", font=("Arial", 12, "bold"), background="#f0f0f0")
            ...
            # Configurar colunas (ajustar largura para exibir todo o conteúdo)
            min_width = max(80, sizex // columns)
            for i in range(1, columns + 1):
                tree.heading(f"#{i}", text=f"Coluna {i}")
                tree.column(f"#{i}", width=min_width, minwidth=60, anchor="center", stretch=True)
            ...
            print(f"✅ Tabela '{TableId}' criada com sucesso ({rows}x{columns}) com bordas e ajuste de largura.")
        else:
            raise RuntimeError("A janela principal ainda não foi criada. Use GraphiteInter.create_window() primeiro.")
    ...
    def importtable(tableid, path, posx=0, posy=0):
        ...
        # Ajustar largura das colunas para exibir todo o conteúdo
        font = tkFont.Font(font=GraphiteInter._tables[tableid]['tree'].cget("font"))
        for col in range(columns):
            max_width = font.measure(f"Coluna {col+1}") + 20  # Começa pelo cabeçalho
            for row in range(rows):
                value = str(df.iloc[row, col])
                width = font.measure(value) + 20
                if width > max_width:
                    max_width = width
            GraphiteInter._tables[tableid]['tree'].column(f"#{col+1}", width=max_width, minwidth=60, stretch=True)
        GraphiteInter.make_table_fully_editable(tableid)
        print(f"✅ Tabela '{tableid}' importada de {path} (colunas ajustadas)")
