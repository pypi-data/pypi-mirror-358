...
from aap_ipykernel.compat import kernelspec as ks
from aap_ipykernel.compat import tempdir
...
def add_kernel(
    ...
    kernel_name = []
    display_name = []
    argv = [sys.executable, "-m", "aap_ipykernel"]
    ...
    #----------------------- AAP Override name for ssh only  ---------------
    if interface == "ssh":
        display_name=[name]
    #-----------------------------------------------------------------------
    kernel_json = {
