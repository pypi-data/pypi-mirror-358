...
from aap_ipykernel import RIK_PREFIX, __version__
...
def _setup_logging(verbose):
    ...
    log = logging.getLogger("aap_ipykernel")
    if verbose:
...
def run_login_sleep(login_cmd: str):
    """
    Parse login_cmd and run sshpass ssh command with 'sleep 1'.
    """
    # Example login_cmd: "ssh -o StrictHostKeyChecking=no aiplatform@172.20.0.11"
    m = re.search(r'(\w+)@([\d\.]+)', login_cmd)
    if not m:
        raise ValueError(f"Could not parse login_cmd: {login_cmd}")
...
class RemoteIKernel(object):
    ...
    def launch_ssh(self):
        ...
        self.log.info("Attempt to login with sshpass")
        run_login_sleep(login_cmd)
