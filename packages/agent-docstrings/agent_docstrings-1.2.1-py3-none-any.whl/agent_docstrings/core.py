"""
    ...
    Classes/Functions:
      - Functions:
        - parse_gitignore(gitignore_path: Path) -> Set[str] (line 54)
        - is_path_ignored(path: Path, ignore_patterns: Set[str], root_dir: Path) -> bool (line 77)
        - load_blacklist_whitelist(directory: Path) -> Tuple[Set[str], Set[str]] (line 101)
        - should_process_file(file_path: Path, root_dir: Path, ignore_patterns: Set[str], blacklist: Set[str], whitelist: Set[str]) -> bool (line 136)
        - _get_header_content_lines(classes: List[ClassInfo], functions: List[SignatureInfo], language: str, line_offset: int) -> List[str] (line 206)
...
def _get_header_content_lines(
    ...
    lines = [
        ...
        f"{style.prefix}This docstring is automatically generated by Agent Docstrings v{__version__}",
    ...
    def format_class(ci: ClassInfo, indent: str):
        ...
        for m in ci.methods:
            lines.append(f"{indent}  - {m.signature} (line {m.line + line_offset})")
        for inner_ci in ci.inner_classes:
            format_class(inner_ci, indent + "  ")

    if classes:
        for c in classes:
            format_class(c, style.prefix)
    if functions:
        lines.append(f"{style.prefix}  - Functions:")
        for f in functions:
            lines.append(
                f"{style.prefix}    - {f.signature} (line {f.line + line_offset})"
            )
...
def process_file(path: Path, verbose: bool = False) -> None:
    ...
    try:
        ...
        if not classes and not functions:
            ...
            if cleaned_body.strip() != code_body.strip():
                 path.write_text(
                    (file_prefix + "\n" + cleaned_body).lstrip(),
                    encoding="utf-8",
                    newline="\n",
...
def discover_and_process_files(directories: List[str], verbose: bool = False) -> None:
    ...
    for dir_str in directories:
        ...
        blacklist_patterns, whitelist_patterns = load_blacklist_whitelist(directory)

        for root, dirs, files in os.walk(directory):
            root_path = Path(root)
            
            # Filter directories to avoid walking into ignored ones
            dirs[:] = [d for d in dirs if d not in DEFAULT_IGNORE_DIRS and not is_path_ignored(root_path / d, ignore_patterns, directory)]
