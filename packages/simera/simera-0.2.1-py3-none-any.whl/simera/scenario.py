...
import pandas as pd
from simera import Ratesheet, Shipment, Cost, Config, ZipcodeManager, calculate_cost
...
test_files = [
    ...
    Path(sc.path.resources / 'ratesheets/PAR_UPS_STD.xlsb'),
    Path(sc.path.resources / 'ratesheets/PAR_UPS_STD_MULTI.xlsb'),
    Path(sc.path.resources / 'ratesheets/PAR_UPS_EXP.xlsb'),
    Path(sc.path.resources / 'ratesheets/PAR_DHL_STD.xlsb'),
    Path(sc.path.resources / 'ratesheets/PAR_DHL_STD_MULTI.xlsb'),
]

# test_files = [Path(sc.path.resources /'ratesheets/DC_GB_Northampton.xlsb')]
test_files = [Path(sc.path.resources / 'ratesheets/PAR_DHL_STD.xlsb'), Path(sc.path.resources / 'ratesheets/PAR_DHL_STD_MULTI.xlsb')]
...
# get shipments
# countries = ['DE', 'DK', 'FI', 'NO', 'SE', 'FR', 'BE', 'NL', 'LU', 'AT', 'GB', 'ES', 'IT', 'PT', 'PL', 'LT', 'LV',
#              'EE', 'CH', 'HR', 'IE', 'BG', 'CZ', 'GR', 'HU', 'RO', 'SI', 'SK', 'MT', 'TR']
countries = ['DE']
...
shipments = [Shipment({'display': {'fuck': 123}, 'lane': {'dest_ctry': ctry, 'dest_zip': zm.zipcode_clean_first[ctry]}, 'unit': {'m3': kg / 150, 'shipment': 1, 'kg': kg}}) for ctry in countries for kg in [0.1, 1, 10, 30, 40, 50]]
...
df = pd.DataFrame([shp.cost_summary for shp in shipments_cost]).sort_values(['sh_id', 'cost_total'], ignore_index=True)
df_best = df.sort_values(by=['sh_id', 'cost_total'], ascending=[True, True]).drop_duplicates(subset=['sh_id'], keep='first')
