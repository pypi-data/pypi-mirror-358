...
class Liev:
    ...
    Attributes:
        __api_token (str): The API token for authentication
    ...
    def __init__(self, api_token, base_url=os.getenv('LIEV_BASE_URL','https://api.liev.ai/')):
        """
        Initialize the Liev client.
        
        Args:
            api_token (str): The API token required for authentication. Cannot be None.
            base_url (str, optional): The base URL for the Liev API. 
                                    Defaults to 'https://api.liev.ai/'.
        
        Raises:
            ValueError: If api_token is None.
        """
        if api_token is None:
            raise ValueError("API token is required. Pass api_token=<token> in constructor.")
    ...
    def ask(self, instruction="", input=None, messages=None, function=None, system_msg="", 
            ...
            try_next_on_failure=True) -> str | dict | bytes:
        """
    ...
    def get_openai_client(self):
        ...
        """
        return OpenAI(api_key=self.__api_token, base_url=self.__base_url+'/api/v2alpha1/openai')
