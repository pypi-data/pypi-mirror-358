...
Usage:
    ...
    With Storage::
        ...
        from mgmt_config import storage_account
        
        if storage_account:
            storage_account.upload_blob("container", "blob-name", b"data")
            data = storage_account.download_blob("container", "blob-name")
...
Environment Variables:
    ...
    Storage Configuration:
        ...
        STORAGE_ENABLE_BLOB: Enable blob storage client (default: "true")
        STORAGE_ENABLE_FILE: Enable file storage client (default: "true")
        STORAGE_ENABLE_QUEUE: Enable queue storage client (default: "true")
...
def _get_storage_configuration() -> tuple[Optional[str], bool, bool, bool, Optional[str]]:
    """Get Storage configuration from environment variables.
    
    Returns:
        A tuple containing (storage_url, blob_enabled, 
        file_enabled, queue_enabled, connection_string).
    """
    storage_url = os.getenv("STORAGE_ACCOUNT_URL")
    blob_enabled = os.getenv("STORAGE_ENABLE_BLOB", "true").lower() == "true"
    file_enabled = os.getenv("STORAGE_ENABLE_FILE", "true").lower() == "true"
    queue_enabled = os.getenv("STORAGE_ENABLE_QUEUE", "true").lower() == "true"
    connection_string = LOGGER_CONNECTION_STRING  # Share with logger by default
    
    return storage_url, blob_enabled, file_enabled, queue_enabled, connection_string


(STORAGE_URL, 
 STORAGE_ENABLE_BLOB, 
 STORAGE_ENABLE_FILE, 
 STORAGE_ENABLE_QUEUE, 
 STORAGE_CONNECTION_STRING) = _get_storage_configuration()
...
def _create_storage_client(account_url: str, identity_instance, logger_instance):
    """Create a Storage client for the specified storage account.
    ...
    Returns:
        Configured AzureStorage instance or None if creation fails.
    """
    if not account_url:
        return None
    ...
    return create_azure_storage(
        ...
        enable_blob_storage=STORAGE_ENABLE_BLOB,
        enable_file_storage=STORAGE_ENABLE_FILE,
        enable_queue_storage=STORAGE_ENABLE_QUEUE,
    )
...
# Initialize Storage client (if URL is configured)
PROJECT_CODE = keyvault.get_secret("project-code")
AZURE_RESOURCE_ENVIRONMENT = keyvault.get_secret(
...
storage_account = _create_storage_client(STORAGE_URL, identity, logger)
...
def _validate_and_log_configuration():
    ...
    if not STORAGE_URL:
        logger.info(
            "No Storage Account URL configured. Storage operations will be disabled.",
            extra={"storage_status": "disabled"}
        )
    ...
    logger.info(
        ...
        extra={
            ...
            "storage_enabled": bool(STORAGE_URL),
            "storage_blob_enabled": STORAGE_ENABLE_BLOB if STORAGE_URL else False,
            "storage_file_enabled": STORAGE_ENABLE_FILE if STORAGE_URL else False,
            "storage_queue_enabled": STORAGE_ENABLE_QUEUE if STORAGE_URL else False,
...
__all__ = ["logger", "local_env_manager", "identity", "keyvault", "head_keyvault", "storage_account"]
