...
echo 'SELECT * FROM users WHERE active = 1' | bkmr add --stdin --type snip --title "Active Users" --tags sql,query
...
# Import scripts and markdown files with frontmatter
bkmr import-files ~/scripts/ ~/docs/ --update

# Execute shell scripts without interactive editing
bkmr open 123 --no-edit  # directly execute shell script with ID 123
...
| `import-files` | Import scripts and markdown files with frontmatter parsing |
...
- **File import system**: Import scripts and markdown files with YAML or hash-style frontmatter parsing
- **Stdin support**: Read content directly from stdin for quick additions
- **Non-interactive execution**: Skip editing for shell scripts with `--no-edit` flag
...
# Non-interactive execution - skip editing with --no-edit flag
bkmr open 123 --no-edit  # directly execute shell script

# Disable interactive mode globally via configuration
...
Import scripts and markdown files with structured metadata using frontmatter:

```bash
# Import files from directories
bkmr import-files ~/scripts/ ~/docs/

# Update existing bookmarks when content changes
bkmr import-files ~/scripts/ --update

# Preview changes without making them
bkmr import-files ~/scripts/ --dry-run

# Import with cleanup of removed files
bkmr import-files ~/scripts/ --update --delete-missing
```

**Supported frontmatter formats:**

*YAML frontmatter* (markdown and scripts):
```yaml
---
...
*Hash-style frontmatter* (scripts):
```bash
#!/bin/bash
# name: Database Backup Script
# tags: database, backup, automation
...
- SHA-256 hash tracking for incremental updates
- `.gitignore` support during directory traversal
- Duplicate name handling with `--update` flag
