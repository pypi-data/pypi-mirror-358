...
def _checkout(repo, commit, remote, path, configfile):
    ...
    with git.GitConfigParser(configfile, read_only=False) as conf:
        section = f'lb-checkout "{remote}.{path}"'
...
def get_packages_of(repo, commit):
    ...
    requirements_packages = {path.rsplit("/", 1)[0] for path in requirements_packages}
...
def main():
    ...
    parser.add_argument(
        "commit",
        metavar="branch",
        help="name of the branch/tag/commit used to get data from (e.g. LHCb/master)",
    )
    parser.add_argument(
        "path",
        nargs="?",
    ...
    try:
        ...
        hats = {os.path.dirname(pkg) for pkg in pkgs}
        ...
        if os.path.exists(top_cmake):
            ...
            if end > start:
                subdirs = {l.strip() for l in lines[start + 1 : end]}
                subdirs.update(paths)
                lines[start + 1 : end] = [f"    {l}\n" for l in sorted(subdirs)]
        ...
        if args.do_commit:
            if len(paths) == 1:
                msg = f"added {paths[0]} from {remote} ({args.commit})"
            else:
