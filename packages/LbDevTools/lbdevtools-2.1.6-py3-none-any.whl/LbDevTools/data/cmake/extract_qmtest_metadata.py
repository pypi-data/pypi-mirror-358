...
def fix_test_name(name, pkg):
    ...
    'Package.simple'
    """
    return re.sub(rf"^({pkg.lower()}\.)?", f"{pkg}.", name)
...
def parse_xml(path):
    ...
    try:
        ...
        sys.stderr.write(f"ERROR: could not parse {path}\n{e}\n")
        sys.stderr.flush()
        exit(1)
...
def analyze_deps(pkg, rootdir):
    ...
    for path in find_files(rootdir, ".qmt"):
        ...
        if prereqs:
            print(
                "set_property(TEST {} APPEND PROPERTY DEPENDS {})".format(
                    name, " ".join(prereqs)
                )
            )
...
def analyze_suites(pkg, rootdir):
    ...
    for path in find_files(rootdir, ".qms"):
        ...
        if tree.findall(suites_xpath):
            sys.stderr.write(
                f"WARNING: {path}: suites of suites are not supported yet\n"
            )
            sys.stderr.flush()
    ...
    for test, labels in six.iteritems(test_labels):
        print(
            "set_property(TEST {} APPEND PROPERTY LABELS {})".format(
                test, " ".join(labels)
            )
        )
...
def analyze_disabling(pkg, rootdir):
    ...
    for path in find_files(rootdir, ".qmt"):
        ...
        if skip_test:
            print(f"set_property(TEST {name} APPEND PROPERTY LABELS disabled)")
