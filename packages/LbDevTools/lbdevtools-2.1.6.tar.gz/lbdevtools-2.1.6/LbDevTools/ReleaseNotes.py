...
def rel_project_path(path, to):
    ...
    return "/".join(list(zip(*parts, strict=False))[1]) if parts else ""
...
def find_merge_request_id(repo, merge_commit, second_parent):
    ...
    if m:
        if len(m) > 1:
            logging.warning(
                "Multiple merge requests associated with %s: %r. Taking the first.",
                second_parent,
                [i.group(1) for i in m],
            )
        return int(m[0].group(1))
    logging.debug(
        "Second parent of %s (%s) does not correspond to a merge request ref.",
        merge_commit,
        second_parent,
    )
    ...
    logging.warning(
        "Could not find MR for %s based on second parent (%s) refs or commit "
...
def find_merge_requests_git(project, repo, since, until=""):
    ...
    if len(mrs) != len(iids):
        msg = f"Could not list all {len(iids)} MRs, got {len(mrs)}"
...
def find_merge_requests_milestone(project, milestone_title):
    ...
    if not milestones:
        logging.debug(
            "Milestone %s not found in GitLab project %s", milestone_title, project.name
        )
        return []
    if len(milestones) > 1:
        logging.error(
            "Multiple milestones %s found in GitLab project %s",
            milestone_title,
            project.name,
        )
        sys.exit(1)
...
def find_merge_requests(server, repo, since, milestone):
    ...
    logging.debug("Found these MRs in git history: %s", git_mrs)
    logging.debug("Found these MRs in GitLab: %s", gitlab_mrs)

    if not gitlab_mrs:
        logging.warning(
            "No merge requests found with %s milestone for %s",
            milestone,
            project_fullname,
        )
    else:
        ...
        for mr in git_mrs:
            if mr.id not in [x.id for x in gitlab_mrs]:
                logging.warning("Milestone not set for MR %s", mr.web_url)
        for mr in gitlab_mrs:
            if mr.id not in [x.id for x in git_mrs]:
                logging.warning(
                    "MR %s is not merged in current branch after tag %s (state %s)",
                    mr.web_url,
                    since,
                    mr.state,
                )
...
def get_template(template, template_paths):
    ...
    def select_mrs(mrs, labels, used=None):
        ...
        label_sets = [{l.lower() for l in ls} for ls in label_sets]
        matches = []
        for mr in mrs:
...
""",
    ...
    logging.debug("Searching for templates in %s", template_paths)
    ...
    if os.path.exists(args.output):
        logging.error("Output %s exists, aborting...", args.output)
        sys.exit(1)
    if not args.token:
        try:
    ...
    print(f"Using template {template.filename}")
    ...
    for name, (_, ver, _) in dependencies.items():
        if name == "LCG":
            ...
            project_deps.append(
                f"LCG [{ver}](http://lcginfo.cern.ch/release/{ver}/) {text}"
            )
        else:
            repo = os.path.join(os.path.dirname(args.stack), name)
    ...
    print(f"Release notes draft written to {args.output}.")
