...
ARCH_DEFS = OrderedDict(
    [
        ...
        ("core2", {"pni", "mmx", "sse2", "lm", "ssse3", "sse"}),
        ("x86_64", set()),
    ]
)
...
def _Linux_os():
    ...
    if dist_name in ("redhat", "centos"):
        if "CERN" in open(f"/etc/{dist_name}-release").read():
            dist_name = "slc"
...
os_id = globals().get(f"_{platform.system()}_os", _unknown_os)


def _compiler_version(cmd=None):
    if cmd is None:
        cmd = os.environ.get("CC", "cc")
...
def arch():
    ...
    if sys.platform == "darwin":
        for l in check_output(["sysctl", "-a"], text=True).split("\n"):
            if l.startswith(
                (
