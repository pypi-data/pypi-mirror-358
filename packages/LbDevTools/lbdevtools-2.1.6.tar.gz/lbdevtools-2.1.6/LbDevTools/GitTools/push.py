        ...
        def cleanup(self, _warn=False):
            if self.name and not self._closed:
                ...
                if _warn:
                    ...
                    self._warn("Implicitly cleaning up {1!r}", UserWarning)
...
def commits_cmp(a, b):
    ...
    if a == b:
        return 0
    try:
        next(a.repo.iter_commits(f"{a.hexsha}..{b.hexsha}"))
        return -1
...
def main():
    ...
    while tmp_branch_name in branches:
        tmp_branch_name = f"{args.branch}-tmp{cnt}"
        cnt += 1
    ...
    try:
        repo.remote(args.remote).push(f"{tmp_branch_name}:{args.branch}")
        ...
        with git.GitConfigParser(configfile, read_only=False) as conf:
            for pkg in pkgs:
                section = f'lb-checkout "{args.remote}.{pkg}"'
        ...
        repo.index.commit(
            f"updated {os.path.basename(configfile)} after push of {args.remote}/{args.branch}"
        )
    finally:
        if args.keep_temp:
            logging.warning(
