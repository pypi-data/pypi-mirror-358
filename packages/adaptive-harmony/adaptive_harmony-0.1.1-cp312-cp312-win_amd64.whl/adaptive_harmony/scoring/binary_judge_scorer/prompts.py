...
You will be given all elements of an interaction between human an AI model:
    ...
    The model's completion is under the COMPLETION header.
...
First, you read the CONTEXT, USER QUESTION and COMPLETION.
Then, you analyze the COMPLETION, and assign it a PASS, FAIL or NA score according to the criterion: FAIL if the completion does not meet the criterion, PASS if it does, and NA if the criterion is not applicable to the example. These are the rules to follow:
- You must always evaluate the COMPLETION based solely on the USER QUESTION, and never on an intermediary question that might have been asked in the CONTEXT. The CONTEXT is there for context only.
- Do not include text that is in the CONTEXT to make your judgement; you are evaluating the COMPLETION text only.
...
- If the criterion is conditional, and is not applicable to the specific USER QUESTION + COMPLETION pair, you must score it as NA.
