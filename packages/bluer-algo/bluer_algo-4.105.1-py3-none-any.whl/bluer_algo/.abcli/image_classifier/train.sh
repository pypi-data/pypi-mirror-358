...
function bluer_algo_image_classifier_train() {
    local options=$1
    ...
    local model_object_name=$(bluer_ai_clarify_object $3 image_classifier_model-$(bluer_ai_string_timestamp))

    bluer_ai_eval dryrun=$do_dryrun \
        python3 -m bluer_algo.image_classifier.train \
        train \
