...
def print_act_cal(fname,idx=-1):
  act_list = get_surface_list(fname,idx=idx)
  row =  "{:<8} {:<20} {:<6} {:>6} {:>6}"
  hrow = "{:<8} {:<20} {:<6} {:>6} {:>6}"
...
def plot_surfaces(fname,firstlast=-1,ax_share=None,fname2=None,xmlname=None):
    act_list = get_surface_list(fname,firstlast=firstlast,fname2=fname2)
    if len(act_list) > 0:
...
def plot_gps(fname,ax_share = None):
    ...
    fig,gs,ax_share,ax = create_subplot(10,ttl = 'GPS Sensor',ax_share = ax_share)
    add_flying_bar(ax,fname)
    ...
    ax.legend()
    ax = fig.add_subplot(gs[10],sharex=ax_share)
...
def plot_vt_control(fname,ax_share=None):
    ...
    ax.plot(com['t_ALTITUDE'][com['ALTITUDE']>0.01],com['ALTITUDE'][com['ALTITUDE']>0.01],'.-',color='xkcd:red')
    ax.plot(tgps,gps['altitude'],color='xkcd:green')
    ax.set_ylim([1580,1650])
    setup_subplot(ax,ylabel='Alt')
    ax2 = ax.twinx()
...
def plot_imu(fname,ax_share = None):
    acc,tacc = get_var(fname,'acc')
    gyr,tgyr = get_var(fname,'gyr')
    mag,tmag = get_var(fname,'mag')

    if mag is None:
      fig,gs,ax_share,ax = create_subplot(2,ttl = 'IMU',ax_share = ax_share)
    else:
      fig,gs,ax_share,ax = create_subplot(3,ttl = 'IMU',ax_share = ax_share)
...
def plot_sys_status(fname,ax_share = None,i_factor=1):
    ...
    fig,gs,ax_share,ax = create_subplot(7,ttl = 'System Status',ax_share = ax_share)
    add_flying_bar(ax,fname)
...
def plot_error_list(ax,fname):
  ...
  for k in err_list.keys():
    ax.plot(err_list[k]['system_time'],err_list[k]['value'],'ok')
