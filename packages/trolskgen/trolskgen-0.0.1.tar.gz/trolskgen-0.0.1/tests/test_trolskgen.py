...
def test_build_list_literal() -> None:
    _eq(
        trolskgen.to_source([]),
        "[]",
    )

    def custom_list_converter(o: Any, f: trolskgen.F) -> ast.AST | None:
        if not isinstance(o, list):
            return None

        return f(t("MyList({a:*})", a=[f(n) for n in o]))

    c = trolskgen.Config().prepend_converter(custom_list_converter)
    _eq(
        trolskgen.to_source([], config=c),
        "MyList()",
    )
    _eq(
        trolskgen.to_source([1, 2], config=c),
        "MyList(1, 2)",
    )
