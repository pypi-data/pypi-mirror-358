...
class DevServerOneOf(BaseModel):
    """
    DevServerOneOf
    """ # noqa: E501
    repo_id: StrictStr = Field(alias="repoId")
    kind: StrictStr
    __properties: ClassVar[List[str]] = ["repoId", "kind"]

    @field_validator('kind')
    def kind_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['repo']):
            raise ValueError("must be one of enum values ('repo')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of DevServerOneOf from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of DevServerOneOf from a dict"""
        if obj is None:
            return None
