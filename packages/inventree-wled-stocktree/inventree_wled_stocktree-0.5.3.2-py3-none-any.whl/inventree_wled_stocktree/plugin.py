...
class WledInventreePlugin(UrlsMixin, LocateMixin, SettingsMixin, InvenTreePlugin):
    ...
    def _set_led(self, target_led: int = None, ip: str = None, request=None, turn_off_others=True):
        ...
        print(f"[DEBUG] _set_led called with target_led={target_led}, ip={ip}, turn_off_others={turn_off_others}")
        if not ip:
            print("[DEBUG] No IP address provided for WLED")
            if request:
                messages.add_message(request, messages.WARNING, "No IP address provided for WLED")
            return

        max_leds = self.get_setting("MAX_LEDS")
        print(f"[DEBUG] max_leds from settings: {max_leds}")
        ...
        if turn_off_others:
            try:
                print(f"[DEBUG] Turning off all LEDs on {ip}")
                requests.post(
                    base_url,
                ...
                print(f"[DEBUG] Exception while turning off all LEDs: {e}")

        if target_led is not None:
            try:
                print(f"[DEBUG] Setting LED {target_led} on {ip} to {color_marked}")
                requests.post(
                    base_url,
                ...
                print(f"[DEBUG] Exception while setting LED {target_led}: {e}")

    def set_leds(self, ip: str, segments: list, request=None):
        """Set a color on a range of LEDs (min to max) on a given WLED IP."""
        print(f"[DEBUG] set_leds called with ip={ip}, segments={segments}")
        if not ip:
            print("[DEBUG] No IP address provided for WLED")
            if request:
                messages.add_message(request, messages.WARNING, "No IP address provided for WLED")
        ...
        print(f"[DEBUG] Sending payload to {base_url}: {payload}")
        ...
        try:
            ...
            print(f"[DEBUG] Exception while setting LEDs: {e}")
            if request:
        ...
        def turn_off_range():
            import time
            print(f"[DEBUG] Sleeping 10s before turning off segments on {ip}")
            time.sleep(10)
            off_segments = []
            for seg in segments:
                off_seg = seg.copy()
            ...
            try:
                print(f"[DEBUG] Turning off segments on {ip} with payload: {off_payload}")
                requests.post(base_url, json=off_payload, timeout=3)
            except Exception as e:
                logger.warning(f"Failed to turn off segments on {ip}: {e}")
                print(f"[DEBUG] Exception while turning off segments: {e}")
