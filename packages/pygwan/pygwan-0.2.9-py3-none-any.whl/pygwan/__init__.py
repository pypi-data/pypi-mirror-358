...
class WhatsApp(object):
    ...
    def upload_media(self, media_path: str, category: str) -> str:
        """
        Upload media file to the WhatsApp Cloud API and return the uploaded media ID.

        Args:
            media_path (str): Local file system path to the media file.
            category (str): One of 'image', 'video', 'audio', or 'document'. Determines
                the allowed file types and MIME type for upload.

        Returns:
            str: The media ID assigned by the WhatsApp Cloud API upon successful upload.

        Raises:
            FileNotFoundError: If the provided media_path does not exist.
            ValueError: If the category is invalid or the file extension is not supported.
            
        logging: For any non-200 HTTP response from the API.

        Example:
            >>> media_id = upload_media('/path/to/photo.png', 'image')
            >>> print(media_id)
            '1234567890'
        """
        MEDIA_CATEGORIES = {
        ...
        if category not in MEDIA_CATEGORIES:
            raise ValueError(f"Unsupported media category '{category}'. Choose from {list(MEDIA_CATEGORIES.keys())}.")

        # Determine file extension and MIME type
        ext = os.path.splitext(media_path)[1].lower()
        category_info = MEDIA_CATEGORIES[category]
        if ext not in category_info['extensions']:
            raise ValueError(
                f"File extension '{ext}' not supported for category '{category}'."
                f" Supported extensions: {category_info['extensions']}"
            )
        # Guess MIME type or use default
        mime_type, _ = mimetypes.guess_type(media_path)
        if not mime_type:
            mime_type = category_info['default_mime']
        ...
        if response.status_code != 200:
            ...
            logging.error(f"Upload failed ({response.status_code}): {error_msg}")

        media_id = result.get('id') or result.get('media')[0].get('id')
        return media_id
