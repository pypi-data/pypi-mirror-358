...
html_template = """<html><body><h2>Dear {name},</h2><p>{content}</p><p>Regards,<br>{sender_name}</p></body></html>"""
feedback_html_template = """<html><body><h2>Feedback from {name}</h2><p>Mobile: {mobile}</p><p>{message}</p></body></html>"""
    ...
    def fetch_notifications(self):
        try:
            ...
            if isinstance(data, dict):
                data = data.get("notifications", [data])
            if not isinstance(data, list):
                raise ValueError("Invalid notification format")
            for item in data:
                if isinstance(item, dict):
                    title = item.get('title', 'No Title')
                    body = item.get('body') or 'No Content'
                    self.notification_area.insert(END, f"📌 {title}\n{body}\n{'-'*80}\n")
                else:
