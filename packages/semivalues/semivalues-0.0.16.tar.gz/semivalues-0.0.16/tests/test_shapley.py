...
def test_exact_shapley(game_utility_u):
    result_array = shapley.exact(game_utility_u, 3)
    assert (np.allclose(result_array, [390, 30, 120]))


def test_approx_shapley(game_utility_u):
