...
class ReaderImg(ReaderInterface):
	...
	def compare(F,compare_floats,param_reader,param_is_ref=True):
		O=False;J=param_is_ref;I=param_reader;P=F if J else I;Q=F if not J else I;V=F.reader_options;W=O;X=O;A=P.img;D=Q.img;Y,K=compare_floats.compare_errors.add_group('pixels','img')
		...
		if E:
			...
			for((C,B),T,U)in E[:10]:logging.debug(f"Pixel at ({C}, {B}) differs: {T} vs {U}")
			H=ImageChops.difference(A,D);G=data_png_base64(H)
