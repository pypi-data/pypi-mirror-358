...
class Classifier(BaseClassifer):
    ...
    def _get_one_sided_type(self, toks: list[Token], style_guide: str = StyleGuide.WRITING) -> str:
        """
        ...
        if self.is_filler_word(toks) and style_guide == StyleGuide.SPEAKING:
            return ParentDisfluencyType.DISFLUENCY + SeparatorTypes.COLON + ChildrenFluencyType.FILLER_PAUSE

        if self.is_repetition(toks) and style_guide == StyleGuide.SPEAKING:
            return ParentDisfluencyType.DISFLUENCY + SeparatorTypes.COLON + ChildrenFluencyType.REPETITION
        if self.is_false_start(toks) and style_guide == StyleGuide.SPEAKING:
    ...
    def _get_two_sided_type(self, o_toks: list[Token], c_toks: list[Token], style_guide: str = StyleGuide.WRITING) -> str:
        """
        ...
        # Repetition error
        if self.is_repetition(o_toks) and style_guide == StyleGuide.SPEAKING:
            return ParentDisfluencyType.DISFLUENCY + SeparatorTypes.COLON + ChildrenFluencyType.REPETITION

        if self.is_false_start(o_toks) and style_guide == StyleGuide.SPEAKING:
            return ParentDisfluencyType.DISFLUENCY + SeparatorTypes.COLON + ChildrenFluencyType.FALSE_START
    ...
    def _classify(self, edit: Edit, style_guide: str = StyleGuide.WRITING) -> Edit:
        ...
        # Missing
            ...
            op = Operator.MISSING
            cat = self._get_one_sided_type(edit.corrected.tokens, style_guide=style_guide)
            edit.edit_type = op + SeparatorTypes.COLON + cat
        # Unnecessary
        elif edit.original.tokens and not edit.corrected.tokens:
            op = Operator.UNNECESSARY
            cat = self._get_one_sided_type(edit.original.tokens, style_guide=style_guide)
            edit.edit_type = op + SeparatorTypes.COLON + cat
        ...
        # Replacement and special cases
            ...
            # Classify the edit as if the last token wasn't there
                ...
                edit = self._classify(edit=edit, style_guide=style_guide)
            ...
            # Replacement
                ...
                op = Operator.REPLACE
                cat = self._get_two_sided_type(edit.original.tokens, edit.corrected.tokens, style_guide=style_guide)
