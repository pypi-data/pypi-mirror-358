...
class InteractiveSAM2Annotator(tk.Tk):
    ...
    def load_model(self, model_path, config_path=None):
        ...
        try:
            ...
            # 决定用于 build_sam2 的 config_file 参数
            final_config_param = None

            # 优先使用作为文件路径传入的 config_path
            if config_path and os.path.exists(config_path):
                final_config_param = config_path
                print(f"使用提供的文件系统路径加载配置: {final_config_param}")
            else:
                # 如果提供的 config_path 不存在或未提供，则尝试使用包资源路径
                # 这要求 'sam2_annotator' 是一个可导入的包，并且配置在其中
                # 你的Mac路径是 /opt/.../site-packages/sam2_annotator/configs/sam2_hiera_l.yaml
                # 对应的包路径应该是 sam2_annotator.configs.sam2.sam2_hiera_l (没有.yaml后缀)
                if "large" in model_path.lower():
                    final_config_param = "sam2_annotator.configs.sam2.sam2_hiera_l"
                else:  # 假设是 base 或其他小模型
                    final_config_param = "sam2_annotator.configs.sam2.sam2_hiera_b"  # 假设存在b模型
                print(f"尝试从包资源加载配置: {final_config_param}")

            if final_config_param is None:
                raise ValueError("无法确定 SAM2 配置文件的路径。请检查模型路径或手动提供配置路径。")

            # 调用 build_sam2
            self.model = build_sam2(final_config_param, model_path, device=self.device)
            print("模型加载成功")
        except Exception as e:
            print(f"加载模型失败: {e}")
            traceback.print_exc()
            messagebox.showerror("模型加载失败",
                                 f"无法加载SAM2模型: {e}\n请检查模型和配置文件路径是否正确，或者SAM2库是否正确安装。")
            self.destroy()
...
def main():
    """运行 SAM2 标注器应用的主函数。"""
    # 确定应用程序的根路径
    # 如果是 PyInstaller 打包的执行文件，其根路径是可执行文件所在的目录
    if getattr(sys, 'frozen', False):
        application_root = os.path.dirname(sys.executable)
    else:
        # 如果是直接运行 Python 脚本
        # 获取当前脚本的目录 (例如: .../EEBDsam2_annotator_project/src/sam2_annotator)
        script_dir = os.path.dirname(os.path.abspath(__file__))

        # 根据你提供的项目结构图，app.py 在 src/sam2_annotator 里面
        # 所以项目的根目录 EEBDsam2_annotator_project 应该是 app.py 所在目录的父目录的父目录的父目录
        # (即向上三级目录)
        application_root = os.path.abspath(os.path.join(script_dir, "..", "..", ".."))

    # --- 用户: 请根据你的实际项目结构和文件位置进行验证和调整 ---
    # 使用 os.path.join 构建跨平台兼容的路径。
    # 确保这些路径与你项目实际的文件夹结构相符。
    # 假设你的项目根目录 (application_root) 如下：
    # EEBDsam2_annotator_project/
    # ├── weights/                     <-- 模型权重通常在这里
    # ├── src/
    # │   └── sam2_annotator/
    # │       ├── configs/             <-- 你的配置文件通常在这里 (sam2_hiera_l.yaml)
    # │       └── app.py
    # └── VOCdevkit/                   <-- 你的图像目录通常在这里

    # 1. 模型权重路径
    # 假设 sam2_hiera_large.pt 位于 application_root/weights/
    model_path_to_use = os.path.join(application_root, "weights", "sam2_hiera_large.pt")

    # 2. 配置文件路径
    # 优先尝试加载项目本地的配置文件。
    # 假设 sam2_hiera_l.yaml 位于 application_root/src/sam2_annotator/configs/sam2/
    local_config_path_attempt = os.path.join(application_root, "src", "sam2_annotator", "configs", "sam2",
                                             "sam2_hiera_l.yaml")

    # 根据 local_config_path_attempt 是否存在来决定最终传递给 InteractiveSAM2Annotator 的 config_path 参数。
    # 如果本地文件系统路径不存在，我们将传递 None，指示 InteractiveSAM2Annotator.load_model 尝试从已安装的包资源中加载。
    config_path_to_use = None  # 默认设置为 None
    if os.path.exists(local_config_path_attempt):
        config_path_to_use = local_config_path_attempt
        print(f"检测到本地配置文件: {config_path_to_use}")
    else:
        print(f"警告: 应用程序目录中的配置文件 '{local_config_path_attempt}' 未找到。将尝试从SAM2安装包中加载配置。")
        # 如果本地文件不存在，load_model 方法会尝试使用 'sam2_annotator.configs.sam2.sam2_hiera_l' 这样的包路径。

    # 3. 标注输出目录
    # 假设 annotations_sam2_tool 文件夹位于 application_root/
    output_dir_to_use = os.path.join(application_root, "annotations_sam2_tool")

    # 4. 初始图像目录
    # 假设 VOCdevkit/VOC2007/JPEGImages_test_small 位于 application_root/
    image_dir_to_use = os.path.join(application_root, "VOCdevkit", "VOC2007", "JPEGImages_test_small")
    ...
    if not os.path.exists(model_path_to_use):
        ...
        messagebox.showerror("启动错误",
                             f"SAM2模型文件未找到: {model_path_to_use}\n请检查模型文件是否存在于正确的位置，并重新下载确保完整性。")
        sys.exit(1)
    else:
        print(f"模型文件已找到: {model_path_to_use}")

    # 如果初始图像目录不存在，给出警告但仍允许启动应用，用户可以后续加载
    if not os.path.exists(image_dir_to_use):
        print(f"警告: 初始图像目录 '{image_dir_to_use}' 不存在。您可以在应用启动后手动加载图像。")
        # 可以选择将 image_dir_to_use 设置为 None，或者保持原样让应用处理
        # 例如：image_dir_to_use = None

    # 创建并运行应用
    app = InteractiveSAM2Annotator(
        model_path=model_path_to_use,
        config_path=config_path_to_use,  # 这里传递的可能是文件系统路径，也可能是 None
