...
def main():
    """运行 SAM2 标注器应用的主函数。"""
    # 确定应用程序的根路径
    if getattr(sys, 'frozen', False): # 如果是 PyInstaller 打包的
        application_root = os.path.dirname(sys.executable)
    else:
        # 获取当前脚本的目录
        script_dir = os.path.dirname(os.path.abspath(__file__))
        # 根据你提供的项目结构图，app.py 在 src/sam2_annotator 里面
        # 所以项目的根目录应该是 app.py 的父目录的父目录的父目录
        # EEBDsam2_annotator_project/src/sam2_annotator/app.py
        # root                         ^    ^           ^
        #                             -3   -2          -1 (script_dir)
