...
class InteractiveSAM2Annotator(tk.Tk):
    ...
    def load_model(self, model_path, config_path=None):
        ...
        try:
            ...
            # 确保 config_path 是一个文件系统路径，并且存在
            # 无论从 main 函数传入的是什么，这里我们只接受并验证文件系统路径
            if config_path is None or not os.path.exists(config_path):
                # 如果 config_path 是 None 或文件不存在，尝试从本地项目目录推断一个绝对路径
                # 警告用户，但允许尝试加载
                if config_path is None:
                    print("警告: 未提供明确的配置文件路径，将尝试从应用程序根目录推断。")
                else:
                    print(f"警告: 提供的配置文件路径 '{config_path}' 不存在。将尝试从应用程序根目录推断。")

                # 这个逻辑需要与 main 函数中 application_root 的计算方式以及项目结构精确匹配
                # 假设 config 文件在 application_root/src/sam2_annotator/configs/sam2/
                # 临时计算一个推断的绝对路径
                temp_script_dir = os.path.dirname(os.path.abspath(__file__))
                temp_application_root = os.path.abspath(os.path.join(temp_script_dir, "..", "..", ".."))

                inferred_config_name = "sam2_hiera_l.yaml" if "large" in model_path.lower() else "sam2_hiera_b.yaml"
                inferred_config_path = os.path.join(temp_application_root, "src", "sam2_annotator", "configs", "sam2",
                                                    inferred_config_name)

                if os.path.exists(inferred_config_path):
                    final_config_param = inferred_config_path
                    print(f"使用推断的本地文件系统路径加载配置: {final_config_param}")
                else:
                    raise FileNotFoundError(f"推断的配置文件 '{inferred_config_path}' 也不存在。请手动检查文件路径。")
            else:
                final_config_param = config_path
                print(f"使用传入的文件系统路径加载配置: {final_config_param}")

            # 调用 build_sam2，确保传递的是一个文件系统路径
            # Hydra 的 compose(config_name=...) 默认是查找文件系统路径的
            # 当你传递 'pkg://...' 时，它会去寻找包资源
            # 当你传递一个文件系统路径时，它会直接加载该文件
...
def main():
    ...
    # 2. 配置文件路径：直接使用项目本地的绝对路径
    # 假设 sam2_hiera_l.yaml 位于 application_root/src/sam2_annotator/configs/sam2/
    config_path_to_use = os.path.join(application_root, "src", "sam2_annotator", "configs", "sam2", "sam2_hiera_l.yaml")
    ...
    if not os.path.exists(config_path_to_use):
        ...
        print(f"配置文件已找到: {config_path_to_use}")
    ...
    app = InteractiveSAM2Annotator(
        ...
        config_path=config_path_to_use, # 明确传递文件系统路径
