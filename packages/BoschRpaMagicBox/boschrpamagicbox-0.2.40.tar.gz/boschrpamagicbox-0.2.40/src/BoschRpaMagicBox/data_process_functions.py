...
def collect_tables_from_pdf_file(pdf_file_path, page_number=1, table_index=1, first_column_name='', extract_all_pages=False, extract_all_tables=False):
    """
...
def save_pdf_table_into_excel(user_name, password, server_name, share_name, port, pdf_file_path, page_number, table_index, first_column_name, save_column_names, excel_file_path,
    ...
    if not sheet_name:
        sheet_name = 'Sheet1'
    pdf_table_df = collect_tables_from_pdf_file(pdf_file_path, page_number, table_index, first_column_name, extract_all_pages, extract_all_tables)
