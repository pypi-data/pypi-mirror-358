...
def extract_and_update_acronyms(workspace_id, dataset_id, endpoint, headers, batch_size=50):
    ...
    for i in range(0, len(dataset_groups), batch_size):
        batch = dataset_groups[i:i + batch_size]
        if not batch:
            continue
        ...
        print(f"\n📤 Sending batch {i//batch_size + 1} to AI for Dataset_ID={dataset_id}")
        response = call_openai_with_retry(messages)
        if not response:
        ...
        print(f"✅ Received {len(acronym_list)} acronyms for batch {i//batch_size + 1}")
...
def ai_run(workspace_id, dataset_id):
    ...
    extract_and_update_acronyms(workspace_id, dataset_id, batch_size=50, endpoint=endpoint, headers=headers)
...
# Main ai_run_acro orchestrator function
def ai_run_acro(workspace_id, dataset_id):
    endpoint, headers = get_openai_auth_and_config()
    sync_and_update_acronyms_extraction()
    extract_and_update_acronyms(workspace_id, dataset_id, batch_size=50, endpoint=endpoint, headers=headers)
    return "✅ Operation Completed for Measure Definition, Complexcity and Acronym_Extraction"
