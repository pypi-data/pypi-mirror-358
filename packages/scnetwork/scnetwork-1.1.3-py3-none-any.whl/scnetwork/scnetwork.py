...
class SCNet():
    def __init__(self, device=None, user_id=0, parallel_prediction=False, minimum_probability_for_candidates=0.9, show_errors=True):
        try:
            ...
            try:
                ...
                from torch.nn.functional import pad as torch_pad, cosine_similarity
            ...
            for index, vector_x in enumerate(vector_matrix):
                ...
                for element in search_vector:
                    if element in target_vector: probability += 1
                        ...
                        probability += abs(element - closest) / max((1, element, closest))
                probability /= length
    ...
    def __vectorComparison(self, input_vector=[], vector_matrix=[]):
        try:
            ...
            def _equalize_arrays(source_array=[], target_array=[]):
                def __zeros_like(item=[]):
                    if isinstance(item, list): return [__zeros_like(sub_item) for sub_item in item]
                    return 0
                def __align(source=[], target=[]):
                    if isinstance(target, list):
                        result = []
                        for index, tgt_item in enumerate(target):
                            if isinstance(source, list) and index < len(source): result.append(__align(source[index], tgt_item))
                            else: result.append(__zeros_like(tgt_item))
                        return result
                    if isinstance(source, list):
                        if source: return __align(source[0], target)
                        return 0
                    return source
                return __align(source=source_array, target=target_array)
            def _tensor_to_always_list(tensor=[]):
                def __contains_tensor(value):
                    if isinstance(value, list):
                        for element in value:
                            if __contains_tensor(element): return True
                        return False
                    return hasattr(value, 'tolist')
                def __convert(value):
                    if isinstance(value, list): return [__convert(element) for element in value]
                    if hasattr(value, 'tolist'): return __convert(value.tolist())
                    return value
                if isinstance(tensor, list) and not __contains_tensor(tensor): return tensor
                return __convert(tensor)
            ...
            if torch_available or numpy_available:
                ...
                def _pad_tensors_to_same_shape(source_tensor=[], target_tensor=[]):
                    if source_tensor.dim() == 1: source_tensor = source_tensor.unsqueeze(0)
                    if target_tensor.dim() == 1: target_tensor = target_tensor.unsqueeze(0)
                    if source_tensor.shape[1] < target_tensor.shape[1]:
                        padding = target_tensor.shape[1] - source_tensor.shape[1]
                        source_tensor = self.__torch_pad(source_tensor, (0, padding))
                    elif target_tensor.shape[1] < source_tensor.shape[1]:
                        padding = source_tensor.shape[1] - target_tensor.shape[1]
                        target_tensor = self.__torch_pad(target_tensor, (0, padding))
                    return source_tensor, target_tensor
                for vector in vector_matrix:
                    length_vector = len(vector)
                    if length_query_tensor > 0 and length_query_tensor != length_vector: query_tensor = self.__as_tensor(_equalize_arrays(source_array=_tensor_to_always_list(tensor=query_tensor), target_array=vector), device=computation_device).float()
                    if length_query_norm > 0 and length_query_norm != length_vector: query_norm = self.__asarray(_equalize_arrays(source_array=_tensor_to_always_list(tensor=query_norm), target_array=vector), float)
                    ...
                    if len(vector_batch) >= batch_size:
                        if torch_available:
                            ...
                            try: similarity_tensor = self.__cosine_similarity(tensor_batch, query_tensor.unsqueeze(0), dim=1)
                            except:
                                tensor_batch, query_tensor = _pad_tensors_to_same_shape(source_tensor=tensor_batch, target_tensor=query_tensor)
                                similarity_tensor = self.__cosine_similarity(tensor_batch, query_tensor, dim=1)
