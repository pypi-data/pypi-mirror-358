...
def getSpritesBin(tiles, outFolder):
    if len(tiles) == 32:
...
def getSpritesAsm(tiles, outFolder):
    if len(tiles) == 32:
        tiles = prepareTiles(tiles)
    else:
        tiles = prepareTilesWithOutMirror(tiles)

    with open(str(Path(outFolder + "/sprites.bas")), "w") as f:
        f.write("Asm\n")
        f.write("SPRITE_BUFFER:\n")
        for idx, tile in enumerate(tiles):
            # Cada sprite ocupa 16x16 bits = 32 bytes (asumiendo 2 bytes por fila)
            # Si tus sprites son de 8x8, ajusta a 8 bytes por sprite
            # Aquí se asume que cada tile es una lista de 32 bytes
            for i in range(0, len(tile), 8):
                chunk = tile[i:i+8]
                line = "    DEFB " + ", ".join(f"{b:03X}h" for b in chunk)
                f.write(line + "\n")
        f.write("SPRITE_INDEX:\n")
        for idx in range(len(tiles)):
            offset = idx * len(tiles[0])
            f.write(f"    DEFW (SPRITE_BUFFER + {offset})\n")
        f.write("SPRITE_COUNT:\n")
        f.write(f"    DEFB {len(tiles)}\n")
        f.write("End Asm\n")
