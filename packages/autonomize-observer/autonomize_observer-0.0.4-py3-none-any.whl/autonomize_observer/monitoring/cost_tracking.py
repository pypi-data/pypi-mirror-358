...
DEFAULT_COST_RATES = {
    ...
    # GPT-4o series
    "gpt-4o": {"input": 0.001, "output": 0.004, "provider": "OpenAI"},  # Updated Aug 2024 pricing
    ...
    "gpt-4o-mini-2024-07-18": {"input": 0.00015, "output": 0.0006, "provider": "OpenAI"},
    ...
    "claude-3.5-haiku": {"input": 0.0008, "output": 0.004, "provider": "Anthropic"},  # Updated pricing
    "claude-3-5-haiku": {"input": 0.0008, "output": 0.004, "provider": "Anthropic"},
    "claude-3.5-haiku-20241022": {"input": 0.0008, "output": 0.004, "provider": "Anthropic"},
    ...
    # Gemini 2.5 series
    "gemini-2.5-pro": {"input": 0.00125, "output": 0.01, "provider": "Google"},  # <=200k tokens
    "gemini-2.5-pro-long": {"input": 0.0025, "output": 0.015, "provider": "Google"},  # >200k tokens
    ...
    "gemini-2.0-flash-lite": {"input": 0.000075, "output": 0.0003, "provider": "Google"},
    
    # Gemini 1.5 series
    "gemini-1.5-pro": {"input": 0.00125, "output": 0.005, "provider": "Google"},  # <=128k tokens
    "gemini-1.5-pro-long": {"input": 0.0025, "output": 0.01, "provider": "Google"},  # >128k tokens
    "gemini-1.5-pro-latest": {"input": 0.00125, "output": 0.005, "provider": "Google"},
    "gemini-1.5-flash": {"input": 0.000075, "output": 0.0003, "provider": "Google"},  # <=128k tokens
    "gemini-1.5-flash-long": {"input": 0.00015, "output": 0.0006, "provider": "Google"},  # >128k tokens
    "gemini-1.5-flash-8b": {"input": 0.0000375, "output": 0.00015, "provider": "Google"},
    ...
    # Perplexity Models
    "llama-3.1-sonar-small": {"input": 0.0002, "output": 0.0002, "provider": "Perplexity"},
    "llama-3.1-sonar-large": {"input": 0.001, "output": 0.001, "provider": "Perplexity"},
    ...
    # Nvidia Models
    "llama-3.1-nemotron-70b": {"input": 0.00035, "output": 0.00045, "provider": "Nvidia"},
...
class CostTracker:
    ...
    def _guess_provider(self, model_name: str) -> str:
        ...
        if any(name in model_lower for name in ["gpt", "openai", "o1", "o3", "o4", "davinci", "babbage", "whisper", "tts"]):
            return "OpenAI"
            ...
            return "Meta"
        elif any(name in model_lower for name in ["mistral", "mixtral", "codestral", "pixtral"]):
            return "Mistral"
    ...
    def get_model_rates(self, model_name: str) -> Dict[str, float]:
        ...
        if "gpt-4.1" in model_lower:
            if "nano" in model_lower:
                rates = DEFAULT_COST_RATES.get("gpt-4.1-nano", {"input": 0.0001, "output": 0.0004}).copy()
            elif "mini" in model_lower:
                rates = DEFAULT_COST_RATES.get("gpt-4.1-mini", {"input": 0.0004, "output": 0.0016}).copy()
            else:
                rates = DEFAULT_COST_RATES.get("gpt-4.1", {"input": 0.002, "output": 0.008}).copy()
            logger.info(f"🔄 Using GPT-4.1 rates for variant: {model_name}")
        elif "gpt-4o" in model_lower:
            rates = DEFAULT_COST_RATES.get("gpt-4o", {"input": 0.001, "output": 0.004}).copy()
            logger.info(f"🔄 Using gpt-4o rates for variant: {model_name}")
        elif "gpt-4" in model_lower:
            rates = DEFAULT_COST_RATES.get("gpt-4", {"input": 0.03, "output": 0.06}).copy()
            logger.info(f"🔄 Using gpt-4 rates for variant: {model_name}")
        elif "gpt-3.5" in model_lower or "gpt-35" in model_lower:
            rates = DEFAULT_COST_RATES.get("gpt-3.5-turbo", {"input": 0.0005, "output": 0.0015}).copy()
            ...
            if "o4" in model_lower:
                rates = DEFAULT_COST_RATES.get("o4-mini", {"input": 0.0006, "output": 0.0024}).copy()
            elif "o3" in model_lower:
                if "mini" in model_lower:
                    rates = DEFAULT_COST_RATES.get("o3-mini", {"input": 0.0005, "output": 0.0015}).copy()
                else:
                    rates = DEFAULT_COST_RATES.get("o3", {"input": 0.001, "output": 0.004}).copy()
            elif "mini" in model_lower:
                rates = DEFAULT_COST_RATES.get("o1-mini", {"input": 0.003, "output": 0.012}).copy()
            else:
                rates = DEFAULT_COST_RATES.get("o1", {"input": 0.015, "output": 0.06}).copy()
        ...
        # Anthropic models
            ...
            if "4" in model_lower:
                if "opus" in model_lower:
                    rates = DEFAULT_COST_RATES.get("claude-4-opus", {"input": 0.015, "output": 0.075}).copy()
                else:
                    rates = DEFAULT_COST_RATES.get("claude-4-sonnet", {"input": 0.003, "output": 0.015}).copy()
                logger.info(f"🔄 Using Claude 4 rates for variant: {model_name}")
            elif "3.7" in model_lower or "3-7" in model_lower:
                rates = DEFAULT_COST_RATES.get("claude-3.7-sonnet", {"input": 0.003, "output": 0.015}).copy()
                logger.info(f"🔄 Using Claude 3.7 rates for variant: {model_name}")
            elif "haiku" in model_lower and ("3.5" in model_lower or "3-5" in model_lower):
                rates = DEFAULT_COST_RATES.get("claude-3.5-haiku", {"input": 0.0008, "output": 0.004}).copy()
                logger.info(f"🔄 Using claude-3.5-haiku rates for variant: {model_name}")
            elif "3.5" in model_lower or "3-5" in model_lower:
                rates = DEFAULT_COST_RATES.get("claude-3.5-sonnet", {"input": 0.003, "output": 0.015}).copy()
                logger.info(f"🔄 Using claude-3.5-sonnet rates for variant: {model_name}")
            else:
                rates = DEFAULT_COST_RATES.get("claude-3-sonnet", {"input": 0.003, "output": 0.015}).copy()
            ...
            if "2.5" in model_lower:
                if "flash" in model_lower:
                    if "lite" in model_lower:
                        rates = DEFAULT_COST_RATES.get("gemini-2.5-flash-lite", {"input": 0.0001, "output": 0.0004}).copy()
                    else:
                        rates = DEFAULT_COST_RATES.get("gemini-2.5-flash", {"input": 0.0003, "output": 0.0025}).copy()
                else:
                    rates = DEFAULT_COST_RATES.get("gemini-2.5-pro", {"input": 0.00125, "output": 0.01}).copy()
                logger.info(f"🔄 Using Gemini 2.5 rates for variant: {model_name}")
            elif "2.0" in model_lower or "2-0" in model_lower:
                if "flash" in model_lower:
                    if "lite" in model_lower:
                        rates = DEFAULT_COST_RATES.get("gemini-2.0-flash-lite", {"input": 0.000075, "output": 0.0003}).copy()
                    else:
                        rates = DEFAULT_COST_RATES.get("gemini-2.0-flash", {"input": 0.00015, "output": 0.0006}).copy()
                else:
                    rates = DEFAULT_COST_RATES.get("gemini-2.0-pro", {"input": 0.00125, "output": 0.005}).copy()
                logger.info(f"🔄 Using Gemini 2.0 rates for variant: {model_name}")
            elif "1.5" in model_lower:
                if "flash" in model_lower:
                    rates = DEFAULT_COST_RATES.get("gemini-1.5-flash", {"input": 0.000075, "output": 0.0003}).copy()
                else:
                    rates = DEFAULT_COST_RATES.get("gemini-1.5-pro", {"input": 0.00125, "output": 0.005}).copy()
                logger.info(f"🔄 Using Gemini 1.5 rates for variant: {model_name}")
            else:
                rates = DEFAULT_COST_RATES.get("gemini-1.0-pro", {"input": 0.0005, "output": 0.0015}).copy()
                logger.info(f"🔄 Using gemini-1.0-pro rates for variant: {model_name}")
        
        # Meta Llama models
        elif "llama" in model_lower:
            if "4" in model_lower:
                rates = DEFAULT_COST_RATES.get("llama-4-maverick", {"input": 0.0005, "output": 0.0005}).copy()
            elif "3.3" in model_lower:
                rates = DEFAULT_COST_RATES.get("llama-3.3-70b", {"input": 0.00035, "output": 0.0004}).copy()
            elif "3.2" in model_lower:
                rates = DEFAULT_COST_RATES.get("llama-3.2-11b", {"input": 0.00006, "output": 0.00006}).copy()
            elif "3.1" in model_lower:
                if "405b" in model_lower:
                    rates = DEFAULT_COST_RATES.get("llama-3.1-405b", {"input": 0.00095, "output": 0.00095}).copy()
                else:
                    rates = DEFAULT_COST_RATES.get("llama-3.1-70b", {"input": 0.00035, "output": 0.0004}).copy()
            elif "3" in model_lower:
                rates = DEFAULT_COST_RATES.get("llama-3-70b", {"input": 0.00035, "output": 0.0004}).copy()
            else:
                rates = DEFAULT_COST_RATES.get("llama-2-70b", {"input": 0.00035, "output": 0.0004}).copy()
            logger.info(f"🔄 Using Llama rates for variant: {model_name}")
        
        # Mistral models
        elif "mistral" in model_lower or "mixtral" in model_lower:
            if "large" in model_lower:
                rates = DEFAULT_COST_RATES.get("mistral-large", {"input": 0.002, "output": 0.006}).copy()
            elif "8x22b" in model_lower:
                rates = DEFAULT_COST_RATES.get("mixtral-8x22b", {"input": 0.00065, "output": 0.00065}).copy()
            elif "8x7b" in model_lower:
                rates = DEFAULT_COST_RATES.get("mixtral-8x7b", {"input": 0.00045, "output": 0.00045}).copy()
            else:
                rates = DEFAULT_COST_RATES.get("mistral-small", {"input": 0.0001, "output": 0.0003}).copy()
            logger.info(f"🔄 Using Mistral rates for variant: {model_name}")
        
        # Other providers
        elif "deepseek" in model_lower:
            if "r1" in model_lower:
                rates = DEFAULT_COST_RATES.get("deepseek-r1", {"input": 0.00055, "output": 0.00216}).copy()
            else:
                rates = DEFAULT_COST_RATES.get("deepseek-v3", {"input": 0.00027, "output": 0.00108}).copy()
            logger.info(f"🔄 Using DeepSeek rates for variant: {model_name}")
        elif "nova" in model_lower:
            rates = DEFAULT_COST_RATES.get("amazon-nova-lite", {"input": 0.00006, "output": 0.00024}).copy()
            logger.info(f"🔄 Using Amazon Nova rates for variant: {model_name}")
        elif "command" in model_lower:
            rates = DEFAULT_COST_RATES.get("command-r", {"input": 0.00015, "output": 0.0006}).copy()
            logger.info(f"🔄 Using Cohere rates for variant: {model_name}")
        elif "grok" in model_lower:
            rates = DEFAULT_COST_RATES.get("grok-2", {"input": 0.001, "output": 0.01}).copy()
            logger.info(f"🔄 Using Grok rates for variant: {model_name}")
        elif "qwen" in model_lower:
            rates = DEFAULT_COST_RATES.get("qwen-2.5-72b", {"input": 0.0003, "output": 0.0004}).copy()
        ...
        # Provider-based fallbacks
            ...
            if provider == "OpenAI":
                rates = DEFAULT_COST_RATES.get("openai-default", {"input": 0.002, "output": 0.002}).copy()
            elif provider == "Anthropic":
                rates = DEFAULT_COST_RATES.get("anthropic-default", {"input": 0.008, "output": 0.024}).copy()
            elif provider == "Google":
                rates = DEFAULT_COST_RATES.get("google-default", {"input": 0.001, "output": 0.002}).copy()
            else:
                # Final fallback to gpt-3.5-turbo (cheapest reasonable option)
                rates = DEFAULT_COST_RATES.get("gpt-3.5-turbo", {"input": 0.0005, "output": 0.0015}).copy()
            logger.warning(f"❌ No cost rates found for model {model_name}, using {provider} fallback")
