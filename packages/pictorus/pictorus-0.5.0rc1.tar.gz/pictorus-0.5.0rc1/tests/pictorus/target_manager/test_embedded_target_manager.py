...
class TestEmbeddedTargetManager(TestCase):
    ...
    def test_listen_for_logs(self, _, __, ___, ____):
        ...
        mgr._start_rtt_listener(mock_up_chan)
    ...
    def test_listen_for_telemetry(self, _, __, ___, ____):
        target_id = "foo"
        build_hash = "abcdef"
        ocd_target = "stm32f4disco"

        data = {"foo": 1.0, "app_time_us": 1234567890}
        ...
        mgr = EmbeddedTargetManager(config)
        mgr._start_rtt_listener(mock_up_chan)
        wait_for_condition(lambda: add_telem_cb.call_count > 0)
        mgr._stop_rtt_listener()
        ...
        add_telem_cb.assert_called_once_with(expected)
