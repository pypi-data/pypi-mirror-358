...
class IMAPServer:
    ...
    def __start_tls(self, imapobj):
        if not self.usessl:
            if 'STARTTLS' not in imapobj.capabilities:
                raise OfflineImapError("Server do not support STARTTLS "
                                        " and repo starttls = yes",
                                        OfflineImapError.ERROR.REPO,
                                        exc_info()[2])
            self.ui.debug('imap', 'Using STARTTLS connection')
            try:
                imapobj.set_fingerprint(self.fingerprint)
                imapobj.starttls(keyfile=self.sslclientkey,
                        certfile=self.sslclientcert,
                        ca_certs=self.sslcacertfile,
                        cert_verify_cb=self.__verifycert,
                        ssl_version=self.sslversion,
                        tls_level=self.tlslevel)
                imapobj.check_fingerprint()
    ...
    def __verifycert(self, cert, hostname):
        ...
        for key, value in cert.get('subjectAltName', []):
            if key == 'DNS':
                certnames.append(value.lower())
            elif key == 'IP Address' and hostname_ip is not None:
                certips.append(ipaddress.ip_address(value))
        ...
        for certname in certnames:
            if (certname == dnsname or
                ...
                return None
        for certip in certips:
            if certip == hostname_ip:
                return None
