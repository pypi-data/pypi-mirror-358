...
Risk/Reward: {risk_reward_ratio:.2f}"""
    ...
    def _plot_trade_chart(
        ...
        # Calculate number of subplots needed (main + indicators + P&L)
        num_subplots = 2 + len(subplot_indicators)  # Main chart + P&L + indicator subplots

        # Create figure with appropriate height ratios
        height_ratios = [4, 2] + [1] * len(subplot_indicators)  # Main larger, P&L medium, indicators small
    ...
    def _plot_main_indicators(self, ax, chart_data: pd.DataFrame, main_indicators: list):
        ...
        for i, indicator in enumerate(main_indicators):
            ...
            label = label_map.get(indicator, indicator[4:])  # Use mapped label or default
            ax.plot(
    ...
    def _plot_trade_markers_and_lines(self, ax, chart_data: pd.DataFrame, trade: Trade, trade_fills: list):
        ...
        # Draw trade line connecting weighted average entry to exit
        if trade.weighted_avg_entry_price and trade.weighted_avg_exit_price:
            # Determine line color based on P&L
            line_color = 'green' if trade.realized_pnl > 0 else 'red'
            line_style = '-' if trade.realized_pnl > 0 else '--'

            ax.plot(
                [trade.entry_time, trade.exit_time],
                [trade.weighted_avg_entry_price, trade.weighted_avg_exit_price],
                color=line_color, linestyle=line_style, linewidth=3, alpha=0.7,
                label=f'Trade Line (P&L: ${trade.realized_pnl:.2f})'
            )
    ...
    def _plot_subplot_indicators(self, ax, chart_data: pd.DataFrame, indicators: list, subplot_num: int):
        ...
        for i, indicator in enumerate(indicators):
            ...
            ax.plot(chart_data['ts_event'], chart_data[indicator],
                   label=indicator[4:], linewidth=1, color=color)

        ax.set_ylabel(f'Subplot {subplot_num}')
