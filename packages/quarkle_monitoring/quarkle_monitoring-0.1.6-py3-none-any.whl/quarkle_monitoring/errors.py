...
class ErrorTracker:
    ...
    def _get_call_context(self) -> Dict[str, Any]:
        """Get function call context - try to find the actual API handler."""
        try:
            frame = inspect.currentframe()
            contexts = []  # Collect multiple contexts for better debugging

            # Walk up the stack to find meaningful contexts
            for i in range(20):  # Look up to 20 frames
                frame = frame.f_back
                if not frame:
                    break
                ...
                if any(
                    skip in file_name
                    for skip in ["metrics.py", "flask", "werkzeug", "threading"]
                ):
                    continue
                ...
                # If we find what looks like a route handler, prioritize it
                if (
                    any(
                    ...
                    and "app" in file_name
                ):
                    # This looks like an API handler
                    context["likely_handler"] = True
                    contexts.insert(0, context)  # Move to front
                    break

            # Return the best context we found
            if contexts:
                # Prefer handlers, then earliest meaningful frame
                primary_context = contexts[0]

                # Add stack trace information for debugging
                primary_context["stack_trace"] = [
                    f"{ctx['file']}:{ctx['function']}:{ctx['line']}"
