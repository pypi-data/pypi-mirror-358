...
from fast_clean_architecture.exceptions import ComponentError, ValidationError
...
class TestSecurityFeatures:
    ...
    def test_file_permission_validation(self, mock_access, tmp_path):
        ...
        with pytest.raises(
            (ComponentError, ValidationError), match="Failed to access directory"
        ):
            generator.create_component(
                base_path=tmp_path,
                system_name="test_system",
    ...
    def test_configuration_file_security(self, tmp_path):
        ...
        config_file = tmp_path / "fca-config.yaml"
        config = Config.create_default()
    ...
    def test_backup_file_security(self, tmp_path):
        ...
        backup_files = list(tmp_path.glob("fca-config.yaml.backup.*"))
