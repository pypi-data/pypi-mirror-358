...
class MultitaskTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        losses = {
            "clf_loss": loss_fct(clf_logits, clf_labels) * self.routine.weights["clf"]
        }
        ...
        for group in self.routine.span_label_groups:
            ...
            losses["span_" + group + "_loss"] = (
                loss_fct(group_logits, group_labels) * self.routine.weights[group]
            )
...
class MultitaskRoutine(Routine):
        ...
        if "label_map" not in self.cache:
            ...
            for group, labels in self.span_label_groups.items():
                ...
                for label in labels:
                    ...
                    group_label_maps[group]["B-" + label_name] = label_map[
                        "B-" + label_name
                    ]
                    group_label_maps[group]["I-" + label_name] = label_map[
                        "I-" + label_name
                    ]
    ...
    def tokenize_hook(self, examples, inputs):
        labels = []
        for input_ids, offset_mapping, example_spans, example_clf in zip(
            inputs["input_ids"],
            ...
            for group in self.span_label_groups:
                span_labels = self.spans_group_to_labels(
                    input_ids, offset_mapping, example_spans, group
                )
        ...
        for i in range(len(input_ids)):
            ...
            if offset[1] == 0:
                ...
                if current_label is None:
                    current_label = (
                        group_name + "-" + spans[0]["label"].upper().replace(" ", "_")
                    )
