...
class BaseData:
    ...
    def __init__(self, urls: numpy.ndarray, metadata: numpy.ndarray):
        self.urls = numpy.asarray(urls)
        self.metadata = numpy.asarray(metadata)
    ...
    def __reduce__(self):
        ...
        new_state = pickled_state[2] + ((self.urls, self.metadata),)
        # Return a tuple that replaces the parent's __setstate__ tuple with our own
        return (pickled_state[0], pickled_state[1], new_state)

    def __setstate__(self, state):
        self.urls, self.metadata = state[-1]  # Set the attributes
    ...
    def flatten_scans(self) -> "Data":
        ...
        data = self._data.reshape(fshape)
        return Data(
            self.urls.flatten(),
    ...
    def ndim(self) -> int:
        ...
        """
        return len(self.scan_shape) + len(self.frame_shape)
    ...
    def reshape(self, shape, order="C") -> "Data":
        ...
        return Data(
            ...
            data=self._data.reshape(shape, order=order),
            in_memory=self.in_memory,
        )
    ...
    def sum(self, axis=None, **kwargs) -> Union[numpy.ndarray, float]:
        ...
        if axis == 1:
            return self._data.sum(axis=axis).sum(axis=1).sum(axis=1)
    ...
    def take(self, indices, axis=None, out=None, mode="raise") -> "Data":
        ...
        data = self._data.take(indices, axis, mode=mode)
        return Data(urls, metadata, data=data, in_memory=self.in_memory)
...
class LazyData(BaseData):

    def __init__(self, urls: numpy.ndarray, metadata: numpy.ndarray):
        super().__init__(urls, metadata)
        self._data = InDiskNdArray(urls)
...
class InMemoryData(BaseData):
    ...
    def __init__(self, urls: numpy.ndarray, metadata: numpy.ndarray, data):
        super().__init__(urls, metadata)
        if self.urls.ndim > 0 and self.urls.shape[0] > 0:
            self._img_shape = utils.get_data(next(self.urls.flat)).shape
        else:
            self._img_shape = 0, 0
...
class Data:
    def __new__(cls, urls, metadata, in_memory=True, data=None):
        ...
        if in_memory:
            obj = InMemoryData(urls, metadata, data)
        else:
            # Access image one at a time using url
            obj = LazyData(urls, metadata)
        return obj
...
#     def __init__(self, urls, metadata, in_memory=True, data=None):
#         urls = numpy.asarray(urls)
...
#                   self.urls[indices[0]])[indices[1], indices[2]]
#             return Data(
#                 self.urls[indices[0]], self.metadata[indices[0]], self.in_memory
#             )
