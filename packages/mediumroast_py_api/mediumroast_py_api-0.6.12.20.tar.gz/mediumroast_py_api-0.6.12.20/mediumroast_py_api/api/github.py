...
class GitHubFunctions:
    ...
    def create_branch_from_main(self, branch_name=None):
        ...
        try:
            ...
            sha_response = self.get_sha("", "", branch_name=self.main_branch_name)
            self.logger.debug(f"SHA response: {sha_response}")
            ...
            self.logger.debug(f"SHA for main branch: {sha}")
            
            # Generate branch name if not provided
            if not branch_name:
                branch_name = str(int(time.time()))

            self.logger.debug(f"Branch name to create: {branch_name}")
            ...
            self.logger.debug(f"Branch creation data: {data}")

            # Create the branch
            r = requests.post(endpoint, headers=self.headers, data=json.dumps(data))
            self.logger.debug(f"Response status code: {r.status_code}")
            self.logger.debug(f"Response content: {r.content}")
