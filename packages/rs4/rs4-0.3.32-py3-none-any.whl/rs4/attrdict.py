...
class FixedKeyDict (dict):
    def __readonly__(self, *args, **kwargs):
        raise RuntimeError ("Cannot modify FixedKeyDict")

    def __setitem__ (self, k, *args, **kwargs):
      if k not in self:
        raise RuntimeError ("Cannot add new key to FixedKeyDict")
      return super ().__setitem__ (k, *args, **kwargs)

    def set (self, k, *args, **kwargs):
      if k not in self:
        raise RuntimeError ("Cannot add new key to FixedKeyDict")
      return super ().set (k, *args, **kwargs)

    def update (self, d, *args, **kwargs):
      for k in d:
        assert k in self
      return super ().update (d, *args, **kwargs)
