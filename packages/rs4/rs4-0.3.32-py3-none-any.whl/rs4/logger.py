...
LIB_SITE_DIR = os.path.dirname (codecs.__file__)
LIB_DIST_DIR = os.path.dirname (os.path.dirname (tqdm.__file__))
...
def traceback (multirows = False):
    ...
    if multirows:
        ...
        c = (c.replace ("] [", "\n").replace ("Traceback: [", "\n")
                .replace ("/opt/skitai", '$OPT')
                .replace (LIB_SITE_DIR, '$LIB')
                .replace (USERLIB_DIR, '$SKT')
                .replace (LIB_DIST_DIR, '$DST'))
        c = c [:-1] + "\n-----------"
        if "$OPT" in c:
            c += '\n* OPT=/opt/skitai'
        elif LIB_DIST_DIR != USERLIB_DIR:
            c += '\n* SKT=%s' % USERLIB_DIR
        c += '\n* DST=%s' % LIB_DIST_DIR
        c += '\n* LIB=%s' % LIB_SITE_DIR
        if ISATTY and len (c) > 1000:
            c = '\n'.join ([it for it in c.split ('\n') if not it.startswith ('$DST') and not it.startswith ('$LIB')])
