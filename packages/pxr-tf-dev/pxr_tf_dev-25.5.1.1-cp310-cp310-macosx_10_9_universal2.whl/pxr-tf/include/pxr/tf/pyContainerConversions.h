  ...
  struct from_python_tuple_impl<std::index_sequence<I...>, TupleType, T...>
  {
    from_python_tuple_impl()
    {
      pxr::boost::python::converter::registry::push_back(
        &convertible,
        &construct,
        pxr::boost::python::type_id<TupleType>());
    }
    ...
    {
      void* storage = (
        (pxr::boost::python::converter::rvalue_from_python_storage<TupleType>*)
          data)->storage.bytes;
      new (storage) TupleType(
        pxr::boost::python::extract<T>(PyTuple_GetItem(obj_ptr, I))()...);
      data->convertible = storage;
    }
  };
