...
/// pxr::boost::python::error_already_set exception. Callers must hold the GIL
...
TF_API bool TfPyIsNone(pxr::boost::python::object const &obj);

/// Return true iff \a obj is None.
TF_API bool TfPyIsNone(pxr::boost::python::handle<> const &obj);
...
/// conversion fails.
template <typename T>
pxr::boost::python::object TfPyObject(T const &t, bool complainOnFailure = true) {
    ...
    try {
        return pxr::boost::python::object(t);
    } catch (pxr::boost::python::error_already_set const &) {
...
inline
pxr::boost::python::object TfPyObject(PyObject* t, bool complainOnFailure = true) {
    TfPyLock pyLock;
    return pxr::boost::python::object(pxr::boost::python::handle<>(t));
}

/// Return repr(t).
///
/// Calls PyObject_Repr on the given python object.
TF_API std::string TfPyObjectRepr(pxr::boost::python::object const &t);
...
TfPyEvaluate(
    ...
    pxr::boost::python::dict const &extraGlobals = pxr::boost::python::dict());
...
TF_API std::string TfPyGetClassName(pxr::boost::python::object const &obj);
...
TF_API
void
Tf_PyWrapOnceImpl(pxr::boost::python::type_info const &,
...
TfPyWrapOnce(std::function<void()> const &wrapFunc)
{
    ...
    Tf_PyWrapOnceImpl(pxr::boost::python::type_id<T>(), wrapFunc, &isTypeWrapped);
}
...
template <class Map>
pxr::boost::python::dict TfPyCopyMapToDictionary(Map const &map) {
    TfPyLock lock;
    pxr::boost::python::dict d;
...
template<class Seq>
pxr::boost::python::list TfPyCopySequenceToList(Seq const &seq) {
    TfPyLock lock;
...
/// pxr::boost::python::error_already_set.
template <class Seq>
pxr::boost::python::object TfPyCopySequenceToSet(Seq const &seq) {
    TfPyLock lock;
    pxr::boost::python::handle<> set{pxr::boost::python::allow_null(PySet_New(nullptr))};
    if (!set) {
        pxr::boost::python::throw_error_already_set();
    }
    for (auto const& item : seq) {
        pxr::boost::python::object obj(item);
        if (PySet_Add(set.get(), obj.ptr()) == -1) {
            pxr::boost::python::throw_error_already_set();
        }
    }
    return pxr::boost::python::object(set);
}

template<class Seq>
pxr::boost::python::tuple TfPyCopySequenceToTuple(Seq const &seq) {
    return pxr::boost::python::tuple(TfPyCopySequenceToList(seq));
}
...
TF_API
pxr::boost::python::object TfPyCopyBufferToByteArray(const char* buffer, size_t size);
...
TF_API bool Tf_PyEvaluateWithErrorCheck(
    const std::string & expr, pxr::boost::python::object * obj);
...
bool TfPyEvaluateAndExtract(const std::string & expr, T * t)
{
    ...
    // Take the lock before doing anything with pxr::boost::python.
    TfPyLock lock;
    ...
    // pxr::boost::python::object, since it will increment and decrement ref counts
    // outside of the call to TfPyEvaluate.
    pxr::boost::python::object obj;
    if (!Tf_PyEvaluateWithErrorCheck(expr, &obj))
        return false;

    pxr::boost::python::extract<T> extractor(obj);
