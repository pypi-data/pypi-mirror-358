...
struct Tf_PyNoticeObjectGenerator {
    ...
    typedef pxr::boost::python::object (*MakeObjectFunc)(TfNotice const &);
    ...
    // Produce a pxr::boost::python::object for the correct derived type of \a n.
    TF_API static pxr::boost::python::object Invoke(TfNotice const &n);

private:

    template <typename T>
    static pxr::boost::python::object _Generate(TfNotice const &n) {
        // Python locking is left to the caller.
        return pxr::boost::python::object(static_cast<T const &>(n));
    }
...
struct TfPyNoticeWrapperBase : public TfType::PyPolymorphicBase {
    ...
    virtual pxr::boost::python::handle<> GetNoticePythonObject() const = 0;
};
...
public:
    ...
    using Bases = std::conditional_t<std::is_same<NoticeType, TfNotice>::value,
                                     pxr::boost::python::bases<>,
                                     pxr::boost::python::bases<BaseType>>;

    typedef pxr::boost::python::class_<NoticeType, This, Bases> ClassType;
    ...
    static ClassType Wrap(std::string const &name = std::string()) {
        ...
        return ClassType(wrappedName.c_str(), pxr::boost::python::no_init)
            .def(TfTypePythonClass());
    }

    // Implement the base class's virtual method.
    virtual pxr::boost::python::handle<> GetNoticePythonObject() const {
        TfPyLock lock;
        return pxr::boost::python::handle<>(pxr::boost::python::borrowed(_self));
    }
