...
///
/// Provides a wrapper around pxr::boost::python::object that works correctly for the
...
/// pxr::boost::python::object.  This class only actually acquires the GIL for default
/// construction, destruction and for some (in)equality comparisons.  The other
/// operations do not require taking the GIL.
///
/// This is primarily useful in cases where a pxr::boost::python::object might be
...
/// pxr::boost::python::object, that type-erased object must be destroyed while the
/// GIL is held but it's unreasonable to require that the registry know that.
/// This class helps solve that problem.
///
/// This class also provides many of the operators that pxr::boost::python::object
/// provides, by virtue of deriving from pxr::boost::python::api::object_operators<T>.
/// However it is important to note that callers must ensure the GIL is held
/// before using these operators!
#ifdef PXR_PYTHON_SUPPORT_ENABLED
class TfPyObjWrapper
    : public pxr::boost::python::api::object_operators<TfPyObjWrapper>
{
    typedef pxr::boost::python::object object;
...
private:
    ...
    friend class pxr::boost::python::api::object_operators<TfPyObjWrapper>;
