...
class Tf_PyEnumRegistry {
  ...
  public:
    ...
    TF_API
    void RegisterValue(TfEnum const &e, pxr::boost::python::object const &obj);

    template <typename T>
    void RegisterEnumConversions() {
        // Register conversions to and from python.
        pxr::boost::python::to_python_converter<T, _EnumToPython<T> >();
        _EnumFromPython<T>();
    }
  ...
  private:
    ...
    struct _EnumFromPython {
        _EnumFromPython() {
            pxr::boost::python::converter::registry::insert
                (&convertible, &construct, pxr::boost::python::type_id<T>());
        }
        ...
        static void construct(PyObject *src, pxr::boost::python::converter::
                              rvalue_from_python_stage1_data *data) {
            void *storage =
                ((pxr::boost::python::converter::
...
TF_API
std::string Tf_PyEnumRepr(pxr::boost::python::object const &self);
...
struct Tf_TypedPyEnumWrapper : Tf_PyEnumWrapper
{
    ...
    static pxr::boost::python::object GetValueFromName(const std::string& name) {
        bool found = false;
        const TfEnum value = TfEnum::GetValueFromName<T>(name, &found);
        return found
            ? pxr::boost::python::object(value)
...
TF_API
void Tf_PyEnumAddAttribute(pxr::boost::python::scope &s,
                           const std::string &name,
                           const pxr::boost::python::object &value);
...
/// may be manually wrapped using pxr::boost::python::enum_ instead.
///
...
private:
    typedef pxr::boost::python::class_<
        Tf_TypedPyEnumWrapper<T>, pxr::boost::python::bases<Tf_PyEnumWrapper> >
    _EnumPyClassType;
...
public:
  ...
  private:
    ...
    void _ExportValues(bool stripPackageName, _EnumPyClassType &enumClass) {
        pxr::boost::python::list valueList;
        ...
        for (const std::string& name : TfEnum::GetAllNames<T>()) {
            ...
            pxr::boost::python::object pyValue(wrappedValue);
        ...
        enumClass.setattr("allValues", pxr::boost::python::tuple(valueList));
    }
