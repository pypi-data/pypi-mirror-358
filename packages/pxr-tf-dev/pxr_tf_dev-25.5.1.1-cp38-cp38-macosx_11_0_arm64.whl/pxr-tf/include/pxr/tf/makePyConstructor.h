...
// be a pxr::boost::python::tuple and dict. These will contain the remaining
...
//     const pxr::boost::python::tuple& args, const pxr::boost::python::dict& kwargs);
//
// class_<MyClass, MyClassPtr>("MyClass", no_init)
//    .def(TfPyRefAndWeakPtr())
...
//    .def(...)
//
// NOTE: The current implementation does not handle pxr::boost::python::arg for
...
struct _RefPtrFactoryConverter {
    ...
    PyTypeObject const *get_pytype() const {
        return pxr::boost::python::objects::registered_class_object(
            pxr::boost::python::type_id<typename WeakPtr::DataType>()).get();
    }
};
...
/// A \c pxr::boost::python result converter generator which converts standard
...
struct Tf_PySequenceToListConverterRefPtrFactory {
    ...
    PyObject *operator()(T seq) const {
        ...
        return pxr::boost::python::incref(l.ptr());
    }
    // Required for boost.python signature generator, in play when
    // BOOST_PYTHON_NO_PY_SIGNATURES is undefined.
    PyTypeObject const *get_pytype() const {
