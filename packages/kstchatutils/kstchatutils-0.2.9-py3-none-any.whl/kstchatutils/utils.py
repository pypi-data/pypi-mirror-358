...
def repl(text, prompt, intent, domain):
    ...
    text = text.replace("<mobile>", "[[KICP_226_customerMobile]]")
    if "微信" in text and re.search(r"[a-zA-Z]{1}[-_a-zA-Z0-9]{5,19}", text):
        text = re.sub("[a-zA-Z]{1}[-_a-zA-Z0-9]{5,19}", "[[KICP_226_customerWeixin]]", text)
    text = text.replace("KICP_226_customerWeixin", "sep1").replace("KICP_226_customerMobile", "sep2")
    # text = re.sub("([a-z]{3,})", "", text)
    text = re.sub("((?!sep)[a-z]{3,})", "", text)
    text = text.replace("sep1", "KICP_226_customerWeixin").replace("sep2", "KICP_226_customerMobile")
