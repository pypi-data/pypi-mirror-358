...
class Email(Terminal):
    ...
    def __armar_asunto(self, exito: bool = True) -> str:
        ...
        if entorno == "test":
            if exito:
                return f"🧪 GLGRPA TEST | {nombre_aprendizaje}"
            else:
                return f"❌ GLGRPA TEST | ERROR | {nombre_aprendizaje}"
        elif entorno == "qa":
            if exito:
                return f"🥁 GLGRPA QA | {nombre_aprendizaje}"
            else:
                return f"❌ GLGRPA QA | {nombre_aprendizaje}"
        else:  # productivo
            if exito:
                return f"🟢 GLGRPA | {nombre_aprendizaje}"
            else:
                return f"🔴 GLGRPA | {nombre_aprendizaje}"
    ...
    def enviar_email(self, destinatarios: list, asunto: str) -> bool:
        ...
        try:
            ...
            msg.attach(MIMEText(self.__armar_cuerpo_email(), 'html'))
