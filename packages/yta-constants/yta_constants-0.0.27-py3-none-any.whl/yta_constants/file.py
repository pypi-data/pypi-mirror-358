...
class FileParsingMethod(Enum):
    ...
    Needed code:
    - `Image.open(filename)` - para fichero
    - `Image.open(io.BytesIO(image_bytes))` - para bytes
    """
    ...
    Needed code:
    - `cv2.imread('image.jpg')` - para fichero
    - TODO: Sin definir - para bytes
    """
    ...
    Needed code:
    - `VideoFileClip(filename)` - para fichero
    - `VideoFileClip(FileWriter.write_bytes('video.mp4', video_bytes))` - para bytes
    """
