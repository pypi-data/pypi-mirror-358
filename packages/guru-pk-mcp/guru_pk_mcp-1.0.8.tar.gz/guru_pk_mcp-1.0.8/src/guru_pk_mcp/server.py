...
💡 **提示**: 完全进入角色，用该思想家的语言风格、思维方式来回答。回答完成后，请使用 `record_round_response` 工具记录你的回答。"""
        ...
        # 工具3: 记录回答
        @self.server.call_tool()
        async def record_round_response(
            name: str, arguments: dict[str, Any]
        ) -> list[TextContent]:
            """记录当前轮次的回答"""
            try:
...
**状态**: {'✅ 已完成' if status['is_completed'] else '🔄 进行中'}"""
        ...
        # 工具6: 查看会话历史
        @self.server.call_tool()
        async def view_session_history(
            name: str, arguments: dict[str, Any]
        ) -> list[TextContent]:
            """查看会话历史"""
            try:
...
**推荐组合**:\n\n"""
        ...
        # 工具2: 获取帮助信息
        @self.server.call_tool()
        async def guru_pk_help(
            name: str, arguments: dict[str, Any]
        ) -> list[TextContent]:
            """获取系统帮助和介绍"""
            # 调试：确认工具名称
            if name != "guru_pk_help":
                return [TextContent(type="text", text=f"错误：期望tool名称为guru_pk_help，但收到了{name}")]
