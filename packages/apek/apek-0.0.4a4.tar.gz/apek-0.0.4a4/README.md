...
# Installation
Install via pip: `pip install apek==0.0.4a4`
...
# Upgraded Contents
- New or changed:
  - Added the `typing` module, which is used for the inheritance relationships of other classes in the code.
  - The `typing.BuiltIn` class has a class `NoneType`, which is used to represent the type of the built-in constant `None`.
- Fixes:
  - None
...
# 安装
通过pip安装：`pip install apek==0.0.4a4`
...
# 更新内容
- 新增或更改：
  - 添加了`typing`模块，用于代码中其它类的继承关系。
  - `typing.BuiltIn`类有一个类`NoneType`，用于表示内置常量`None`的类型。
- 修复：
  - 无
