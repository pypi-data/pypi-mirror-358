...
async def decrease_user_limit(headers: dict, input_text_length: str):
    ...
    async with httpx.AsyncClient() as client:
        decrease_response = await client.get(
            os.getenv("DECREACE_USER_TOKEN_API_URL"), headers=auth_headers
        )
    
    if decrease_response.status_code != 200:
        raise Exception(
            f"Failed to decrease user token. Status: {decrease_response.status_code}, Response: {decrease_response.text}"
        )
