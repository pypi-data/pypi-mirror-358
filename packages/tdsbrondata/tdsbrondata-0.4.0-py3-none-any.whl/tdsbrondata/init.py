...
def lakehouse(schemaName, sourceDataMode, sourceDataPeriod):
    ...
    databaseName = f"{lakehouseName}.{schemaName}"
    databasesAvailable = [f"{db.name}" for db in tdsbrondata._spark.catalog.listDatabases()]
    if databaseName in databasesAvailable:
        tdsbrondata._spark.sql(f"USE DATABASE {databaseName}")
