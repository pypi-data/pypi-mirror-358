...
class Product(BaseModel):
    ...
    A product object
    """ # noqa: E501
    id: Optional[StrictStr] = Field(default=None, description="ID of the product object")
    ...
    parent: Optional[StrictStr] = Field(default=None, description="Parent product ID")
    values: Optional[Dict[str, Value]] = Field(default=None, description="Product values. Keys are attribute IDs and values either a SingleValue or a MultiValue depending on whether the attribute is multivalued.")
    references: Optional[Dict[str, Reference]] = Field(default=None, description="References / links from the product. Keys are reference / link type IDs and values either a SingleReference or a MultiReference depending on whether the reference type is multivalued.")
    attribute_links: Optional[List[AttributeLink]] = Field(default=None, description="Attribute links from the product", alias="attributeLinks")
    data_containers: Optional[Dict[str, DataContainer]] = Field(default=None, description="Product data containers. Keys are data container type IDs and values either a SingleDataContainer or a MultiDataContainer depending on whether the data container type is multivalued.", alias="dataContainers")
    __properties: ClassVar[List[str]] = ["id", "name", "objectType", "parent", "values", "references", "attributeLinks", "dataContainers"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * OpenAPI `readOnly` fields are excluded.
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in attribute_links (list)
        _items = []
        if self.attribute_links:
            for _item_attribute_links in self.attribute_links:
                if _item_attribute_links:
                    _items.append(_item_attribute_links.to_dict())
            _dict['attributeLinks'] = _items
        # override the default output from pydantic by calling `to_dict()` of each value in data_containers (dict)
        _field_dict = {}
        if self.data_containers:
            for _key_data_containers in self.data_containers:
                if self.data_containers[_key_data_containers]:
                    _field_dict[_key_data_containers] = self.data_containers[_key_data_containers].to_dict()
            _dict['dataContainers'] = _field_dict
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "attributeLinks": [AttributeLink.from_dict(_item) for _item in obj["attributeLinks"]] if obj.get("attributeLinks") is not None else None,
            "dataContainers": dict(
                (_k, DataContainer.from_dict(_v))
                for _k, _v in obj["dataContainers"].items()
            )
            if obj.get("dataContainers") is not None
            else None
        })
        return _obj
