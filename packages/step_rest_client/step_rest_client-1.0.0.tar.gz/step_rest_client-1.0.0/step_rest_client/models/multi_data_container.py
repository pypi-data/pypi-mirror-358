...
class MultiDataContainer(BaseModel):
    """
    Data containers for a multivalued data container type
    """ # noqa: E501
    data_containers: Optional[List[DataContainerEntry]] = Field(default=None, description="Contained data entries / objects", alias="dataContainers")
    inherited: Optional[StrictBool] = Field(default=None, description="Indicates whether the datacontainer is inherited from parent node.")
    __properties: ClassVar[List[str]] = ["dataContainers", "inherited"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of MultiDataContainer from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * OpenAPI `readOnly` fields are excluded.
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in data_containers (list)
        _items = []
        if self.data_containers:
            for _item_data_containers in self.data_containers:
                if _item_data_containers:
                    _items.append(_item_data_containers.to_dict())
            _dict['dataContainers'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of MultiDataContainer from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "dataContainers": [DataContainerEntry.from_dict(_item) for _item in obj["dataContainers"]] if obj.get("dataContainers") is not None else None,
...
MultiDataContainer.model_rebuild(raise_errors=False)
