...
class MatchAndMergeResponse(BaseModel):
    """
    Object holding the result of a Match And Merge operation.
    """ # noqa: E501
    match_and_merge_general_execution_report: Optional[MatchAndMergeGeneralExecutionReport] = Field(default=None, alias="matchAndMergeGeneralExecutionReport")
    match_and_merge_responses: Optional[List[MatchAndMergeRecordOut]] = Field(default=None, description="List of individual Record result from a Match and Merge Request, correlated by correlation ID to a MatchAndMergeRecordIn", alias="matchAndMergeResponses")
    __properties: ClassVar[List[str]] = ["matchAndMergeGeneralExecutionReport", "matchAndMergeResponses"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of MatchAndMergeResponse from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * OpenAPI `readOnly` fields are excluded.
        ...
        # override the default output from pydantic by calling `to_dict()` of match_and_merge_general_execution_report
        if self.match_and_merge_general_execution_report:
            _dict['matchAndMergeGeneralExecutionReport'] = self.match_and_merge_general_execution_report.to_dict()
        # override the default output from pydantic by calling `to_dict()` of each item in match_and_merge_responses (list)
        _items = []
        if self.match_and_merge_responses:
            for _item_match_and_merge_responses in self.match_and_merge_responses:
                if _item_match_and_merge_responses:
                    _items.append(_item_match_and_merge_responses.to_dict())
            _dict['matchAndMergeResponses'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of MatchAndMergeResponse from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "matchAndMergeGeneralExecutionReport": MatchAndMergeGeneralExecutionReport.from_dict(obj["matchAndMergeGeneralExecutionReport"]) if obj.get("matchAndMergeGeneralExecutionReport") is not None else None,
            "matchAndMergeResponses": [MatchAndMergeRecordOut.from_dict(_item) for _item in obj["matchAndMergeResponses"]] if obj.get("matchAndMergeResponses") is not None else None
        })
        return _obj
