...
class WorkflowNode(BaseModel):
    """
    A node for which a workflow instance can exist
    """ # noqa: E501
    id: Optional[StrictStr] = Field(default=None, description="The node ID")
    type: Optional[StrictStr] = Field(default=None, description="Node supertype")
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in set(['product', 'classification', 'asset', 'entity', 'attribute', 'list_of_values']):
            raise ValueError("must be one of enum values ('product', 'classification', 'asset', 'entity', 'attribute', 'list_of_values')")
        return value
