...
class IncomingReferenceEntry(BaseModel):
    """
    An atomic reference seen from the reference target. Can represent both a \"cross reference\" and a \"classifictaion product link\".
    """ # noqa: E501
    source: Optional[StrictStr] = Field(default=None, description="ID of the reference source")
    source_type: Optional[StrictStr] = Field(default=None, description="Source \"supertype\"", alias="sourceType")
    ...
    __properties: ClassVar[List[str]] = ["source", "sourceType", "values"]
    ...
    def source_type_validate_enum(cls, value):
        ...
        if value not in set(['product', 'classification', 'entity', 'asset']):
            raise ValueError("must be one of enum values ('product', 'classification', 'entity', 'asset')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IncomingReferenceEntry from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IncomingReferenceEntry from a dict"""
        if obj is None:
            return None
