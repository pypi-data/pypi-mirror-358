...
class FindSimilarEntitiesResponse(BaseModel):
    """
    Object holding the result of a Find Similar operation
    """ # noqa: E501
    find_similar_execution_report: Optional[FindSimilarExecutionReport] = Field(default=None, alias="findSimilarExecutionReport")
    find_similar_responses: Optional[List[FindSimilarEntitiesResponseRecord]] = Field(default=None, description="Potential duplicates with rank scores", alias="findSimilarResponses")
    __properties: ClassVar[List[str]] = ["findSimilarExecutionReport", "findSimilarResponses"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of FindSimilarEntitiesResponse from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * OpenAPI `readOnly` fields are excluded.
        ...
        # override the default output from pydantic by calling `to_dict()` of find_similar_execution_report
        if self.find_similar_execution_report:
            _dict['findSimilarExecutionReport'] = self.find_similar_execution_report.to_dict()
        # override the default output from pydantic by calling `to_dict()` of each item in find_similar_responses (list)
        _items = []
        if self.find_similar_responses:
            for _item_find_similar_responses in self.find_similar_responses:
                if _item_find_similar_responses:
                    _items.append(_item_find_similar_responses.to_dict())
            _dict['findSimilarResponses'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of FindSimilarEntitiesResponse from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "findSimilarExecutionReport": FindSimilarExecutionReport.from_dict(obj["findSimilarExecutionReport"]) if obj.get("findSimilarExecutionReport") is not None else None,
            "findSimilarResponses": [FindSimilarEntitiesResponseRecord.from_dict(_item) for _item in obj["findSimilarResponses"]] if obj.get("findSimilarResponses") is not None else None
        })
        return _obj
