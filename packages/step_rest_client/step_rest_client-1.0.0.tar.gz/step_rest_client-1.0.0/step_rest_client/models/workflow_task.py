...
class WorkflowTask(BaseModel):
    ...
    workflow: Optional[StrictStr] = Field(default=None, description="ID of the workflow to which this task belongs")
    state: Optional[StrictStr] = Field(default=None, description="ID of the workflow state for this task")
    user_assignee: Optional[StrictStr] = Field(default=None, description="ID of the user to which this task is assigned. Will be null if task is assigned to a user group in which case 'groupAssignee' will be populated.", alias="userAssignee")
    group_assignee: Optional[StrictStr] = Field(default=None, description="ID of the user group to which this task is assigned. Will be null if task is assigned to a user in which case 'userAssignee' will be populated.", alias="groupAssignee")
    deadline: Optional[StrictStr] = Field(default=None, description="Deadline for this task in yyyy-mm-ddThh:mm:ss&#177hh:mm format. Null if no deadline.")
    status_flag: Optional[StatusFlag] = Field(default=None, alias="statusFlag")
    task_creation_time: Optional[StrictStr] = Field(default=None, description="Time that the task was created in yyyy-mm-ddThh:mm:ss&#177hh:mm format", alias="taskCreationTime")
    instance: Optional[StrictStr] = Field(default=None, description="Instance ID")
    __properties: ClassVar[List[str]] = ["id", "node", "workflow", "state", "userAssignee", "groupAssignee", "deadline", "statusFlag", "taskCreationTime", "instance"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of status_flag
        if self.status_flag:
            _dict['statusFlag'] = self.status_flag.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "state": obj.get("state"),
            "userAssignee": obj.get("userAssignee"),
            "groupAssignee": obj.get("groupAssignee"),
            "deadline": obj.get("deadline"),
            "statusFlag": StatusFlag.from_dict(obj["statusFlag"]) if obj.get("statusFlag") is not None else None,
            "taskCreationTime": obj.get("taskCreationTime"),
