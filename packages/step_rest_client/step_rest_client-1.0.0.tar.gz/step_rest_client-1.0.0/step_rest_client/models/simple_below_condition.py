...
class SimpleBelowCondition(BaseModel):
    """
    Simple below query condition
    """ # noqa: E501
    condition_type: StrictStr = Field(description="The condition type. Must have the value \"simplebelow\" for this type of condition.", alias="conditionType")
    top_node_id: StrictStr = Field(description="ID of the node to match results below", alias="topNodeId")
    top_node_type: StrictStr = Field(description="Type of node to match results below", alias="topNodeType")
    __properties: ClassVar[List[str]] = ["conditionType", "topNodeId", "topNodeType"]
    ...
    @field_validator('top_node_type')
    def top_node_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['classification', 'entity', 'product']):
            raise ValueError("must be one of enum values ('classification', 'entity', 'product')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of SimpleBelowCondition from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of SimpleBelowCondition from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "topNodeId": obj.get("topNodeId"),
            "topNodeType": obj.get("topNodeType")
        })
        return _obj
