...
class MatchAndMergeExecutionReport(BaseModel):
    """
    Execution report for a single Record in a Match and Merge Request
    """ # noqa: E501
    operation: Optional[StrictStr] = Field(default=None, description="The operation performed for the Merge Golden Record in the response\"")
    error_message: Optional[StrictStr] = Field(default=None, description="Error message from the operation", alias="errorMessage")
    rejected_by_business_condition: Optional[List[RejectedByBusinessCondition]] = Field(default=None, description="Records rejected by business conditions", alias="rejectedByBusinessCondition")
    rejected_by_potential_duplicates: Optional[StrictBool] = Field(default=None, description="Indicates that the record was rejected by potential duplicates", alias="rejectedByPotentialDuplicates")
    __properties: ClassVar[List[str]] = ["operation", "errorMessage", "rejectedByBusinessCondition", "rejectedByPotentialDuplicates"]
    ...
    def operation_validate_enum(cls, value):
        ...
        if value not in set(['NEW', 'UPDATE', 'FAILED']):
            raise ValueError("must be one of enum values ('NEW', 'UPDATE', 'FAILED')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of MatchAndMergeExecutionReport from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * OpenAPI `readOnly` fields are excluded.
        ...
        excluded_fields: Set[str] = set([
            "operation",
            "error_message",
            "rejected_by_business_condition",
            "rejected_by_potential_duplicates",
        ])
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in rejected_by_business_condition (list)
        _items = []
        if self.rejected_by_business_condition:
            for _item_rejected_by_business_condition in self.rejected_by_business_condition:
                if _item_rejected_by_business_condition:
                    _items.append(_item_rejected_by_business_condition.to_dict())
            _dict['rejectedByBusinessCondition'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of MatchAndMergeExecutionReport from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "rejectedByBusinessCondition": [RejectedByBusinessCondition.from_dict(_item) for _item in obj["rejectedByBusinessCondition"]] if obj.get("rejectedByBusinessCondition") is not None else None,
            "rejectedByPotentialDuplicates": obj.get("rejectedByPotentialDuplicates")
        })
        return _obj
