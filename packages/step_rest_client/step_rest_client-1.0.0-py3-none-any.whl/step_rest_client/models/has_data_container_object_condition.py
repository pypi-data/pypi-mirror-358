...
class HasDataContainerObjectCondition(BaseModel):
    """
    Condition type for matching objects that have data container objects with specific properties
    """ # noqa: E501
    condition_type: StrictStr = Field(description="The condition type. Must have the value \"hasdatacontainerobject\" for this type of condition.", alias="conditionType")
    data_container_type: StrictStr = Field(description="The data container type ID", alias="dataContainerType")
    match_inherited_data_container_objects: Optional[StrictBool] = Field(default=False, description="Should the query match hierarchically inherited data container objects", alias="matchInheritedDataContainerObjects")
    condition: DataContainerObjectCondition
    __properties: ClassVar[List[str]] = ["conditionType", "dataContainerType", "matchInheritedDataContainerObjects", "condition"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of HasDataContainerObjectCondition from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of HasDataContainerObjectCondition from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "dataContainerType": obj.get("dataContainerType"),
            "matchInheritedDataContainerObjects": obj.get("matchInheritedDataContainerObjects") if obj.get("matchInheritedDataContainerObjects") is not None else False,
            "condition": DataContainerObjectCondition.from_dict(obj["condition"]) if obj.get("condition") is not None else None
        })
        return _obj

from step_rest_client.models.data_container_object_condition import DataContainerObjectCondition
# TODO: Rewrite to not use raise_errors
HasDataContainerObjectCondition.model_rebuild(raise_errors=False)
