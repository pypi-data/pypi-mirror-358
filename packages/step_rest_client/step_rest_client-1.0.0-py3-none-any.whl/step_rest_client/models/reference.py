...
REFERENCE_ONE_OF_SCHEMAS = ["MultiReference", "SingleReference"]

class Reference(BaseModel):
    """
    An \"abstract\" representation of references for a specific reference type. Will always either be a SingleReference or a MultiReference depending on whether the reference type is multivalued.
    """
    # data type: SingleReference
    oneof_schema_1_validator: Optional[SingleReference] = None
    # data type: MultiReference
    oneof_schema_2_validator: Optional[MultiReference] = None
    actual_instance: Optional[Union[MultiReference, SingleReference]] = None
    one_of_schemas: Set[str] = { "MultiReference", "SingleReference" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        error_messages = []
        match = 0
        # validate data type: SingleReference
        if not isinstance(v, SingleReference):
            error_messages.append(f"Error! Input type `{type(v)}` is not `SingleReference`")
        else:
            match += 1
        # validate data type: MultiReference
        if not isinstance(v, MultiReference):
            error_messages.append(f"Error! Input type `{type(v)}` is not `MultiReference`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in Reference with oneOf schemas: MultiReference, SingleReference. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in Reference with oneOf schemas: MultiReference, SingleReference. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into SingleReference
        try:
            instance.actual_instance = SingleReference.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into MultiReference
        try:
            instance.actual_instance = MultiReference.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into Reference with oneOf schemas: MultiReference, SingleReference. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into Reference with oneOf schemas: MultiReference, SingleReference. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], MultiReference, SingleReference]]:
