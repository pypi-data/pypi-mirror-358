...
class EventProcessorStatistics(BaseModel):
    """
    Statistics for event processor
    """ # noqa: E501
    enabled_on: Optional[StrictStr] = Field(default=None, description="Time that the event processor was enabled in yyyy-mm-ddThh:mm:ss&#177hh:mm format. Will be null for disabled event processors.", alias="enabledOn")
    last_run: Optional[StrictStr] = Field(default=None, description="Time for last run (invocation) in yyyy-mm-ddThh:mm:ss&#177hh:mm format. Will be null if the event processor has never run.", alias="lastRun")
    next_run: Optional[StrictStr] = Field(default=None, description="Time for next run (invocation) in yyyy-mm-ddThh:mm:ss&#177hh:mm format. Will be null if no next run is scheduled.", alias="nextRun")
    __properties: ClassVar[List[str]] = ["enabledOn", "lastRun", "nextRun"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of EventProcessorStatistics from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of EventProcessorStatistics from a dict"""
        if obj is None:
            return None
