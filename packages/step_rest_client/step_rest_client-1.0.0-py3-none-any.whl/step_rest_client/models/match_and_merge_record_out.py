...
class MatchAndMergeRecordOut(BaseModel):
    """
    Individual Record result from a Match and Merge Request, correlated by correlation ID to a MatchAndMergeRecordIn
    """ # noqa: E501
    correlation_id: Optional[StrictStr] = Field(default=None, description="Correlation ID as posted in the request", alias="correlationID")
    status: Optional[StrictStr] = Field(default=None, description="Status of operation")
    potential_duplicates: Optional[List[MatchAndMergePotentialDuplicate]] = Field(default=None, description="Potential duplicates for the input record.", alias="potentialDuplicates")
    match_and_merge_execution_report: Optional[MatchAndMergeExecutionReport] = Field(default=None, alias="matchAndMergeExecutionReport")
    entity: Optional[Entity] = None
    __properties: ClassVar[List[str]] = ["correlationID", "status", "potentialDuplicates", "matchAndMergeExecutionReport", "entity"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of MatchAndMergeRecordOut from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * OpenAPI `readOnly` fields are excluded.
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in potential_duplicates (list)
        _items = []
        if self.potential_duplicates:
            for _item_potential_duplicates in self.potential_duplicates:
                if _item_potential_duplicates:
                    _items.append(_item_potential_duplicates.to_dict())
            _dict['potentialDuplicates'] = _items
        # override the default output from pydantic by calling `to_dict()` of match_and_merge_execution_report
        if self.match_and_merge_execution_report:
            _dict['matchAndMergeExecutionReport'] = self.match_and_merge_execution_report.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of MatchAndMergeRecordOut from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "potentialDuplicates": [MatchAndMergePotentialDuplicate.from_dict(_item) for _item in obj["potentialDuplicates"]] if obj.get("potentialDuplicates") is not None else None,
            "matchAndMergeExecutionReport": MatchAndMergeExecutionReport.from_dict(obj["matchAndMergeExecutionReport"]) if obj.get("matchAndMergeExecutionReport") is not None else None,
