...
class HasReferenceToCondition(BaseModel):
    """
    Has reference query condition. Matches objects that have cross references to the specified target object. This condition type cannot be used to match objects by classification product links.
    """ # noqa: E501
    condition_type: StrictStr = Field(description="The condition type. Must have the value \"hasreferenceto\" for this type of condition.", alias="conditionType")
    reference_type: StrictStr = Field(description="The reference type ID", alias="referenceType")
    target: StrictStr = Field(description="The target node ID. Super type will be deduced from the reference type.")
    match_inherited_references: Optional[StrictBool] = Field(default=False, description="Should the query match hierarchically inherited cross references", alias="matchInheritedReferences")
    metadata_condition: Optional[ReferenceMetadataCondition] = Field(default=None, alias="metadataCondition")
    __properties: ClassVar[List[str]] = ["conditionType", "referenceType", "target", "matchInheritedReferences", "metadataCondition"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of HasReferenceToCondition from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of metadata_condition
        if self.metadata_condition:
            _dict['metadataCondition'] = self.metadata_condition.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of HasReferenceToCondition from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "matchInheritedReferences": obj.get("matchInheritedReferences") if obj.get("matchInheritedReferences") is not None else False,
            "metadataCondition": ReferenceMetadataCondition.from_dict(obj["metadataCondition"]) if obj.get("metadataCondition") is not None else None
        })
        return _obj

from step_rest_client.models.reference_metadata_condition import ReferenceMetadataCondition
# TODO: Rewrite to not use raise_errors
HasReferenceToCondition.model_rebuild(raise_errors=False)
