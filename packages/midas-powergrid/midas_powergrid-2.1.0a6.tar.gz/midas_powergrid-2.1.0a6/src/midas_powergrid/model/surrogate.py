...
class Capsule:
    ...
    def load(self, filename: os.PathLike, device: str | None = None) -> Any:
        """Load model with optional device override for torch models."""

        data = cast(dict[str, Any], joblib.load(filename))
        ...
        try:
            ...
            self.model.set_state(data["state"], map_location=device)
        except ImportError:
