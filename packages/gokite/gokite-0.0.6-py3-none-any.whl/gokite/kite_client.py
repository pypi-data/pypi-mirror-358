...
from .util import openapi_to_description
...
class KiteClient:
    ...
    def _extract_path_from_url(self, service_url: str) -> str:
        """
    ...
    def _find_matching_endpoint(self, service_url: str, schema: dict) -> tuple:
        """
        ...
        """
        service_path = self._extract_path_from_url(service_url)
    ...
    def call_service(self, service_id: str, payload: dict, headers: dict = {}) -> dict:
        ...
        if schema:
            try:
                ...
                path, method = self._find_matching_endpoint(service_url, schema_json)
                if path and method:
                    http_method = method
                    # Validate payload against OpenAPI schema for the specific endpoint
                    self._validate_payload_against_openapi_for_endpoint(payload, schema_json, path, method)
                else:
    ...
    def get_service_info(self, service_id: str) -> str:
        ...
        info_parts.append(f"Service URL: {service_url}")
    ...
    def _validate_payload_against_openapi_for_endpoint(self, payload: dict, schema: dict, path: str, method: str) -> bool:
        """
