...
def find_default_env_files(workflow_path=None):
    ...
    if workflow_path:
        ...
        if examples_dir and (
            ...
            # Check for the copied environment files in the local examples directory first
            local_examples_dir = Path("./examples")
            ...
            # If not found locally, fall back to the bundled env files
            return "fastworkflow/examples/fastworkflow.env", "fastworkflow/examples/fastworkflow.passwords.env"
...
def train_with_defaults(args):
    ...
    if not os.path.exists(args.env_file_path):
        raise ValueError(f"Environment file not found at: {args.env_file_path}")
    if not os.path.exists(args.passwords_file_path):
        raise ValueError(f"Passwords file not found at: {args.passwords_file_path}")
