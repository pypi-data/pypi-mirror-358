...
3. If screenshot fails with "result exceeds maximum length", retry with compress=true
...
class ScreenshotRequest(BaseModel):
    ...
    compress: Annotated[bool, Field(
        default=False,
        description="Compress image if too large (may reduce quality)")]
...
def compress_image(image_path: str, max_size_bytes: int = 600_000) -> bytes:
    """
    Compress PNG image to fit within size limit.
    Target 600KB to account for ~33% base64 overhead = ~800KB < 1MB limit.
    """
    original_size = Path(image_path).stat().st_size
    if original_size <= max_size_bytes:
        return Path(image_path).read_bytes()

    compression_ratio = max_size_bytes / original_size
    with Image.open(image_path) as img:
        ...
        logger.info(
            f"Compressed image from {original_size} to {len(compressed_data)} bytes (scale={scale_factor:.2f})"
        )
...
def take_screenshot(request: ScreenshotRequest) -> list[ImageContent | TextContent]:
    ...
    - compress (bool): Optional. Compress image if too large (may reduce quality)
    ...
    If result exceeds maximum length error, retry with compress=true.
    """
    global _approved_windows
    ...
    try:
        ...
        if request.compress:
            image_data = compress_image(tmp_path)

        base64_data = b64encode(image_data).decode('utf-8')

        logger.info(
            f"Successfully captured screenshot for window: {window_info['title']} ({len(image_data)} bytes, compressed={request.compress})")
