...
Screenshots are returned at full quality by default. Use compression only when needed — when screenshot fails with "result exceeds maximum length"."""
)
...
def compress_image(image_path: str, target_size_bytes: int) -> tuple[bytes, str]:
    """
    Compress PNG image to fit within target size for MCP transmission.
    ...
    Returns:
        Tuple of (compressed image data as bytes, debug log string)
    """
    original_size = Path(image_path).stat().st_size
    debug_steps = []
    debug_steps.append(
        f"COMPRESS_START: {original_size} bytes ({original_size/1024:.1f} KB)")

    with Image.open(image_path) as img:
        debug_steps.append(f"COMPRESS_DIMS: {img.width}x{img.height} pixels")
        debug_steps.append(
            f"COMPRESS_TARGET: {target_size_bytes} bytes ({target_size_bytes/1024:.1f} KB)")

        if original_size <= target_size_bytes:
            # Image is already smaller than target, but user wants compression anyway
            # Apply moderate compression (reduce to ~70% of original size)
            scale_factor = sqrt(0.70)
            debug_steps.append(
                f"COMPRESS_STRATEGY: Moderate (70% size), scale={scale_factor:.3f}")
        else:
            # Image is larger than target, compress to fit within target
            compression_ratio = target_size_bytes / original_size
            scale_factor = compression_ratio ** 0.5
            debug_steps.append(
                f"COMPRESS_STRATEGY: Aggressive (fit target), ratio={compression_ratio:.3f}, scale={scale_factor:.3f}")

        new_width = max(1, int(img.width * scale_factor))
        new_height = max(1, int(img.height * scale_factor))
        debug_steps.append(
            f"COMPRESS_RESIZE: {img.width}x{img.height} → {new_width}x{new_height}")
        ...
        compression_percent = (1 - len(compressed_data)/original_size) * 100
        debug_steps.append(
            f"COMPRESS_RESULT: {original_size} → {len(compressed_data)} bytes ({len(compressed_data)/1024:.1f} KB, -{compression_percent:.1f}%)")

        debug_log = " | ".join(debug_steps)
        logger.info(debug_log)

        return compressed_data, debug_log
...
def take_screenshot(request: ScreenshotRequest) -> list[ImageContent | TextContent]:
    ...
    try:
        ...
        debug_log = []
        debug_log.append(
            f"1. Original screenshot: {original_size} bytes ({original_size/1024:.1f} KB)")

        if request.compress:
            debug_log.append("2. Compression requested")
            ...
            debug_log.append(
                f"3. Target size calculated: {target_raw_size} bytes ({target_raw_size/1024:.1f} KB)")

            pre_compress_size = len(image_data)
            image_data, compress_log = compress_image(
                tmp_path, target_raw_size)
            post_compress_size = len(image_data)

            debug_log.append(
                f"4. Compression: {pre_compress_size} → {post_compress_size} bytes ({post_compress_size/1024:.1f} KB)")
            debug_log.append(
                f"5. Compression ratio: {post_compress_size/pre_compress_size:.3f}")
            debug_log.append(f"6. Compression details: {compress_log}")
        else:
            debug_log.append("2. No compression requested - using original")

        base64_data = b64encode(image_data).decode('utf-8')
        final_b64_size = len(base64_data)

        debug_log.append(
            f"7. Base64 encoding: {len(image_data)} → {final_b64_size} bytes ({final_b64_size/1024:.1f} KB)")
        debug_log.append(
            f"8. Base64 overhead: {final_b64_size/len(image_data):.3f}x")
        debug_log.append(
            f"9. Final result: {final_b64_size} bytes, Under 1MB: {final_b64_size < 1048576}")
        ...
        # DEBUG MODE: Return only metadata, no image data
        return [
            TextContent(
                type="text",
                text=f"DEBUG MODE - No image returned, only processing info:\n\nWindow: {window_info['app']} - {window_info['title']}\n\nProcessing Details:\n{json.dumps(metadata, indent=2)}"
            )
        ]
