...
def compress_image(image_path: str, target_size_bytes: int) -> bytes:
    """
    ...
    with Image.open(image_path) as img:
        ...
        logger.info(
            f"Compressed: {original_size} → {len(compressed_data)} bytes ({len(compressed_data) / 1024:.1f} KB)")
...
def take_screenshot(request: ScreenshotRequest) -> list[ImageContent | TextContent]:
    ...
    try:
        ...
        if request.compress:
            ...
            image_data = compress_image(tmp_path, target_raw_size)

        base64_data = b64encode(image_data).decode('utf-8')
        final_b64_size = len(base64_data)

        logger.info(
            f"Screenshot: {len(image_data)} bytes, base64: {final_b64_size / 1024:.1f} KB, compressed: {request.compress}"
        )
