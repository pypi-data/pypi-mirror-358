...
from .core import load_config_key, save_config_key
...
class PubStompGUI(tk.Tk):
    ...
    def _build_scan_tab(self):
        ...
        ttk.Button(frame, text="Start Scan", command=self.start_scan).grid(row=5, column=0, padx=5, pady=10)

        self.progress = scrolledtext.ScrolledText(frame, height=25)
        self.progress.grid(row=6, column=0, columnspan=3, sticky="nsew", padx=5, pady=5)
    ...
    def start_scan(self):
        ...
        cmd = [sys.executable, "-m", "pubstomp", target, "--report"]
