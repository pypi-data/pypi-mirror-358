...
def run_sft(
    ...
    if model_args.lora:
        ...
        if model_args.loraga and last_checkpoint is None and model_args.lora_path is None:
            ...
            loraga_trainer = LoRAGATrainer(
                ...
                train_dataset=train_dataset if finetuning_args.should_load_dataset else None,
                tokenizer=tokenizer,
    ...
    if not finetuning_args.autotuner_benchmark and model_args.stage == "SFT" and finetuning_args.max_steps == -1:
        if finetuning_args.should_load_dataset and paddle.distributed.get_rank() == 0:
            if data_args.dataset_type != "map":
                ...
                train_dataset = create_dataset(
                    task_group=data_args.train_dataset_path,
                    task_group_prob=data_args.train_dataset_prob,
                    sub_dataset_type=data_args.train_dataset,
                    **dataset_config,
                )
            else:
                global_batch_size = (
    ...
    if finetuning_args.do_train:
        ...
        if not finetuning_args.autotuner_benchmark and not finetuning_args.sft_benchmark:
