...
def build(manager):
    ...
    default_train_use_lowprecision_moment = str(config.get_default_user_dict("train", "use_lowprecision_moment"))
    default_train_tensorwise_offload_optimizer = str(
        config.get_default_user_dict("train", "tensorwise_offload_optimizer")
    )
    ...
    with gr.Tab() as train_tab:
        ...
        with gr.Accordion(open=False) as eval_dataset_setting_tab:
            with gr.Row():
                ...
                with gr.Accordion(label="设置内置数据集", open=False) as eval_builtin_dataset_tab:
                        ...
                        with gr.Row():
                            with gr.Row():
                                ...
                                eval_existed_dataset_prob = gr.Textbox(
                                    interactive=False, value=default_train_eval_existed_dataset_prob
                                )
                ...
                with gr.Accordion(label="设置自定义数据集", open=False) as eval_customize_dataset_tab:
                    with gr.Column(scale=1):
        ...
        with gr.Accordion(open=False) as optimizer_parameters_tab:
            ...
            with gr.Row():
                ...
                use_lowprecision_moment = gr.Dropdown(
                    choices=config.get_choices_kwargs("boolean_choice"),
                    value=default_train_use_lowprecision_moment,
                )
                tensorwise_offload_optimizer = gr.Dropdown(
                    choices=config.get_choices_kwargs("boolean_choice"),
                    value=default_train_tensorwise_offload_optimizer,
                )
            with gr.Row():
    ...
    manager.add_elem(
        ...
        use_lowprecision_moment,
        default_train_use_lowprecision_moment,
    )
    manager.add_elem(
        "train",
        "tensorwise_offload_optimizer",
        tensorwise_offload_optimizer,
        default_train_tensorwise_offload_optimizer,
    )
