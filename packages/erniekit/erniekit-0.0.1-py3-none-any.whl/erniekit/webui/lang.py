...
LOCALES = {
    ...
    "model_name": {
        "zh": {"label": "模型名称", "info": "提供预置模型名称"},
        "en": {"label": "model name", "info": "Settings related to model name"},
    },
    "model_path": {
        "zh": {"label": "模型路径", "info": "本地模型的文件路径"},
        "en": {"label": "model path", "info": "Settings related to model path"},
    },
    "fine_tuning": {
    ...
    "compute_type": {
        "zh": {"label": "计算类型", "info": "是否使用混合精度"},
        "en": {"label": "compute type", "info": "Settings related to compute type"},
    },
    "tensor_parallel_degree": {
        "zh": {"label": "tensor parallel degree", "info": "把transformer层分成多少块"},
        "en": {
    ...
    "pipeline_parallel_degree": {
        "zh": {
            ...
            "info": "把所有的transformer layers分成多少stage",
        },
        "en": {
            "label": "pipeline parallel degree",
    ...
    "sharding_parallel_degree": {
        "zh": {"label": "sharding parallel degree", "info": "卡组的sharding参数"},
        "en": {
    ...
    "eval_dataset_path": {
        "zh": {"label": "评测数据路径", "info": "关于评测数据路径的相关设置"},
        "en": {
    ...
    "eval_dataset_prob": {
        "zh": {"label": "评测数据概率", "info": "关于评测数据概率的相关设置"},
        "en": {
    ...
    "max_shard_size": {
        "zh": {"label": "最大分块大小", "info": "单个模型文件的最大大小"},
        "en": {"label": "max shard size", "info": "Settings related to max shard size"},
    },
    "output_dir_view": {
    ...
    "train_dataset_prob": {
        "zh": {"label": "训练数据概率", "info": "关于训练数据概率的相关设置"},
        "en": {
    ...
    "num_samples_each_epoch": {
        "zh": {
            "label": "每epoch采样的数据量",
            "info": "关于每epoch采样的数据量的相关设置",
        },
        "en": {
            "label": "num samples each epoch",
            "info": "Settings related to num samples each epoch",
        },
    },
    "num_train_epochs": {
        ...
        "en": {
            ...
            "info": "Settings related to num train epochs",
        },
    },
    ...
    "max_steps": {
        "zh": {"label": "最大训练步数", "info": "关于最大训练步数的相关设置"},
    ...
    "lora_rank": {
        "zh": {"label": "lora rank", "info": "关于lora rank的相关设置"},
        "en": {"label": "lora rank", "info": "Settings related to lora rank"},
    },
    "lora_alpha": {
        "zh": {"label": "lora alpha", "info": "关于lora alpha的相关设置"},
        "en": {"label": "lora alpha", "info": "Settings related to lora alpha"},
    },
    "lora_plus_scale": {
        "zh": {"label": "lora plus scale", "info": "关于lora plus scale的相关设置"},
        "en": {"label": "lora plus scale", "info": "Lora B scale in LoRA+ technique"},
    },
    "rslora": {
    ...
    "layerwise_lr_decay_bound": {
        "zh": {
            "label": "分层学习率衰减系数",
            "info": "关于分层学习率衰减系数的相关设置",
        },
        "en": {
            "label": "layerwise lr decay bound",
            "info": "Settings related to layerwise lr decay bound",
        },
    },
    "weight_decay": {
        "zh": {
    ...
    "pp_seg_method": {
        "zh": {
            "label": "pp seg method",
            "info": "pipeline stages之间分割pipeline layers的方法",
        },
        "en": {
            "label": "pp seg method",
    ...
    "sharding": {
        "zh": {"label": "sharding", "info": "是否使用Paddle Sharding数据并行训练"},
        "en": {
            "label": "sharding",
            "info": "Whether or not to use Paddle Sharding Data Parallel training",
        },
    },
    "moe_group": {
    ...
    "disable_ckpt_quant": {
        "zh": {"label": "disable ckpt quant", "info": "是否禁用检查点量化"},
        "en": {
    ...
    "optim": {
        "zh": {"label": "optim", "info": "优化器"},
        "en": {"label": "optim", "info": "The optimizer to use"},
    },
    "use_lowprecision_moment": {
        "zh": {
            "label": "use lowprecision moment",
            "info": "是否在adamw中使用16bit moment作为模型参数",
        },
        "en": {
            "label": "use lowprecision moment",
            "info": "AdamW use 16bit moment as model parameter.",
        },
    },
    "tensorwise_offload_optimizer": {
        "zh": {
            "label": "tensorwise offload optimizer",
            "info": "将优化器状态offload到CPU",
        },
        "en": {
            "label": "tensorwise offload optimizer",
            "info": "Offload all optimizer states to CPU memory.",
        },
    },
    "scale_loss": {
    ...
    "train_existed_dataset_path": {
        "zh": {"label": "内置数据集路径", "info": "训练使用的内置数据集的存储路径"},
        "en": {"label": "Built-in Dataset Path", "info": "Storage path of the built-in dataset for training"},
    },
    "train_existed_dataset_prob": {
        "zh": {"label": "内置数据集概率", "info": "训练时使用内置数据集的概率权重"},
        "en": {"label": "Built-in Dataset Probability", "info": "Probability weight of using the built-in dataset"},
    },
    ...
    "train_customize_dataset_type": {
        "zh": {"label": "数据类型", "info": "训练数据集的数据类型选择"},
        "en": {"label": "Data Type", "info": "Data type selection for the training dataset"},
    },
    "train_customize_dataset_prob": {
        "zh": {"label": "自定义数据集概率", "info": "训练时使用自定义数据集的概率权重"},
        "en": {
    ...
    "train_customize_dataset_path": {
        "zh": {"label": "自定义数据集路径", "info": "训练使用的自定义数据集的存储路径"},
        "en": {"label": "Custom Dataset Path", "info": "Storage path of the custom dataset for training"},
    },
    "eval_customize_dataset_path": {
        "zh": {"label": "自定义数据集路径", "info": "评估使用的自定义数据集的存储路径"},
        "en": {"label": "Custom Dataset Path", "info": "Storage path of the custom dataset for evaluation"},
    },
    "eval_customize_dataset_prob": {
        "zh": {"label": "自定义数据集概率", "info": "评估时使用自定义数据集的概率权重"},
        "en": {
            "label": "Custom Dataset Probability",
    ...
    "eval_customize_dataset_type": {
        "zh": {"label": "数据类型", "info": "评估数据集的数据类型选择"},
        "en": {"label": "Data Type", "info": "Data type selection for the evaluation dataset"},
    },
    ...
    "eval_existed_dataset_path": {
        "zh": {"label": "内置数据集路径", "info": "评估使用的内置数据集的存储路径"},
        "en": {"label": "Built-in Dataset Path", "info": "Storage path of the built-in dataset for evaluation"},
    },
    "eval_existed_dataset_prob": {
        "zh": {"label": "内置数据集概率", "info": "评估时使用内置数据集的概率权重"},
        "en": {"label": "Built-in Dataset Probability", "info": "Probability weight for using the built-in dataset"},
    },
    ...
    "chat_info": {
        "zh": {
            "value": chat_info_html.format(
                "⚠️ 对话功能暂不支持加载checkpoint，如果需要加载checkpoint请自行导出模型，并修改模型名称和路径"
            ),
        },
        "en": {
            "value": chat_info_html.format(
                "⚠️ Conversation feature doesn't support checkpoint loading. "
                "Export model & set model name/path manually."
            )
        }
    },
    "model_path_selector": {
