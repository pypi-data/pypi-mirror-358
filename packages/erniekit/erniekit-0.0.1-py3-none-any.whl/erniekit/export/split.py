...
def run_split(args: Optional[dict[str, Any]] = None) -> None:
    ...
    split_model_path = os.path.join(finetuning_args.output_dir, 'export')
    model_config = Ernie4_5_MoeConfig.from_pretrained(
        ...
        pipeline_parallel_degree=finetuning_args.pipeline_parallel_degree,
        sharding_parallel_degree=finetuning_args.sharding_parallel_degree,
        sharding=finetuning_args.sharding,
        pipeline_parallel_config=finetuning_args.pipeline_parallel_config,
    ...
    model.save_pretrained(
        save_dir=os.path.join(split_model_path, 'shard'),
