...
if PYDANTIC_V2:
    ...
    from pydantic import confloat, conint, conlist, root_validator, validator
    ...
    **constraints,
    ...
    if PYDANTIC_V2:
        ...
        if metadata is not None:
            # Convert metadata to dict for json_schema_extra
            metadata_dict = {}
            if hasattr(metadata, 'valid_query_types'):
                metadata_dict['valid_query_types'] = metadata.valid_query_types
            if hasattr(metadata, 'hidden'):
                metadata_dict['hidden'] = metadata.hidden
            if hasattr(metadata, 'tunable'):
                metadata_dict['tunable'] = metadata.tunable

            if metadata_dict:
                field_kwargs['json_schema_extra'] = metadata_dict
...
class TrainingTableGenerationPlan(PlanMixin):
    ...
    train_start_offset: Union[int, InferredType, MissingType] = compat_field(
        default_factory=lambda: MissingType.VALUE,
        metadata=Metadata(valid_query_types=[QueryType.TEMPORAL]),
        ge=0,
    )

    train_end_offset: Union[int, InferredType, MissingType] = compat_field(
        default_factory=lambda: MissingType.VALUE,
        metadata=Metadata(valid_query_types=[QueryType.TEMPORAL]),
        ge=0,
    )
    ...
    timeframe_step: Union[int, InferredType, MissingType] = compat_field(
        default_factory=lambda: MissingType.VALUE,
        metadata=Metadata(valid_query_types=[QueryType.TEMPORAL]),
        ge=1,
    )
    ...
    forecast_length: Optional[Union[int, MissingType]] = compat_field(
        default_factory=lambda: MissingType.VALUE,
        metadata=Metadata(
    ...
    lag_timesteps: Optional[Union[int, MissingType]] = compat_field(
        default_factory=lambda: MissingType.VALUE,
        metadata=Metadata(
            valid_task_types=[TaskType.REGRESSION, TaskType.FORECASTING],
            valid_query_types=[QueryType.TEMPORAL],
