...
** This File: /bisos/git/bxRepos/bisos-pip/sbom/py3/bisos/sbom/seedIf.py
...
** [[elisp:(org-cycle)][| *DocStr | ] Examples of Service Access Instance Commands.
    ...
    aptPkgsNames = pkgsSeed.pkgsSeedInfo.aptPkgsNames()
    pipPkgsNames = pkgsSeed.pkgsSeedInfo.pipPkgsNames()
    pipxPkgsNames = pkgsSeed.pkgsSeedInfo.pipxPkgsNames()

    if len(aptPkgsNames) != 0:
        cs.examples.menuChapter(f'=APT Sbom Packages {aptPkgsNames}=')
    ...
    if len(pipPkgsNames) != 0:
        cs.examples.menuChapter(f'=PIP Sbom Packages {pipPkgsNames}=')
    ...
    if len(pipxPkgsNames) != 0:
        cs.examples.menuChapter(f'=PIPX Sbom Packages {pipxPkgsNames}=')
    ...
    cmnd('sbom_fullUpdate', comment=f" # Apt, Pip and  Pipx")


    # literal("hostname --fqdn", comment=" # usually used as control/me")

    if pkgsSeed.pkgsSeedInfo.examplesHook is not None:
        pkgsSeed.pkgsSeedInfo.examplesHook()
...
####+BEGIN: b:py3:cs:cmnd/classHead :cmndName "sbom_fullUpdate" :comment "" :extent "verify" :ro "cli" :parsMand "" :parsOpt "" :argsMin 0 :argsMax 0 :pyInv ""
""" #+begin_org
*  _[[elisp:(blee:menu-sel:outline:popupMenu)][±]]_ _[[elisp:(blee:menu-sel:navigation:popupMenu)][Ξ]]_ [[elisp:(outline-show-branches+toggle)][|=]] [[elisp:(bx:orgm:indirectBufOther)][|>]] *[[elisp:(blee:ppmm:org-mode-toggle)][|N]]*  CmndSvc-   [[elisp:(outline-show-subtree+toggle)][||]] <<sbom_fullUpdate>>  =verify= ro=cli   [[elisp:(org-cycle)][| ]]
#+end_org """
class sbom_fullUpdate(cs.Cmnd):
    cmndParamsMandatory = [ ]
    cmndParamsOptional = [ ]
...
** [[elisp:(org-cycle)][| *CmndDesc:* | ]]  A starting point command.
        ...
        if pkgsSeed.pkgsSeedInfo.aptPkgsList is not None:
            for eachPkg in pkgsSeed.pkgsSeedInfo.aptPkgsList:
                print(f"pkgName = {eachPkg.name}")
        else:
            print("Empty Pkgs List")

        return cmndOutcome.set(opResults=pkgsSeed.pkgsSeedInfo.aptPkgsNames())
...
** [[elisp:(org-cycle)][| *CmndDesc:* | ]]  A starting point command.
        ...
        if cmndArgs[0] == "all":
            cmndArgs = pkgsSeed.pkgsSeedInfo.aptPkgsNames()
            if len(cmndArgs) == 0:
                # cmndOutcome.report("No AptPkgs -- Installation skipped")
                print("No AptPkgs -- Installation skipped")
        ...
        for each in cmndArgs:
            if aptPkg_isInstalled().pyCmnd(cmndOutcome=cmndOutcome, argsList=[f"{each}"]).results:
                ...
                aptPkg  = pkgsSeed.pkgsSeedInfo.namedAptPkg(each)
                if aptPkg is None:
...
** [[elisp:(org-cycle)][| *CmndDesc:* | ]]  A starting point command.
        ...
        if pkgsSeed.pkgsSeedInfo.pipPkgsList is not None:
            for eachPkg in pkgsSeed.pkgsSeedInfo.pipPkgsList:
                pass
                # print(f"pkgName = {eachPkg.name}")
        else:
            print("Empty Pkgs List")

        return cmndOutcome.set(opResults=pkgsSeed.pkgsSeedInfo.pipPkgsNames())
...
** [[elisp:(org-cycle)][| *CmndDesc:* | ]]  A starting point command.
        ...
        if cmndArgs[0] == "all":
            cmndArgs = pkgsSeed.pkgsSeedInfo.pipPkgsNames()
        ...
        for each in cmndArgs:
            if pipPkg_isInstalled().pyCmnd(cmndOutcome=cmndOutcome, argsList=[f"{each}"]).results:
                ...
                pipPkg  = pkgsSeed.pkgsSeedInfo.namedPipPkg(each)
                if pipPkg is None:
                    b_io.eh.problem_usageError(f"{each}")
                    continue
                if b.subProc.WOpW(invedBy=self, log=1).bash(
...
** [[elisp:(org-cycle)][| *CmndDesc:* | ]]  A starting point command.
        ...
        if pkgsSeed.pkgsSeedInfo.pipxPkgsList is not None:
            for eachPkg in pkgsSeed.pkgsSeedInfo.pipxPkgsList:
                pass
                # print(f"pkgName = {eachPkg.name}")
        else:
            print("Empty Pkgs List")

        return cmndOutcome.set(opResults=pkgsSeed.pkgsSeedInfo.pipxPkgsNames())
...
** [[elisp:(org-cycle)][| *CmndDesc:* | ]]  A starting point command.
        ...
        if cmndArgs[0] == "all":
            cmndArgs = pkgsSeed.pkgsSeedInfo.pipxPkgsNames()
        ...
        for each in cmndArgs:
            if pipxPkg_isInstalled().pyCmnd(cmndOutcome=cmndOutcome, argsList=[f"{each}"]).results:
                ...
                pipPkg  = pkgsSeed.pkgsSeedInfo.namedPipxPkg(each)
                if pipPkg is None:
