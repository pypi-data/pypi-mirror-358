"""
This file contains functions for file management, data parsing, and data import/export.
It includes functions to select folders, find data files, parse files with metadata,
"""
...
def select_wanted_folder(initial_dir=None):
    """
    This function is used to open a GUI for selecting a folder.
...
def find_datafiles(key_words: list[str] = [""], directory_to_search="./") -> list:
    """
    This function is used to collect all the files in a specific folder with a pattern in the name
    the words in the key_words list must be all present in the files otherwise the file is not returned.
...
def find_rows_to_skip(datafile) -> int:
    """
    open epda files start with an arbitrary number of rows that are used to carry metadato
    with this function the row before all the actual data is selected and the row number returned.
    Args:
    ...
    with open(datafile, "r") as f:
        for i, line in enumerate(f):
            if "---" in line:
                return i
        print(datafile)
        raise ValueError(f"Not found the end of the documentation")
...
def parse_files_with_metadata(
    ...
    Args:
        ...
        delimiter (str): The delimiter used to separate values in the file (default: ',')

    Returns:
        list[dict]: List of dictionaries containing metadata and measurements for each file
    """
    results = []
...
def import_wldata_openepda(datafile, wl_min=None, wl_max=None) -> tuple:
    """
    function to import a datafile in openepda format
    The data are expected to be wavelength dependent.
    Args:
        datafile: datafile path
        wl_min: minimum wavelength, used to truncate the data at a min wavelength, in nm
        wl_max: maximum wavelength, used to truncate the data at a max wavelength, in nm
...
if __name__ == "__main__":
    ...
    path_to_check = r"C:\Users\DamianoMassella\Smart Photonics\Design Enablement - Documents\Device Engineering\Compact Modeling\DBR_modelling\Scripts_and_data"
