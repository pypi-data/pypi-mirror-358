...
@LastEditTime: 2025-06-30 18:22:57
...
class QueueUpHelper:
        ...
        try:
            ...
            if not redis_init.zscore(zset_name, data):
                ...
                if share_config.get_value("queue_is_log", False):
                    self.add_log(app_id=app_id, queue_name=queue_name, user_id=user_id, operate_type=1, remain_time=0, info_json=info_dict)
    ...
    def pop(self, app_id, queue_name, user_id, is_user_operate=True):
        ...
        try:
            ...
            if redis_init.zscore(zset_name, data):
                ...
                pipeline.execute()
                if share_config.get_value("queue_is_log", False):
                    operate_type = 5 if is_user_operate else 4
                ...
                if len(value_list) > 0:
                    ...
                    for i in range(len(value_list)):
                        ...
                        if is_delete == True:
                            ...
                            pipeline.execute()
                            if share_config.get_value("queue_is_log", False):
                                self.add_log(app_id=app_id, queue_name=queue_name, user_id=data, operate_type=4, remain_time=0, info_json=hash_value['info_json'])
            except Exception as ex:
    ...
    def sign(self, app_id, queue_name, user_id, pop_other_queue=True, queue_operate_time=None):
        ...
        try:
            ...
            if score:
                ...
                redis_init.hset(hash_name, hash_key, SevenHelper.json_dumps(hash_value))
                if share_config.get_value("queue_is_log", False):
                    self.add_log(app_id=app_id, queue_name=queue_name, user_id=user_id, operate_type=2, remain_time=queue_operate_time, info_json=hash_value['info_json'])
    ...
    def add_log(self, app_id, queue_name, user_id, operate_type, remain_time, info_json={}):
        ...
        try:
            ...
            queueup_log.create_date = SevenHelper.get_now_day_int()
