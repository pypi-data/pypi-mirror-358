...
VERSION = "5.0.0a13"  # gnuheath-control version
...
"""
    ...
    def update(self):
        ...
        for package in distributions():
            ...
            show_logs(win=logwin, row=1, col=1, data="Updating...", attr=0)
            msg = f"Updating {pkg} ..."
            show_logs(win=logwin, row=row, col=1, data=msg, attr=0)
    ...
    def add_package(self):
        ...
        win.addstr(6, 1, f"Install {pkg}? (yes/no)")
        confirm = win.getstr(7, 1).decode()
        if (confirm != "yes"):
            return
        ...
        for pkg in packages:
            pkg = f"{pkg}{MIN_VER}"  # Append version pinning
            show_logs(win=logwin, row=5, col=1,
                      data=f"Installing {pkg}...", attr=0)
            ...
            try:
                ...
                show_logs(win=logwin, row=2, col=1, data="ERROR", attr=0)
