...
def test_smoketest(tmp_path) -> None:
    os.chdir(tmp_path)
...
def test_smoketest_workdir(tmp_path) -> None:
...
def test_smoketest_removal(tmp_path) -> None:
    subprocess.check_call(["git", "init"], cwd=tmp_path)
    Path(tmp_path, "foo.txt").write_text("line\nunchanged\n")
...
def test_smoketest_binary(tmp_path) -> None:
    os.chdir(tmp_path)
    subprocess.check_call(["git", "init"])
