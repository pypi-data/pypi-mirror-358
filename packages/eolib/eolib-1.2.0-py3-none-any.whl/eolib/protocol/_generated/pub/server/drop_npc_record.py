...
class DropNpcRecord:
    ...
    _drops_count: int = None # type: ignore [assignment]
    _drops: list[DropRecord] = None # type: ignore [assignment]
    ...
    @drops.setter
    def drops(self, drops: list[DropRecord]) -> None:
        """
        Note:
          - Length must be 64008 or less.
        """
    ...
    def deserialize(reader: EoReader) -> "DropNpcRecord":
        ...
        """
        data: DropNpcRecord = DropNpcRecord()
        old_chunked_reading_mode: bool = reader.chunked_reading_mode
        try:
            reader_start_position: int = reader.position
            data._npc_id = reader.get_short()
            data._drops_count = reader.get_short()
            data._drops = []
            for i in range(data._drops_count):
                data._drops.append(DropRecord.deserialize(reader))
