...
# ------------------------------------------------------------------
# File Name:        benchmark_deriv_gl_cy.py
# Author:           Jiwei Huang
# Version:          0.0.1
# Created:          2012/01/01
# Description:      测试cython.deriv_gl_cy模块中函数或类的性能。
# History:
...
def main():
    ...
    print(f"{'Order':>8} | {'deriv_gl (s)':>15} | {'deriv_gl_cy (s)':>15} | {'Ratio':>15}")
    print("-" * 65)

    for order in deriv_orders:
        time = benchmark_deriv(deriv_gl, data_y, order, num_runs)
        time_0 = benchmark_deriv(deriv_gl_cy, data_y, order, num_runs)
        time_1 = benchmark_deriv(deriv_gl_cy_0, data_y, order, num_runs)

        print(f"   {order:>8.2f} | {time:>15.6f} | {time_0:>15.6f} | {time / time_0:>15.6f}")
        print(f" 0 {order:>8.2f} | {time:>15.6f} | {time_1:>15.6f} | {time / time_1:>15.6f}")
