...
def _rgb_to_color_name(rgb: np.ndarray) -> str:
    """
    Convert an RGB color to the closest XKCD color name using LAB color distance.
    
    Args:
        rgb: RGB triplet as np.ndarray or list, in 0–255 range
        
    Returns:
        Closest color name as string
    """
    rgb_normalized = np.array(rgb) / 255.0
    rgb_lab = color.rgb2lab([[rgb_normalized]])[0][0]

    min_dist = float('inf')
    closest_name = "unknown"

    for name, ref_rgb in XKCD_COLORS.items():
        ref_lab = color.rgb2lab([[ref_rgb]])[0][0]
        dist = np.linalg.norm(rgb_lab - ref_lab)
        if dist < min_dist:
            min_dist = dist
            closest_name = name
