...
class TemporalViolationBuffer:
    """Buffer to maintain last N frames' PPE violations for temporal smoothing."""
    def __init__(self, buffer_size=10):
        self.buffer = deque(maxlen=buffer_size)

    def update(self, current_detections):
        self.buffer.append(current_detections)
        # Merge all bounding boxes from the buffer
        merged = []
        for frame_detections in self.buffer:
            merged.extend(frame_detections)
        # Deduplicate by (category, bounding_box)
        unique = {(d['category'], tuple(d['bounding_box'])): d for d in merged}
...
class PPEComplianceUseCase(BaseProcessor):
    ...
    def process(self, data: Any, config: ConfigProtocol,
        ...
        try:
            ...
            # Remove metrics from result
            result.metrics = None
            # Add agg_summary to result
