...
class OSL(QDialog):
    ...
    def __load_initial_content(self):
        __notice:str = FileManager.read_file(f'{os.path.dirname(os.path.abspath(__file__))}/interface/translations/others/NOTICE_{self.__lang.lang_code}.srm')
    ...
    def __load_library_osl(self):
        if self.__osl.select_library.currentIndex() > 0:
            ...
            __license_file:str = f'{os.path.dirname(os.path.abspath(__file__))}/OSL/LICENSE_{__name_library.upper()}'
            __links:dict = json.loads(FileManager.read_file(f'{os.path.dirname(os.path.abspath(__file__))}/OSL/url_licenses.srm'))
