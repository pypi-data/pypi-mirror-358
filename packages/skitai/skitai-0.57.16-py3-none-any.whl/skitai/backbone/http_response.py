...
class http_response:
    ...
    # only for WSGI ------------------------------------------------
    def set_etag (self, identifier, max_age = -1, as_etag = False):
    ...
    def set_mtime (self, mtime, length = None, max_age = -1):
    ...
    def set_etag_mtime (self, identifier, mtime, length = None, max_age = -1, as_etag = False):
        self.set_mtime (mtime, length, max_age)
        return self.set_etag (identifier, max_age, as_etag) # return etag

    def set_cache_control (self, request, mtime = None, etag = None, max_age = -1):
        if max_age > -1:
            if request.version not in ('0.9', '1.0'):
                self.set_header ('Cache-Control', "max-age=%d" % max_age)
            else:
