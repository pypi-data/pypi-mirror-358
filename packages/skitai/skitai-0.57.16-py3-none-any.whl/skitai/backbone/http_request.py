...
from rs4.attrdict import AttrDict, FixedKeyDict
...
class http_request:
    ...
    def dejwt (self, token = None, salt = None):
        ...
        if not token:
            ...
            if not token_ or token_ [:7].lower () != "bearer ":
                self._jwt = {"err": "no bearer token", "ecd": 5}
                return self._jwt
            token = token_ [7:]
        ...
        try:
            ...
            claims = {"err": "decoding error", "ecd": 9}
        else:
            now = time.time ()
            if claims ["exp"] < now:
                claims.update ({"err": "token expired", "ecd": 3})
            elif "nbf" in claims and claims ["nbf"] > now:
                claims.update ({"err": "token not activated yet", "ecd": 4})

        if 'ecd' not in claims:
            claims ['ecd'] = 0
        self._jwt = claims
    ...
    def set_query (self, d):
        self.URL = FixedKeyDict (d)
    ...
    def json (self, ct = None):
        ...
        self.JSON = FixedKeyDict (json.loads (self.body.decode ('utf8')))
        return self.JSON
    ...
    def form (self, ct = None):
        ...
        if isinstance (self.FORM, dict):
            self.FORM = FixedKeyDict (self.FORM)
        return self.FORM
    ...
    def dict (self):
        ...
        if maybe is None:
            maybe = self.form (ct)
        return maybe.copy () if maybe else {}
