...
class sentry_traceback_logger (logger.multi_logger):
	def __init__(self, name, loggers = [], cacheline = 100, flushnow = 0):
		self.name = name
		super ().__init__ (loggers, cacheline, flushnow)

	def trace (self, name = ''):
		super ().trace (name)
		sentry.capture_exception (sys.exc_info (), contexts = {'object': {'id': name}}, tags = {'context': self.name})
...
class Logger:
	...
	def make_logger (self, prefix, freq = "daily"):
		with self.lock:
			...
			_logger = sentry_traceback_logger (prefix)
