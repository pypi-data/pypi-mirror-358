...
class Loader:
    ...
    def WAS_initialize (self):
        ...
        self.wasc.register ("debug", self.debug, False)
        self.wasc.register ("clusters", {}, False)
        self.wasc.register ("clusters_for_distcall", {}, False)
        self.wasc.register ("workers", 1, False)
    ...
    def WAS_finalize (self):
        ...
        self.wasc.register ("lifetime", lifetime, False)
    ...
    def config_executors (self, workers, zombie_timeout, process_start = None, enable_async = 0):
        ...
        self.wasc.register ("executors", _executors, False)
        self.wasc.register ("thread_executor", _executors.get_tpool (), False)
        self.wasc.register ("process_executor", _executors.get_ppool (), False)

        if enable_async:
            self._async_enabled = True
            async_executor = executors.AsyncExecutor (enable_async)
            async_executor.start ()
            self.wasc.register ("async_executor", async_executor, False)
    ...
    def config_forward_server (self, ip = "", port = 80, forward_to = 443):
        ...
        self.wasc.register ("forwardserver", forward_server, False)
    ...
    def config_webserver (self, port, ip = "", name = "", ssl = False, keep_alive = 10, network_timeout = 60, single_domain = None, thunks = [], quic = None, backlog = 100, multi_threaded = False, max_upload_size = 256000000, start = True):
        ...
        self.wasc.register ("httpserver", httpserver, False)
    ...
    def config_scheduler (self, conffile):
        if os.name == "nt":
            ...
            self.wasc.register ("scheduler", scheduler, False)
    ...
    def config_logger (self, path, media = None, log_off = [], file_loggings = None):
        ...
        self.wasc.register ("logger", triple_logger.Logger (media, path, file_loggings), False)
    ...
    def config_threads (self, numthreads = 0):
        if numthreads > 0:
            ...
            self.wasc.register ("queue",  queue, False)
            self.wasc.register ("threads", tpool, False)
