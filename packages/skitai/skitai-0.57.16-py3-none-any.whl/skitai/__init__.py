...
def allocate_gpu ():
    ...
    system_gpus = int (os.getenv ("GPU_COUNT"))
    current_gpu = int (os.getenv ('SKITAI_WORKER_ID')) % system_gpus if system_gpus > 1 else 0
    os.environ ["NVIDIA_VISIBLE_DEVICES"] = os.environ ["CUDA_VISIBLE_DEVICES"] = str (current_gpu)
...
def has_option (name):
    return name in get_options ()

def get_option (k, d = None, astype = None):
    global options
    options = argopt.options ()
    return options.get (k, d, astype)
    ...
    class SkitaiServer (Skitai.Loader):
        ...
        def configure (self):
            ...
            try:
                ...
                if is_devel ():
                    self.wasc.logger.get ("server") ("reboot worker after a few seconds...", "error")
