...
class ContentAdmContentClass:
    ...
    def verify_content_class(self, cc_name) -> bool:
        ...
        try:
            ...
            headers['Accept'] = 'application/vnd.asg-mobius-admin-reports.v3+json,application/vnd.asg-mobius-admin-reports.v2+json,application/vnd.asg-mobius-admin-reports.v1+json'

            tm = str(int(time.time() * 1000))
            content_class_get_url = self.repo_admin_url + f"/reports?limit=5&&reportid={cc_name}&timestamp={tm}"
            ...
            # Send the request
            response = requests.get(content_class_get_url, headers=headers, verify=False)
    ...
    def create_content_class(self, cc_id, cc_name): 
        ...
        try:
            ...
            if self.verify_content_class(cc_name):
                self.logger.error(f"Content class with name '{cc_name}' already exists.")
                return 409
