    ...
    for (i, file_path) in session_files.iter().enumerate() {
        ...
        match parser.parse() {
            ...
            Err(e) => {
                ...
                match e {
                    claude_code_analytics::ClaudeError::ParseError(claude_code_analytics::ParseError::EmptyFile) => {
                        empty_file_errors += 1;
                    }
                    _ => {
    ...
    for file_path in session_files.iter().take(20) {
        ...
        if let Ok(tools) = parser.extract_tool_usage() {
            if !tools.is_empty() {
                ...
                // Analyze patterns
                let patterns = claude_code_analytics::utils::analyze_tool_patterns(&tools);
                println!("\nTool usage patterns:");
