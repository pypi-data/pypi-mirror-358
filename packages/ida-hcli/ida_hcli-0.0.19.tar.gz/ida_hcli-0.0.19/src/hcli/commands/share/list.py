...
async def interactive_file_management(files: List[SharedFile]) -> None:
    ...
    file_choices = [
        Choice(title=f"{file.name or 'unnamed'} ({file.code}) - {format_size(file.size)}", value=file) for file in files
    ]
...
async def perform_download_action(selected_files: List[SharedFile]) -> None:
    """Perform download action on selected files."""
    # Get output directory
    output_dir = await safe_ask_async(questionary.text("Output directory", default="./downloads"))
...
async def perform_delete_action(selected_files: List[SharedFile]) -> None:
