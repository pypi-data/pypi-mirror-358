...
def main():
    # Static list of all 8 APIs
    submodule_types = [
        "work", "reporting", "lookup", "geojson", "party", "dataexport", "event", "sampling"
    ]
    print(f"Processing APIs: {', '.join(submodule_types)}")
    
    # Process each submodule type
    for submodule_type in submodule_types:
        print(f"\nProcessing {submodule_type}...")
        
        # Find all Python files in the submodule's swagger_client directory
        swagger_client_path = f"src/streetmanager/{submodule_type}/swagger_client"
        
        if not os.path.exists(swagger_client_path):
            print(f"Warning: {swagger_client_path} does not exist, skipping...")
            continue
        
        # Get all Python files recursively
        python_files = glob.glob(f"{swagger_client_path}/**/*.py", recursive=True)
        
        for file_path in python_files:
            fix_imports_in_file(file_path, submodule_type)
