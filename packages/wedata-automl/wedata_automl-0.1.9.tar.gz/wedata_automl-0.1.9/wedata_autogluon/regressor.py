...
class WeDataAutoGluonRegressor:
    """
    AutoGluon TabularPredictor 的 wedata 风格包装器，支持所有主流参数，并集成 MLflow 日志。
    ...
    def log_to_mlflow(self, experiment_name = "default", run_name = "defalue", test_data = None):
        ...
        with mlflow.start_run(run_name=run_name, experiment_id=experiment_id) as parent_run:
            ...
            for model_name in self.predictor.model_names():
                ...
                y_pred = self.predictor.predict(test_data, model=model_name)
                # 选择合适的指标
                if self.problem_type in ['regression', 'quantile', None]:
            ...
            if self.problem_type in ['regression', 'quantile', None]:
                mlflow.log_metric("best_rmse", mean_squared_error(y_true, y_pred, squared=False))
                mlflow.log_metric("best_mae", mean_absolute_error(y_true, y_pred))
                mlflow.log_metric("best_r2", r2_score(y_true, y_pred))
            else:
                from sklearn.metrics import accuracy_score, f1_score
                mlflow.log_metric("best_accuracy", accuracy_score(y_true, y_pred))
                mlflow.log_metric("best_f1_weighted", f1_score(y_true, y_pred, average='weighted'))
            mlflow.log_param("best_model", best_model)

            # # 保存整体 AutoGluon 模型
            # model_path = os.path.join(self.predictor.path, "autogluon_model")
            # self.predictor.save(silent=False)
...
if __name__ == "__main__":
    ...
    wedata_autogluon = WeDataAutoGluonRegressor(label="MedHouseVal")
    wedata_autogluon.fit(train_data, test_data, time_limit=60, presets='medium_quality')
    wedata_autogluon.log_to_mlflow(experiment_name="test_wedata_autogluon_1", test_data=test_data)
