...
def plot_data_path(local: bool = False, version: str = None):
    ...
    if not local:
        core_path = os.sep.join(__file__.split(os.sep)[:-1] + [filename])
        if os.path.isfile(core_path):
            return core_path.replace(" ", "%20")
        print(f"Pre-compiled version at {core_path} not found, fall back to local compiled.")

    core_path = os.sep.join(__file__.split(os.sep)[:-2] + [folder, filename])
    if os.path.isfile(core_path):
        return core_path.replace(" ", "%20")
    raise FileNotFoundError(f'Local compiled {core_path} not found. Compile it with npm according to README.md.')
