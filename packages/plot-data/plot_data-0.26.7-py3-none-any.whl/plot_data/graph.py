...
class NetworkxGraph(plot_data.PrimitiveGroup):
    ...
    def __init__(self, graph: nx.Graph, node_radius: float = 0.04, show_edge_name: bool = False, name: str = ''):
        self.graph = graph
        self.node_radius = node_radius
        self.show_edge_name = show_edge_name
    ...
    def _to_primitives(self, text_style: plot_data.TextStyle = None):
        ...
        for node1, node2, data in self.graph.edges(data=True):
            ...
            if self.show_edge_name and "name" in data:
                name = data["name"]
                mid_x = (pos1[0] + pos2[0]) / 2
                mid_y = (pos1[1] + pos2[1]) / 2
                if text_style is None:
                    text_style = plot_data.TextStyle(text_color='rgb(0,0,0)', text_align_x='center',
                                                     text_align_y='middle')

                text = plot_data.Text(name, mid_x, mid_y, text_style=text_style, text_scaling=True,
                                      max_width=4 * self.node_radius, multi_lines=False)
        ...
        for node, data in self.graph.nodes(data=True):
            ...
            if shape == '.':
                ...
                primitive = plot_data.Circle2D(x_coord, y_coord, self.node_radius, edge_style=edge_style,
                                               surface_style=surface_style)

            elif shape == 's':
                x_left, x_right, y_down, y_up = x_coord - self.node_radius, x_coord + self.node_radius, y_coord - self.node_radius, y_coord + self.node_radius
            ...
            text = plot_data.Text(name, x_coord, y_coord, text_style=text_style, text_scaling=True,
                                  max_width=2 * self.node_radius, multi_lines=False)
            primitives.append(text)
