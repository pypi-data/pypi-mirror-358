...
class FeatureEngineeringClient:
    ...
    def update_online_store(
        ...
        instance = self._workspace_client.database.update_database_instance(
            name=name,
            database_instance=database.DatabaseInstance(name=name, capacity=capacity),
            update_mask="capacity",
        )
        return DatabricksOnlineStore._from_database_instance(instance)
    ...
    def delete_online_store(self, *, name: str) -> None:
        ...
        """
        try:
            self._workspace_client.database.delete_database_instance(
                name=name, purge=True
            )
        except NotFound:
            raise NotFound(f"Online store with name '{name}' not found")
