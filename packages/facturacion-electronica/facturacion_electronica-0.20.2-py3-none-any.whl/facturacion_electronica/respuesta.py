...
class Respuesta(DOC):
    ...
    def __init__(self, vals):
        util.set_from_keys(self, vals, priorizar=['Emisor', 'RutRecibe'])
    ...
    def DTEs(self, vals):
        ...
        if type(vals) is list:
            for v in vals:
                ...
                if emis and util.formatear_rut(
                    emis['RUTEmisor']) == self._dte_emisor.RUTEmisor:
                        if not recep:
                            raise UserError("No hay información del emisor del documento a validar")
                        emis['RUTEmisor'] = recep['RUTRecep']
                        recep['RUTRecep'] = self._dte_emisor.RUTEmisor
                elif not emis:
                    v['Encabezado']['Emisor'] = {
                        'RUTEmisor': self.RutRecibe,
                    }
                if not recep:
