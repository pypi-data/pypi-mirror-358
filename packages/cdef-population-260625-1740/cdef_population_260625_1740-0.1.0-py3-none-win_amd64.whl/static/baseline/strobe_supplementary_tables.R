...
if (file.exists(file.path(output_path, "baseline_characteristics_child_data.csv"))) {
  ...
  stop("Please run baseline_characteristics.R first to generate required data files.")
}
...
# Use updated SOCIO13 mapping from mapping utilities
# Create enhanced mapping with both detailed descriptions and consistent groups
create_socio13_enhanced_mapping <- function(mappings) {
  ...
  socio13_enhanced <- data.frame(
    ...
    mutate(
      # Use consistent grouped categories from mapping utilities
      occupation_group = as.character(socio13_to_grouped_categories(socio13_code)),
      ...
      occupation_detailed = case_when(
        ...
        str_detect(occupation_detailed_da, "Lønmodtager") ~ "Employee",
        ...
        str_detect(occupation_detailed_da, "Kontanthjælp") ~ "Social benefits",
...
missing_patterns <- child_data %>%
  summarise(
    ...
    missing_child_age = sum(is.na(child_age_years)),
