...
class Denoiser(nn.Module):
    ...
    def __init__(self, inner_model, sigma_data=1., weighting='karras', scales=1, algo=None):
        super().__init__()
        self.inner_model = inner_model
        self.sigma_data = sigma_data
        self.scales = scales
        self.algo = algo if algo is not None else 'diffusion'
    ...
    def loss_flow(self, input, noise, sigma, debug=False, **kwargs):
        self.algo = 'flow'
        # For flow matching:
        # input = target data (real images)
        # noise = source (random noise)
        # sigma = time t ∈ [0,1] (reusing sigma as time parameter)
        t = sigma  # assume sigma is now sampled from [0,1]
    
        # Interpolate between noise and target: x_t = (1-t)*noise + t*target
        t_expanded = utils.append_dims(t, input.ndim)
        x_t = (1 - t_expanded) * noise + t_expanded * input  # correct but isn't working
        #x_t = (1 - t_expanded) * input + t_expanded * noise   # wrong but might work
    
        v_guess = input - noise
        v_model = self.inner_model(x_t, t, **kwargs)
        flow_loss =  ((v_model - v_guess) ** 2).flatten(1).mean(1)
        if debug:
            print("\nloss_flow:\nName        min       max       mean     std")
            for name, val in zip(['noise', 'input', 't', 'x_t', 'v_guess', 'v_model'],[noise, input, t, x_t, v_guess, v_model]):
                print(f"{name:8} {val.min().item():8.6g} {val.max().item():8.6g} {val.mean().item():8.6g} {val.std().item():8.6g}")
            print(f"flow_loss = {flow_loss.mean().item():.6f}")
            t_skew = ((t - t.mean()) ** 3).mean() / (t.std() ** 3)
            print(f"t skewness: {t_skew:.4f}")
      
        return flow_loss * 2 # fake extra 10x learning rate
