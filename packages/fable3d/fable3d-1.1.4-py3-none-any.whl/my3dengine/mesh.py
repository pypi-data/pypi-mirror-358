...
class Mesh:
    ...
    def water(
        ...
        # --- Dodajemy zestaw losowych fal ---
        mesh._wave_params = []
        for _ in range(4):  # więcej fal = więcej nieregularności
            angle = random.uniform(0, 2 * np.pi)
            scale = random.uniform(0.2, 0.6)
            phase = random.uniform(0, 2 * np.pi)
            mesh._wave_params.append((np.cos(angle), np.sin(angle), scale, phase))
        ...
        def update_water():
            ...
            X = verts[:, 0] * wave_tiling + wave_offset[0]
            Z = verts[:, 2] * wave_tiling + wave_offset[1]

            height = np.zeros_like(X)
            for dx, dz, scale, phase in mesh._wave_params:
                height += np.sin((X * dx + Z * dz) * scale + mesh._water_time + phase)

            if mesh._second_wave:
                height += np.cos((X + Z) * 0.1 + mesh._water_time * 0.7)

            verts[:, 1] = height * (mesh._wave_height / len(mesh._wave_params))
