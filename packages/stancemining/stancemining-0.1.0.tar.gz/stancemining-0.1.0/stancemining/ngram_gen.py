...
class NGramGeneration(BaseRepresentation):
    ...
    def extract_topics(
        ...
        for topic, docs in tqdm(repr_docs_mappings.items(), disable=not topic_model.verbose):
            ...
            outputs = self.model.generate(input_ids=input_ids, max_new_tokens=self.max_len_ngram, num_beams=20, num_return_sequences=self.num_ngrams, do_sample=False)
            topic_descriptions = [self.tokenizer.decode(outputs[i][input_ids.shape[0]:], skip_special_tokens=True) for i in range(self.num_ngrams)]
            
            updated_topics[topic] = topic_descriptions
