...
def create_agent(output):
    ...
    if click.confirm(
        ...
        while True:
            renderer = click.prompt(
                f"{Fore.BLUE}Renderer name{Style.RESET_ALL}", default=""
            )
            if not renderer:
                break
        ...
        if supported_renderers:
            click.echo(
                f"{Fore.YELLOW}Enter the parser code to extract artifact data from conversations:{Style.RESET_ALL}"
            )
            click.echo(
                f"{Fore.YELLOW}The parser should define a 'parse' function that takes a conversation history string and returns a dict.{Style.RESET_ALL}"
            )
            parser_code = click.edit(
                text="""import json, re
...
def push(config, api_url):
    ...
    try:
        ...
        agent_data = {
            ...
            "supportedArtifacts": agent_config.supportedArtifacts
            if hasattr(agent_config, "supportedArtifacts")
            else [],
        }
