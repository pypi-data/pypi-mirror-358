...
def run_benchmarks():
    ...
    for n_samples in data_sizes:
        ...
        np.random.seed(0)
        src_np = np.random.randn(n_samples)
        trg_np = np.random.randn(n_samples)
...
def print_summary_table(results):
    ...
    headers = ["Data Size", "Original (scipy)", "OmegaID (numpy)", "OmegaID (cupy)", "NumPy Speedup", "CuPy Speedup"]
    
    print(f"{headers[0]:<12} | {headers[1]:<18} | {headers[2]:<18} | {headers[3]:<18} | {headers[4]:<15} | {headers[5]:<15}")
    print("-" * 110)
    ...
    for size, times in results.items():
        ...
        print(f"{size:<12} | {original_time_str:<18} | {numpy_time_str:<18} | {cupy_time_str:<18} | {numpy_speedup:<15} | {cupy_speedup:<15}")
