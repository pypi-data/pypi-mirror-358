...
class AddComparisonRequest(BaseModel):
    """@public"""
    metric: Annotated[str, Field(description='id or key of the entity', examples=['76d1fab3-214c-47ef-bb04-16270639bf89'])]
    prompt: Annotated[Optional[str], Field(description='Required (or messages) when using raw text for completion, ignored if using ids')] = None
    messages: Annotated[Optional[List[ChatMessage]], Field(description='Required (or prompt) when using raw text for completion, ignored if using ids')] = None
