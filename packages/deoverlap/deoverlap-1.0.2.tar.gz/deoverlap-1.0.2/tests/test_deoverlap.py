...
from shapely.geometry import (LineString, Point, MultiLineString,
                             Polygon, MultiPolygon, GeometryCollection, MultiPoint)
...
def test_complex_geometries_are_preserved():
    """
    Tests de-overlapping with a list of complex geometries using pytest,
    ensuring their types are preserved where possible after clipping when
    preserve_types=True.
    """
    # 1. Define a list of complex, overlapping geometries in the correct
    # sequential order for clipping. (Clipper comes before clipped).
    geometries = [
        # A. A MultiPolygon.
        MultiPolygon([
        ...
        # B. A Polygon to be clipped by A.
        Polygon([(4, 1), (4, 3), (6, 3), (6, 1), (4, 1)]),

        # D. A Polygon that will clip C.
        Polygon([(16, 9), (16, 13), (18, 13), (18, 9), (16, 9)]),
        # C. A MultiLineString to be clipped by D.
        MultiLineString([
            LineString([(10, 10), (12, 12)]),
            LineString([(15, 10), (17, 12)])
        ]),

        # F. A Polygon that will clip E.
        Polygon([(21, 4), (21, 6), (23, 6), (23, 4), (21, 4)]),
        # E. A MultiPoint to be clipped by F.
        MultiPoint([(20, 5), (22, 5)]),

        # H. A Polygon that will clip G.
        Polygon([(31, 1), (31, 3), (33, 3), (33, 1), (31, 1)]),
        # G. A GeometryCollection to be clipped by H.
        GeometryCollection([
            Polygon([(30, 0), (30, 2), (32, 2), (32, 0), (30, 0)]),
            LineString([(35, 0), (35, 5)])
        ]),
    ]

    # 2. Run the de-overlapping engine
    kept_geoms, _, _, _ = deoverlap(
        geometries,
        tolerance=0.1,
        preserve_types=True
    )
    ...
    assert len(kept_geoms) == 8

    # --- Assertions for A and B ---
    kept_A = kept_geoms[0]
    kept_B = kept_geoms[1]
    assert isinstance(kept_A, MultiPolygon)
    assert kept_B.area < geometries[1].area # Check original index

    # --- Assertions for D and C ---
    kept_D = kept_geoms[2]
    kept_C = kept_geoms[3]
    assert isinstance(kept_D, Polygon)
    assert isinstance(kept_C, MultiLineString)
    assert kept_C.length < geometries[3].length # Check original index

    # --- Assertions for F and E ---
    kept_F = kept_geoms[4]
    kept_E = kept_geoms[5]
    assert isinstance(kept_F, Polygon)
    assert isinstance(kept_E, Point) # Should be clipped to a single Point
    assert kept_E.equals(Point(20, 5))

    # --- Assertions for H and G ---
    kept_H = kept_geoms[6]
    kept_G = kept_geoms[7]
    assert isinstance(kept_H, Polygon)
    assert kept_G.length < geometries[7].length # Check original index
    # The clipped GeometryCollection becomes a MultiLineString in this case
    assert isinstance(kept_G, MultiLineString)