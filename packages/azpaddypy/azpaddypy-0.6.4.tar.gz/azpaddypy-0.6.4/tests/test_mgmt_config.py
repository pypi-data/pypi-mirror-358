...
def create_mock_env_config():
    ...
    return EnvironmentConfiguration(
        ...
        keyvault_connection_string=None,
        storage_url=None,
...
class TestConfigurationLoading:
    ...
    def test_default_configuration(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            config = ConfigurationSetupDirector.build_default_setup()
            ...
            assert isinstance(config.keyvault_enable_secrets, bool)
            assert isinstance(config.storage_enable_blob, bool)
...
class TestMultipleKeyVaults:
    ...
    def test_named_keyvaults_with_env_config(self, mock_logger, mock_identity, mock_keyvault, mock_env):
        ...
        env_config = EnvironmentConfiguration(
            ...
            identity_custom_credential_options=None,
            identity_connection_string=None,
            keyvault_url="https://default-env.vault.azure.net/",
            keyvault_enable_secrets=True,
            keyvault_enable_keys=False,
            keyvault_enable_certificates=False,
            keyvault_connection_string=None,
        ...
        config = (AzureManagementBuilder(env_config)
                  .with_logger()
                  .with_identity()
                  .with_keyvault("default")  # Should use env config
...
class TestDirectorPatterns:
    ...
    def test_management_director_default(self, mock_logger, mock_identity, mock_keyvault, mock_env):
        ...
        from azpaddypy.builder.directors import AzureManagementDirector

        mock_logger.return_value = Mock(spec=AzureLogger)
        mock_identity.return_value = Mock(spec=AzureIdentity)
        mock_env.return_value = Mock()

        config = AzureManagementDirector.build_default_management()
    ...
    def test_configuration_director_default(self):
        ...
        from azpaddypy.builder.directors import AzureConfigurationDirector
    ...
    def test_module_exports(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            expected_exports = [
                ...
                "ConfigurationSetupDirector", "AzureManagementDirector", "AzureConfigurationDirector",
                "AzureResourceDirector"  # Backward compatibility alias
            ]
...
class TestConfigurationSetupBuilder:
    ...
    def test_setup_builder_custom_values(self):
        ...
        config = (ConfigurationSetupBuilder()
                  ...
                  .with_keyvault_configuration(enable_keys=True)
                  .with_storage_configuration(enable_blob=False)
                  .build())
        ...
        assert config.keyvault_enable_keys is True
        assert config.storage_enable_blob is False
    ...
    def test_setup_builder_environment_override(self):
        ...
        with patch.dict(os.environ, test_env, clear=True):
            ...
            assert config.identity_enable_token_cache is False
            assert config.keyvault_url == "https://test-vault.vault.azure.net/"
            assert config.storage_url == "https://test-storage.blob.core.windows.net/"
    ...
    def test_setup_director_patterns(self):
        ...
        # Test default setup
        default_config = ConfigurationSetupDirector.build_default_setup()
        assert default_config.logger_log_level == "INFO"
        assert default_config.logger_enable_console is True
        assert default_config.identity_enable_token_cache is True
    ...
    def test_environment_detection(self, mock_docker_check):
        ...
        with patch.dict(os.environ, test_env, clear=False):
            ...
            assert "AzureWebJobsStorage" not in config.local_settings
        ...
        with patch.dict(os.environ, test_env, clear=False):
            ...
            assert "AZURE_CLIENT_ID" in config.local_settings
            assert "AzureWebJobsStorage" in config.local_settings
    ...
    def test_resource_builder_with_custom_env_config(self):
        ...
        env_config = (ConfigurationSetupBuilder()
                      ...
                      .with_identity_configuration()
                      .with_keyvault_configuration()
                      .with_storage_configuration(account_url="https://custom.blob.core.windows.net/")
                      .build())

        # Verify the custom environment config has expected values
        assert env_config.service_name == "test-service"
        assert env_config.storage_url == "https://custom.blob.core.windows.net/"
        ...
        builder = AzureResourceBuilder(mgmt_config, env_config)
        assert builder._env_config.service_name == "test-service"
        assert builder._env_config.storage_url == "https://custom.blob.core.windows.net/"