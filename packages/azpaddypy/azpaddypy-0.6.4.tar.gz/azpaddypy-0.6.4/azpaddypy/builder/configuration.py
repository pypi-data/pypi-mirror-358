...
class EnvironmentConfiguration:
    ...
    keyvault_url: Optional[str]
    keyvault_enable_secrets: bool
    keyvault_enable_keys: bool
    keyvault_enable_certificates: bool
    keyvault_connection_string: Optional[str]
    
    # Storage configuration
    storage_url: Optional[str]
    storage_enable_blob: bool
    storage_enable_file: bool
    storage_enable_queue: bool
...
class ConfigurationSetupBuilder:
    ...
    def with_environment_detection(self) -> 'ConfigurationSetupBuilder':
        ...
        # Initialize with empty dict and populate with current environment
        if not hasattr(self, '_local_settings'):
            self._local_settings = {}
        
        # Define environment variables by category
        # Azure identity variables - needed in both environments
        azure_identity_vars = [
            'AZURE_CLIENT_ID', 'AZURE_TENANT_ID', 'AZURE_CLIENT_SECRET'
        ]
        
        # Azure Functions specific variables - only needed in local environment
        azure_functions_vars = [
            'AzureWebJobsStorage', 'AzureWebJobsDashboard', 
            'input_queue_connection__queueServiceUri',
            'AzureWebJobsStorage__accountName', 'AzureWebJobsStorage__blobServiceUri'
        ]
        
        # Always include Azure identity variables
        for env_var in azure_identity_vars:
            if env_var in os.environ:
                self._local_settings[env_var] = os.environ[env_var]
        
        # Only include Azure Functions variables when running locally (not in Docker)
        if not running_in_docker:
            for env_var in azure_functions_vars:
                if env_var in os.environ:
                    self._local_settings[env_var] = os.environ[env_var]
    ...
    def with_keyvault_configuration(self, vault_url: Optional[str] = None,
                                   enable_secrets: Optional[bool] = None,
                                   enable_keys: Optional[bool] = None,
                                   enable_certificates: Optional[bool] = None) -> 'ConfigurationSetupBuilder':
        """Parse KeyVault configuration."""
        actual_vault_url = vault_url or os.getenv("key_vault_uri")
        actual_secrets = enable_secrets if enable_secrets is not None else os.getenv("KEYVAULT_ENABLE_SECRETS", "true").lower() == "true"
        actual_keys = enable_keys if enable_keys is not None else os.getenv("KEYVAULT_ENABLE_KEYS", "false").lower() == "true"
        actual_certificates = enable_certificates if enable_certificates is not None else os.getenv("KEYVAULT_ENABLE_CERTIFICATES", "false").lower() == "true"
        actual_connection_string = self._logger_connection_string  # Share with logger by default
        
        self._keyvault_url = actual_vault_url
        self._keyvault_enable_secrets = actual_secrets
        self._keyvault_enable_keys = actual_keys
        self._keyvault_enable_certificates = actual_certificates
        self._keyvault_connection_string = actual_connection_string
        
        return self
    
    def with_storage_configuration(self, account_url: Optional[str] = None,
                                  enable_blob: Optional[bool] = None,
                                  enable_file: Optional[bool] = None,
                                  enable_queue: Optional[bool] = None) -> 'ConfigurationSetupBuilder':
        """Parse storage configuration."""
        actual_storage_url = account_url or os.getenv("STORAGE_ACCOUNT_URL")
        actual_blob = enable_blob if enable_blob is not None else os.getenv("STORAGE_ENABLE_BLOB", "true").lower() == "true"
        actual_file = enable_file if enable_file is not None else os.getenv("STORAGE_ENABLE_FILE", "true").lower() == "true"
        actual_queue = enable_queue if enable_queue is not None else os.getenv("STORAGE_ENABLE_QUEUE", "true").lower() == "true"
        actual_connection_string = self._logger_connection_string  # Share with logger by default
        
        self._storage_url = actual_storage_url
        self._storage_enable_blob = actual_blob
        self._storage_enable_file = actual_file
        self._storage_enable_queue = actual_queue
        self._storage_connection_string = actual_connection_string
    ...
    def build(self) -> EnvironmentConfiguration:
        ...
        return EnvironmentConfiguration(
            ...
            # KeyVault configuration
            keyvault_url=self._keyvault_url,
            keyvault_enable_secrets=self._keyvault_enable_secrets,
            keyvault_enable_keys=self._keyvault_enable_keys,
            keyvault_enable_certificates=self._keyvault_enable_certificates,
            keyvault_connection_string=self._keyvault_connection_string,
            
            # Storage configuration
            storage_url=self._storage_url,
            storage_enable_blob=self._storage_enable_blob,
            storage_enable_file=self._storage_enable_file,
            storage_enable_queue=self._storage_enable_queue,
...
class AzureResourceConfiguration:
    ...
    def __post_init__(self):
        """Initialize storage_accounts as empty dict if None."""
...
class AzureManagementBuilder:
    ...
    def with_logger(self, log_level: Optional[str] = None, enable_console: Optional[bool] = None) -> 'AzureManagementBuilder':
        ...
        # Include local env manager here since it's foundational management
        self._local_env_manager = create_local_env_manager(
            file_path=".env",
            settings=self._env_config.local_settings,
            override_json=True,
    ...
    def with_keyvault(self, name: str = "default", vault_url: Optional[str] = None,
        ...
        # Use provided values or fall back to environment configuration (only for default)
        if not vault_url and name == "default":
            vault_url = self._env_config.keyvault_url
        
        actual_enable_secrets = enable_secrets if enable_secrets is not None else self._env_config.keyvault_enable_secrets
        actual_enable_keys = enable_keys if enable_keys is not None else self._env_config.keyvault_enable_keys
        actual_enable_certificates = enable_certificates if enable_certificates is not None else self._env_config.keyvault_enable_certificates
        ...
        if vault_url:
            keyvault = create_azure_keyvault(
                ...
                service_version=self._env_config.service_version,
                logger=self._logger,
                connection_string=self._env_config.keyvault_connection_string,
...
class AzureResourceBuilder:
    ...
    def with_storage(self, name: str = "default", account_url: Optional[str] = None, 
        ...
        # Use provided values or fall back to environment configuration (only for default)
        storage_url = account_url or (self._env_config.storage_url if name == "default" else None)
        actual_enable_blob = enable_blob if enable_blob is not None else self._env_config.storage_enable_blob
        actual_enable_file = enable_file if enable_file is not None else self._env_config.storage_enable_file
        actual_enable_queue = enable_queue if enable_queue is not None else self._env_config.storage_enable_queue
        ...
        if storage_url:
            storage_account = create_azure_storage(
                ...
                connection_string=self._env_config.storage_connection_string,
