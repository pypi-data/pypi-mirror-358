...
def create_persistent_config(force=False):
    ...
    config_data = {
        "__instructions": "Some instructions in config.py can be accessed at: elabftw-cli --config-path",
        "verbose": 1,
        "servers": {
...
if str(base_url).startswith("<"):
    ...
    message = f"""
    This seems to be your first use please edit the config file at {config_check} to add your api_keys and the server url and remember {instructions}
    """
    exit(message)
