...
@click.option('--auto-start', '-a', is_flag=True, default=False,
              help='Enable auto-start monitoring mode (detects website launches)')
@click.option('--target-port', '-t', type=int, default=3456,
              help='Target port to monitor for auto-start (default: 3456)')
@click.option('--version', '-v', is_flag=True, default=False,
              help='Show version information')
def main(port, workdir, debug, auto_start, target_port, version):
    """
    ...
    if auto_start:
        from .auto_start import AutoStartNoLinkConnector
        connector = AutoStartNoLinkConnector(
            target_port=target_port,
            workdir=workdir,
            debug=debug
        )
        try:
            success = asyncio.run(connector.start_monitoring())
            sys.exit(0 if success else 1)
        except KeyboardInterrupt:
            print(f"\n{Fore.YELLOW}👋 Auto-start monitoring stopped!")
            sys.exit(0)
        except Exception as e:
            print(f"{Fore.RED}💥 Auto-start error: {e}")
            if debug:
                import traceback
                traceback.print_exc()
            sys.exit(1)
    
    # Create and start normal connector
