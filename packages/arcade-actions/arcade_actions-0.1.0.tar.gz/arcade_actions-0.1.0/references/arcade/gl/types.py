...
class BufferDescription:
    ...
    def __init__(
        ...
        if len(non_padded_formats) != len(self.attributes):
            raise ValueError(
                f"Different lengths of formats ({len(non_padded_formats)}) and attributes ({len(self.attributes)})"
            )
        ...
        if self.buffer.size % self.stride != 0:
            raise ValueError(f"Buffer size must align by {self.stride} bytes. {self.buffer} size={self.buffer.size}")
    ...
    def __eq__(self, other) -> bool:
        if not isinstance(other, BufferDescription):
            raise ValueError(
                f"The only logical comparison to a BufferDescriptionis a BufferDescription not {type(other)}"
            )
...
class GLTypes:
    ...
    types = {
        ...
        # Unsigned Integers
        enums.UNSIGNED_INT: TypeInfo("GL_UNSIGNED_INT", enums.UNSIGNED_INT, enums.UNSIGNED_INT, 4, 1),
        enums.UNSIGNED_INT_VEC2: TypeInfo("GL_UNSIGNED_INT_VEC2", enums.UNSIGNED_INT_VEC2, enums.UNSIGNED_INT, 4, 2),
        enums.UNSIGNED_INT_VEC3: TypeInfo("GL_UNSIGNED_INT_VEC3", enums.UNSIGNED_INT_VEC3, enums.UNSIGNED_INT, 4, 3),
        enums.UNSIGNED_INT_VEC4: TypeInfo("GL_UNSIGNED_INT_VEC4", enums.UNSIGNED_INT_VEC4, enums.UNSIGNED_INT, 4, 4),
        # Unsigned Short (mostly used for short index buffers)
        enums.UNSIGNED_SHORT: TypeInfo("GL.GL_UNSIGNED_SHORT", enums.UNSIGNED_SHORT, enums.UNSIGNED_SHORT, 2, 2),
        # Byte
        enums.BYTE: TypeInfo("GL_BYTE", enums.BYTE, enums.BYTE, 1, 1),
        enums.UNSIGNED_BYTE: TypeInfo("GL_UNSIGNED_BYTE", enums.UNSIGNED_BYTE, enums.UNSIGNED_BYTE, 1, 1),
        # Matrices
