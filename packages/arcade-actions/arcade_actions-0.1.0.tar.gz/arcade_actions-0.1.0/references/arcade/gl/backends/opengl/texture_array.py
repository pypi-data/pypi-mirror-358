...
class OpenGLTextureArray(TextureArray):
    ...
    def __init__(
        ...
        *,
        ...
        self._target = gl.GL_TEXTURE_2D_ARRAY if self._samples == 0 else gl.GL_TEXTURE_2D_MULTISAMPLE_ARRAY
    ...
    def read(self, level: int = 0, alignment: int = 1) -> bytes:
        ...
        if self._ctx.gl_api == "opengl":
            ...
            buffer = (gl.GLubyte * (self.width * self.height * self.layers * self._component_size * self._components))()
    ...
    def __repr__(self) -> str:
        return f"<TextureArray glo={self._glo.value} size={self._width}x{self._layers}x{self._height} components={self._components}>"
