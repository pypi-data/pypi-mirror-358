    ...
    * :py:meth:`object.__deepcopy__` (used by :py:func:`copy.deepcopy`)
    ...
    def __copy__(self):  # noqa
        raise NotImplementedError(
            f"{self.__class__.__name__} does not implement __copy__, butyou may implement it on a custom subclass."
        )
    ...
    def __deepcopy__(self, memo):  # noqa
        raise NotImplementedError(
            f"{self.__class__.__name__} does not implement __deepcopy__, but you may implement it on a custom subclass."
        )
...
def unpack_asfloat_or_point(value: AsFloat | Point2) -> Point2:
    ...
    if isinstance(value, float | int):
        ...
        try:
            ...
            raise ValueError("value must be a float, int, or tuple-like which unpacks as two float-like values")
        except TypeError:
            raise TypeError("value must be a float, int, or tuple-like unpacks as two float-like values")
