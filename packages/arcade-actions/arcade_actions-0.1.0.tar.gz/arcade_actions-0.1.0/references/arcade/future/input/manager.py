...
class InputManager:
    ...
    def clear_axis_input(self, axis: str):
        ...
        _clean_dicts(axis, self.keys_to_axes, self.controller_analog_to_axes, self.controller_buttons_to_axes)
            ...
            for name, axis in self.axes.items():
                for mapping in tuple(axis._mappings):
                    if mapping._input_type == InputType.KEYBOARD:
                        ...
                        self.axes_state[name] = self.window.mouse[mapping._input.name.lower()] * mapping._scale
                    elif mapping._input_type == InputType.MOUSE_BUTTON:
