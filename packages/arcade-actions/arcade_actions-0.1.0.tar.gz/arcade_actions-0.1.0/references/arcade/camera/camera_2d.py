...
class Camera2D:
    ...
    def __init__(
        ...
        *,
        ...
        if projection is not None:
            if left == right:
                raise ZeroProjectionDimension(f"projection width is 0 due to equal {left=} and {right=} values")
            if bottom == top:
                raise ZeroProjectionDimension(f"projection height is 0 due to equal {bottom=} and {top=}")
        if near == far:
            raise ZeroProjectionDimension(f"projection depth is 0 due to equal {near=} and {far=} values")
    ...
    def match_target(
        ...
        if self.render_target is None:
            raise ValueError("Tried to match a non-exsistant render target. Please use `match_window` instead")
