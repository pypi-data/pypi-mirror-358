...
class SpriteListBufferData(SpriteListData):
    ...
    def __init__(self, ctx: ArcadeContext, capacity: int, atlas: TextureAtlasBase) -> None:
        ...
        self._storage_pos_angle: Buffer = self.ctx.buffer(reserve=self._buf_capacity * 16)  # 4 x 32 bit floats
        self._storage_size: Buffer = self.ctx.buffer(reserve=self._buf_capacity * 8)  # 2 x 32 bit floats
        self._storage_color: Buffer = self.ctx.buffer(reserve=self._buf_capacity * 4)  # 4 x bytes colors
        self._storage_texture_id: Buffer = self.ctx.buffer(reserve=self._buf_capacity * 4)  # 32 bit int
        # Index buffer
        self._storage_index: Buffer = self.ctx.buffer(reserve=self._idx_capacity * 4)  # 32 bit unsigned integers
...
class SpriteListTextureData(SpriteListData):
    ...
    def __init__(self, ctx: ArcadeContext, capacity: int, atlas: TextureAtlasBase) -> None:
        ...
        self._storage_pos_angle: Texture2D = self.ctx.texture(size=(capacity, 1), components=4, dtype="f4")
        self._storage_size: Texture2D = self.ctx.texture(size=(capacity, 1), components=2, dtype="f4")
        self._storage_color: Texture2D = self.ctx.texture(size=(capacity, 1), components=4, dtype="f1")
        self._storage_texture_id: Texture2D = self.ctx.texture(size=(capacity, 1), components=1, dtype="f4")
        self._storage_index: Texture2D = self.ctx.texture(size=(capacity, 1), components=1, dtype="i4")
