...
class BasicSprite:
    ...
    def rgb(self, color: RGBOrA255):
        ...
        try:
            ...
            raise ValueError(
                f"{self.__class__.__name__},rgb takes 3 or 4 channel colors, but got {len(color)} channels"
            )
    ...
    def alpha(self, alpha: int):
        self._color = Color(self._color[0], self._color[1], self._color[2], max(0, min(255, int(alpha))))
