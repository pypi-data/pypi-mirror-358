...
class TileMap:
    ...
    def __init__(
        ...
        if not map_file and not tiled_map:
            raise AttributeError("Initialized TileMap with an empty map_file or no tiled_map argument")
        ...
        if tile.objects is not None:
            ...
            for hitbox in tile.objects.tiled_objects:
                ...
                if isinstance(hitbox, pytiled_parser.tiled_object.Rectangle):
                    ...
                    if not hitbox.size:
                        print(f"Warning: Ellipse hitbox created without a height  or width for {tile.image}. Ignoring.")
                        continue
