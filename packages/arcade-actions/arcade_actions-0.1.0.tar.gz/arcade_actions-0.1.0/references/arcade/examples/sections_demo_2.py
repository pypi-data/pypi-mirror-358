...
import arcade
from arcade.color import BEAU_BLUE, BLACK, BLUE, GRAY, RED
from arcade.key import DOWN, UP, S, W
...
class Player(arcade.Section):
    ...
    def __init__(self, left: int, bottom: int, width: int, height: int, key_up: int, key_down: int, **kwargs):
        super().__init__(
            left,
            bottom,
            width,
...
class GameView(arcade.View):
    ...
    def on_update(self, delta_time: float):
        ...
        if self.ball.bottom <= 0 or self.ball.top >= self.window.height:
