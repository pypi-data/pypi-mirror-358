...
class GameView(arcade.View):
    ...
    def setup(self):
        ...
        for enemy_marker in enemies_layer:
            coordinates = self.tile_map.get_cartesian(enemy_marker.shape[0], enemy_marker.shape[1])
            enemy_type = enemy_marker.properties["type"]
            if enemy_type == "robot":
                enemy = RobotEnemy()
            elif enemy_type == "zombie":
                enemy = ZombieEnemy()
            enemy.center_x = math.floor(coordinates[0] * TILE_SCALING * self.tile_map.tile_width)
            enemy.center_y = math.floor((coordinates[1] + 1) * (self.tile_map.tile_height * TILE_SCALING))
    ...
    def on_update(self, delta_time):
        ...
        self.scene.update_animation(delta_time, ["Coins", "Background", "Player", "Enemies"])
        ...
        for enemy in self.scene["Enemies"]:
            if (
                ...
                or enemy.left < enemy.boundary_left
                and enemy.change_x < 0
            ):
                enemy.change_x *= -1
        ...
        for bullet in self.scene["Bullets"]:
            hit_list = arcade.check_for_collision_with_lists(
                bullet, [self.scene["Enemies"], self.scene["Platforms"], self.scene["Moving Platforms"]]
            )
    ...
    def process_keychange(self):
        ...
        if self.physics_engine.is_on_ladder():
            if not self.up_pressed and not self.down_pressed or self.up_pressed and self.down_pressed:
