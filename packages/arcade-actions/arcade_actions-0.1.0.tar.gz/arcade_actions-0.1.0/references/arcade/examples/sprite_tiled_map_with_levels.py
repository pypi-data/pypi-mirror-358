...
class GameView(arcade.View):
    ...
    def load_level(self, level):
        ...
        self.tile_map = arcade.load_tilemap(f":resources:tiled_maps/level_{level}.json", scaling=TILE_SPRITE_SCALING)
    ...
    def on_update(self, delta_time):
        ...
        if not self.game_over:
            ...
            self.game_camera.position = arcade.math.smerp_2d(
                self.game_camera.position, self.player_sprite.position, delta_time, CAMERA_PAN_SPEED
            )
            self.game_camera.position = arcade.camera.grips.constrain_xy(self.game_camera.view_data, self.camera_bounds)
