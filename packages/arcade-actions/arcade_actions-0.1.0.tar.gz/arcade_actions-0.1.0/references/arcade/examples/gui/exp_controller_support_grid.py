...
def setup_grid_focus_transition(grid: dict[tuple[int, int], UIWidget]):
    ...
    cols = max(x for x, y in grid) + 1
    rows = max(y for x, y in grid) + 1
    for c in range(cols):
        for r in range(rows):
            btn = grid.get((c, r))
...
class MyView(ControllerView, UIView):
    def __init__(self):
        ...
        grid = self.root.add(UIGridLayout(column_count=3, row_count=3, vertical_spacing=10, horizontal_spacing=10))
