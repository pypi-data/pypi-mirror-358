...
class GameView(arcade.View):
    ...
    def on_draw(self):
        ...
        with self.window.default_camera.activate():
            ...
            # Draw game over
            if self.game_over:
                arcade.draw_text("Game Over", self.width / 2, self.height / 2, arcade.color.BLACK, 30)
    ...
    def on_update(self, delta_time):
        ...
        coins_hit = arcade.check_for_collision_with_list(self.player_sprite, self.scene.get_sprite_list("Coins"))
        for coin in coins_hit:
            coin.remove_from_sprite_lists()
            self.score += 1

        # Bomb hits
        bombs_hit = arcade.check_for_collision_with_list(self.player_sprite, self.scene.get_sprite_list("Bombs"))
