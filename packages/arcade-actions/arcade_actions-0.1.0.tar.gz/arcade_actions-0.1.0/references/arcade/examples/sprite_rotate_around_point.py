...
class GameView(arcade.View):
    def __init__(self):
        ...
        self.left_rotating_laser_sprite = RotatingSprite(
            ":resources:images/space_shooter/laserBlue01.png", center_x=QUARTER_WIDTH + 26, center_y=HALF_HEIGHT
        )

        self.laser_base_sprite = arcade.Sprite(
            ":resources:images/tiles/boxCrate.png", scale=0.25, center_x=QUARTER_WIDTH, center_y=HALF_HEIGHT
        )

        self.laser_text = arcade.Text("change_angle = True", QUARTER_WIDTH, WINDOW_HEIGHT // 2 - 150, anchor_x="center")

        # This example demonstrates how to make platforms rotate around a point
        self.right_rotating_platform_sprite = RotatingSprite(
            ":resources:images/tiles/grassHalf.png", scale=0.25, center_x=3 * QUARTER_WIDTH + 50, center_y=HALF_HEIGHT
        )

        self.platform_base_sprite = arcade.Sprite(
            ":resources:images/tiles/boxCrate.png", scale=0.25, center_x=3 * QUARTER_WIDTH, center_y=HALF_HEIGHT
        )
    ...
    def on_update(self, delta_time: float):
        ...
        self.left_rotating_laser_sprite.rotate_around_point(self.laser_base_sprite.position, 120 * delta_time)
