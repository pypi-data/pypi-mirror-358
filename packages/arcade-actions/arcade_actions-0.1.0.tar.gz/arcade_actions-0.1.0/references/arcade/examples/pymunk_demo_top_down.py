...
class GameView(arcade.View):
    ...
    def setup(self):
        ...
        for x in range(0, WINDOW_WIDTH + 1, SPRITE_SIZE):
            wall = arcade.Sprite(":resources:images/tiles/grassCenter.png", scale=SPRITE_SCALING_PLAYER)
            wall.center_x = x
            wall.center_y = 0
        ...
        for x in range(SPRITE_SIZE * 2, SPRITE_SIZE * 13, SPRITE_SIZE):
            ...
            item = arcade.Sprite(
                f":resources:images/space_shooter/meteorGrey_big{rock}.png", scale=SPRITE_SCALING_PLAYER
            )
            item.center_x = x
            item.center_y = 400
        ...
        self.physics_engine.add_sprite_list(
            self.wall_list, friction=0.6, collision_type="wall", body_type=PymunkPhysicsEngine.STATIC
        )

        # Create some boxes to push around.
        # Mass controls, well, the mass of an object. Defaults to 1.
        self.physics_engine.add_sprite_list(self.rock_list, mass=2, friction=0.8, damping=0.1, collision_type="rock")
        # Create some boxes to push around.
        # Mass controls, well, the mass of an object. Defaults to 1.
        self.physics_engine.add_sprite_list(self.gem_list, mass=0.5, friction=0.8, damping=0.4, collision_type="rock")
    ...
    def on_key_press(self, key, modifiers):
        ...
        if key == arcade.key.UP:
            ...
            bullet.center_x += 30
            self.bullet_list.append(bullet)
            self.physics_engine.add_sprite(bullet, mass=0.2, damping=1.0, friction=0.6, collision_type="bullet")
            force = (3000, 0)
            self.physics_engine.apply_force(bullet, force)
