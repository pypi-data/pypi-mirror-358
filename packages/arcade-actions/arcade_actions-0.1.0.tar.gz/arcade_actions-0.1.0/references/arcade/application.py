...
class Window(pyglet.window.Window):
    ...
    def __init__(
        ...
        try:
            ...
            raise NoOpenGLException(
                "Unable to create an OpenGL 3.3+ context. Check to make sure your system supports OpenGL 3.3 or higher."
            )
        if antialiasing:
            try:
        ...
        assert update_rate <= draw_rate, "An arcade window's draw rate cannot be faster than its update rate"
    ...
    def set_draw_rate(self, rate: float) -> None:
        ...
        """
        assert self._update_rate <= rate, "An arcade window's draw rate cannot be faster than its update rate"
    ...
    def show_view(self, new_view: View) -> None:
        ...
        if not isinstance(new_view, View):
            raise TypeError(f"Window.show_view() takes an arcade.View, but it got a {type(new_view)}.")
...
class View:
    ...
    def __init__(self, window: Window | None = None, background_color: RGBOrA255 | None = None) -> None:
        ...
        self._background_color: Color | None = background_color and Color.from_iterable(background_color)
