...
class LLMClient:
    def __init__(self):
        ...
        self.default_timeout_invoke = system_config["llm_config"]["timeout"]["invoke"]
        self.default_timeout_stream = system_config["llm_config"]["timeout"]["stream"]
    ...
    def llm_invoke(self, request: LLMRequest):
        ...
        timeout = request.get("timeout", self.default_timeout_invoke)
        rid = request.get("rid")
        ...
        if len(message_str) > 30:
            message_str = message_str[:15] + "[...]" + message_str[-15:]
        for url in all_urls:
            model = ""
            filtered_params = {}
            try:
    ...
    async def _call_llm_service(
        ...
        errors = []
        if not timeout:
            timeout = self.default_timeout_stream
        if not rid:
            rid = correlation_id.get()
            if not rid:
                rid = "agent_" + str(uuid.uuid4())
