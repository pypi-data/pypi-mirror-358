...
from frogml_core.clients.secret_service import SecretServiceClient
from frogml_core.exceptions import FrogmlException, FrogmlSuggestionException
from frogml_core.inner.build_logic.constants.host_resource import HOST_TEMP_BUILD_DIR
from frogml_core.inner.build_logic.interface.step_inteface import Step
from frogml_core.inner.build_logic.tools.dependencies_tools import find_dependency_files
from frogml_core.inner.build_logic.tools.text import snake_case
from frogml_core.inner.instance_template.verify_template_id import verify_template_id
from frogml_core.inner.provider import Provider
...
class PreFetchValidationStep(Step):
    ...
    def validate_frogml_model(self):
        if self.config.pre_built_model:
            ...
            if not isinstance(self.config.pre_built_model, BaseModel):
                raise FrogmlException(
                    self.INVALID_FROGML_MODEL_MSG_FORMAT.format(
                        expected_class=BaseModel.__name__,
