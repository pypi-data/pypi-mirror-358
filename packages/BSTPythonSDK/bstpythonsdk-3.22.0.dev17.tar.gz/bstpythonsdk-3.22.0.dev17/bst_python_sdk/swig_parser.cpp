...
// Packet types to parse when quick mode and gcs addr are enabled
std::unordered_set<int> quick_gcs_packets = {
    97,  // TELEMETRY_POSITION
    101, // TELEMETRY_SYSTEM
};
...
std::vector<Packet> parse(const char* file_path, bool has_addr, bool quick_mode) {
    ...
    if (has_addr) {
        quick_mode_packets.insert(
                quick_gcs_packets.begin(),
                quick_gcs_packets.end());
    }
