...
class GEO:
    ...
    def train(self, X: torch.Tensor, Y: torch.Tensor, mr: float=0.1, dr: float=0.999, generations: int=1000, population: int=50, batch_size=None, progress_style: str='tqdm', early_stop=None, optim_mr: bool=False, threshold: int=50) -> None:
        if not isinstance(X, torch.Tensor) or not isinstance(Y, torch.Tensor):
