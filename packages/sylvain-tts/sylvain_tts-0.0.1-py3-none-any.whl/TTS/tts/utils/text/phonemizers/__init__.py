...
try:
    from TTS.tts.utils.text.phonemizers.zh_hk_phonemizer import ZH_HK_Phonemizer
except ImportError:
    ZH_HK_Phonemizer = None
...
if ZH_HK_Phonemizer is not None:
    PHONEMIZERS[ZH_HK_Phonemizer.name()] = ZH_HK_Phonemizer
    DEF_LANG_TO_PHONEMIZER["zh-hk"] = ZH_HK_Phonemizer.name()
...
def get_phonemizer_by_name(name: str, **kwargs) -> BasePhonemizer:
    ...
    if name == "zh_hk_phonemizer":
        if ZH_HK_Phonemizer is None:
            raise ValueError("You need to install ZH HK phonemizer dependencies. Try `pip install coqui-tts[zh]`.")
        return ZH_HK_Phonemizer(**kwargs)
