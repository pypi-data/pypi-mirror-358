...
def test_claude_file_creation_and_append(mock_get_rules, isolated_fs_with_config):
    ...
    result1 = runner.invoke(app, ["claude", "--project-path", str(project_path)], catch_exceptions=False)
    ...
    result2 = runner.invoke(app, ["claude", "--project-path", str(project_path)], catch_exceptions=False)
...
def test_cursor_file_creation(mock_get_rules, isolated_fs_with_config):
    ...
    project_path = isolated_fs_with_config
    result = runner.invoke(app, ["cursor", "--project-path", str(project_path)], catch_exceptions=False)
...
def test_cline_file_creation(mock_get_rules, isolated_fs_with_config):
    ...
    project_path = isolated_fs_with_config
    result = runner.invoke(app, ["cline", "--project-path", str(project_path)], catch_exceptions=False)
    assert result.exit_code == 0, result.output
...
def test_roo_file_creation(mock_get_rules, isolated_fs_with_config):
    ...
    project_path = isolated_fs_with_config
    result = runner.invoke(app, ["roo", "--project-path", str(project_path)], catch_exceptions=False)
    assert result.exit_code == 0, result.output
...
def test_copilot_file_creation(mock_get_rules, isolated_fs_with_config):
    ...
    project_path = isolated_fs_with_config
    result = runner.invoke(app, ["copilot", "--project-path", str(project_path)], catch_exceptions=False)
    assert result.exit_code == 0, result.output
