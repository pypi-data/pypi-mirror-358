...
from lego.llm.prompt import ChatTemplate, ClaudeChatTemplate
...
from rec_bot.db.context import ContextManager, ExpireTime
...
class RedisQueue(RedisBase):
    ...
    async def compile_enqueue_script(self) -> None:
        ...
        """
            redis.call('JSON.SET', '151-dg1-vladislav.lukoshkin@qoreai.com', '$', '{{}}')
            redis.call('JSON.SET', '151-dg1-vladislav.lukoshkin@qoreai.com', '$._queue', '[]')
        """
    ...
    async def enqueue(self, items: list) -> None:
        ...
        print("Enqueuing: ", [json.dumps(item) for item in items])
        print(await self._enqueue_script(
    ...
    async def _sanity_checks(self) -> None:
        ...
        if type_ := await self.redon.type(self.ctx_id, f"$.{self.queue_name}"):
            if not type_:
...
class ConversationHistory:
    base_url: str = "https://data.stage.qoreai.com/api"

    class ConversationHistoryFetchError(Exception):
        """Exception raised when fetching conversation history fails."""
    ...
    async def pull(self) -> None:
        """Fetch the conversation history by its `ctx_id`."""
        if not await self.queue.is_full():
            response = await self.client.get(
                "messageHistory", params={"customerId": self._ctx_id}
            )
            if response.status_code == 200:
                ...
                raise self.ConversationHistoryFetchError(
                    "Failed to fetch conversation history.\n"
                    f"Status code: {response.status_code},\n"
                    f"Text response: {response.text}"
                )

    async def push(self, messages: OneOrMany[Message]) -> None:
        """Push messages to the conversation history."""
        if not isinstance(messages, list):
            messages = [messages]
        await self.queue.enqueue(messages)

    async def chat_template(
        self, chat_template: ChatTemplate | None = None
    ) -> str:
        """Generate a chat template based on the conversation history."""
        chat_template = chat_template or self._chat_template
        if not chat_template:
            raise ValueError("Chat template is not set.")

        chat_template = cast(ChatTemplate, chat_template)
        return chat_template.apply_chat_template(await self.queue.get_list())


async def main():
    from rec_bot.db.context import ContextManager, ExpireTime

    ctx_id = "151-dg1-vladislav.lukoshkin@qoreai.com"
    async with AsyncClient() as client:
        print(
            await client.get(
                "https://data.stage.qoreai.com/api/messageHistory",
                params={"customerId": ctx_id},
            )
        )
    ctx_manager = ContextManager(
        expires_in=ExpireTime.from_str("1h"),
        refresh_on_call=True,
    )
    await ctx_manager.context(ctx_id)
    conversation_history = ConversationHistory(
        ctx_id, chat_template=ClaudeChatTemplate, max_size=10
    )
    await conversation_history.pull()
    await conversation_history.push([{"role": "user", "content": "Hello!"}, {"role": "assistant", "content": "Hi there!"}])
    print(await conversation_history.queue.get_list())
    print(await conversation_history.chat_template())
