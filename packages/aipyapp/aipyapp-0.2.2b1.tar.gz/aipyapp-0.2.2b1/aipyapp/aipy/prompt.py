...
1. 代码块必须用一对注释标记包围，格式如下：
   - 代码开始：<!-- Block-Start: {"id": "代码块ID", "version": 数字版本号如1/2/3, "deps": ["依赖的代码块ID"], "path": "该代码块的可选文件路径"} -->
   - 代码本体：用 Markdown 代码块包裹（如 ```python 或 ```html 等)。
   - 代码结束：<!-- Block-End: { "id": "与开始一致的唯一字符串" } -->

2. (代码块ID, version) 必须在整个会话过程中唯一。
...
5. **正确示例：**
<!-- Block-Start: {"id": "abc123", "version": 1, "path": "main.py"} -->
```python
print("hello world")
```
<!-- Block-End: {"id": "abc123"} -->
...
1. 所有 JSON 内容必须写成**单行紧凑格式**，例如：
   <!-- Block-Start: {"id": "abc123", "path": "main.py", "version": 1} -->
...
### `runtime.get_block_by_id` 方法
- 功能: 获取指定 ID 的最新版本的代码块对象
- 定义: `get_block_by_id(code_block_id)`
- 参数: `code_block_id` 为代码块的标识符
