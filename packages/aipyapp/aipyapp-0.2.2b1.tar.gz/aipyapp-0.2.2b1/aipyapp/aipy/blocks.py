...
class CodeBlock:
    ...
    def __repr__(self):
        return f"<CodeBlock id={self.id}, version={self.version}, lang={self.lang}, deps={self.deps}, path={self.path}>"
...
class CodeBlocks:
    ...
    def parse(self, markdown_text, parse_mcp=False):
        ...
        for match in self.code_pattern.finditer(markdown_text):
            ...
            if (code_id in blocks or code_id in self.blocks) and version == self.blocks.get(code_id).version:
                self.log.error("Duplicate code id with same version", code_id=code_id, version=version)
                error = {'Duplicate code id with same version': {'code_id': code_id, 'version': version}}
                errors.append(error)
                continue
