...
from ecmind_blue_client import (FieldGroupOperators, ImportActions, Job, Jobs,
                                QueryConditionField, QueryConditionGroup,
                                QueryResultField, ResultFile, SortOrder,
                                SystemFields)
from ecmind_blue_client.blue_exception import BlueException
from ecmind_blue_client.const import (FieldSchema, MainTypeId, QueryOperators,
...
class TestTcpPoolClient(unittest.TestCase):
    ...
    def test_simple_job(self):
        client = TcpPoolClient(
            self.connection_string, "TestApp", "root", "!;`|K!llEF!llE6!;k", self.use_ssl
        )
    ...
    def test_unicode_error(self):
        client = TcpPoolClient(
            self.connection_string, "TestApp", "root", "optimal", self.use_ssl
        )
        ...
        self.assertRegex(
            result.error_message, ".*The DMS field >InvalüdField< could not be found.*"
        )
    ...
    def test_permission_error(self):
        def test_function():
            client = TcpPoolClient(
                self.connection_string, "TestApp", "unknown", "user", self.use_ssl
            )
    ...
    def test_xml_import_from_bytes(self):
        ...
        result = client.xml_import(
            ...
            folder_id=self.folder_id,
            action1=ImportActions.UPDATE,
            files=[RequestFileFromBytes(bytes, "md")],
            options=Options(REPLACETABLEFIELDS=True),
        )
        self.assertEqual(result.return_code, 0)   
    ...
    def test_get_result_list(self):
        ...
        result_rows = result_xml["Archive"][0]["ObjectType"][0]["Rowset"][0]["Rows"][0][
            "Row"
        ]
    ...
    def test_get_object_details(self):
        ...
        self.assertEqual(
            int(obj_details_document[SystemFields.OBJECT_ID.name]), self.doc_id
        )
        self.assertEqual(
            obj_details_document[SystemFields.OBJECT_MAIN.name], "MULTIMEDIA"
        )
        self.assertGreater(len(obj_details_document["TableField"]), 1)

        obj_details_folder = client.get_object_details(
            "Unittest", obj_details_document[SystemFields.SDSTA_ID.name]
        )
    ...
    def test_execute_sql(self):
        ...
        data = client.execute_sql(
            f"UPDATE benutzer SET bemerkung = '{test_data}' WHERE benutzer = 'ROOT'"
        )
        data = client.execute_sql("SELECT * FROM benutzer")
        self.assertEqual(data[0]["bemerkung"], test_data)
        self.assertRaises(Exception, "DEFECT_STATEMENT")
        ...
        result_and = list(
            client.lol_query(
                ...
                conditions=QueryConditionGroup(
                    object_name=object_name, field_conditions=field_conditions
                ),
            )
        )
        result_or = list(
        ...
        result = list(
            client.lol_query(
                ...
                conditions=[
                    ...
                    QueryConditionGroup(
                        object_name="Unittest",
                        field_conditions=[QueryConditionField("StringField", "Unittest Folder")]
                    )
                ],
                result_fields=[
                    "StringField","DateField", SystemFields.OBJECT_ID
                ],
                page_size=2,
                max_hits=10,
                include_file_info=True,
    ...
    def test_custom_field_schema_query(self):
        ...
        result = list(
            client.lol_query(
                object_name="UnittestDoc",
                result_fields=[
                    "DateField", SystemFields.OBJECT_ID, SystemFields.SDSTA_ID
                ],
                page_size=1,
                max_hits=1,
                include_file_info=True,
    ...
    def test_store_in_cache_by_id_convert(self):
        ...
        for doc_info in self.multifile_docs:
            files = client.store_in_cache_by_id(
                doc_info["id"], convert=StoreInCacheByIdConversion.PDF
            )
            self.assertEqual(len(files), 1)
            for file in files:
                self.assertGreater(file.size(), 0)
                self.assertEqual(file.bytes()[0:4].decode("ascii"), "%PDF")
