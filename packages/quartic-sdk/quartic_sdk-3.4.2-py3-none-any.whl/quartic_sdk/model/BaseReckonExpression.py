...
class BaseReckonExpression(metaclass=abc.ABCMeta):
  ...
  """
    ...
    def save(self, client, output_tag_name: str,
             needs: List[str], asset: int, is_streaming: bool,
               tag_category: int, test_df: pd.DataFrame, update: bool = False):
        """
        ...
        try:
            ...
            if get_result['data']['TagExpression'] and get_result['data']['TagExpression'][0]['id']:
                tag_id = get_result['data']['TagExpression'][0]['id']
                self.log.warning(f"Model with name {output_tag_name} with TagExpression id {tag_id} already exists.")
                if not update:
        ...
        if update:
            graphQLQuery = """
                mutation MyMutation($id: ID!, $asset: ID!, $model: String!, $isStreaming: Boolean!, $needs: [String]!, $name: String!, $tagCategory: Int!, $fileName: String!) {
                    TagexpressionUpdate(
                        updateTagexpression: {id: $id, asset: $asset, model: $model,isStreaming: $isStreaming, needs: $needs, name: $name, tagCategory: $tagCategory, fileName: $fileName}
                    ) {
                        ok
                        errors {
                            field
        ...
        try:
            ...
            if update:
                if not res['data']['TagexpressionUpdate']['ok']:
                    raise Exception(res['data']['TagexpressionUpdate']['errors'])
                self.log.info("Successfully updated the model to Quartic Platform")
            else:
