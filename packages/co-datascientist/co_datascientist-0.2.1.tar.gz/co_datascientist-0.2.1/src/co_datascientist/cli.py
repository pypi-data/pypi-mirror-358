...
@click.option('--best-only', is_flag=True, help='Only show the best result found so far (for evolving engines)')
...
@click.option('--batch-size', type=int, default=1, show_default=True,
              help='Number of programs to run in parallel (default: 1 = sequential)')
@click.option('--max-concurrent', type=int, default=None,
              help='Max concurrent processes (default: same as batch-size)')
@click.option('--debug', is_flag=True, help='Show detailed logs (default: minimal output)')
def run(script_path, python_path, best_only, checkpoint_interval, batch_size, max_concurrent, debug):
    """Process a file"""
    ...
    if batch_size > 1:
        max_concurrent = max_concurrent or min(batch_size, 4)  # Default to reasonable limit
        print_info(f"⚡ Batch Mode: Running {batch_size} programs in parallel (max {max_concurrent} concurrent)")
        print()
    ...
    with yaspin(text="Initializing workflow...", color="magenta") as spinner:
        try:
            asyncio.run(workflow_runner.run_workflow(
                ...
                best_only, checkpoint_interval, batch_size, max_concurrent, debug
            ))
