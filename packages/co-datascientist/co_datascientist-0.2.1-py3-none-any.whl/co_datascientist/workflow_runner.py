...
class BatchExecutor:
    """Handles parallel execution of code batches"""
    
    def __init__(self, max_concurrent: int = 4, debug: bool = True):
        self.max_concurrent = max_concurrent
        self.debug = debug
        
    async def run_batch_parallel(self, batch: list, python_path: str) -> list:
        """Run a batch of code programs in parallel using async subprocess (no threading!)"""
        ...
        actual_concurrent = min(self.max_concurrent, len(batch), available_cores)
        threads_per_process = max(1, available_cores // actual_concurrent) if actual_concurrent > 0 else 1
        
        if self.debug:
            print(f"🖥️  [ASYNC] Running {len(batch)} programs with max {actual_concurrent} concurrent processes")
            print(f"   📊 Available CPU cores: {available_cores}")
            print(f"   🎛️ User max_concurrent: {self.max_concurrent}")
            print(f"   🧵 Limiting subprocesses to {threads_per_process} threads each")
            print(f"   ⏰ Program timeout: {settings.script_execution_timeout}s")
        
        # Control concurrency with semaphore only (no threading!)
        semaphore = asyncio.Semaphore(actual_concurrent)
        
        async def run_single_async(code_version, num_threads: int):
            program_start_time = time.time()
            
            try:
                async with semaphore:
                    execution_start_time = time.time()
                    wait_time_ms = int((execution_start_time - program_start_time) * 1000)
                    
                    # Create temp file for the Python code
                    with tempfile.NamedTemporaryFile(delete=False, suffix=".py") as temp_file:
                        temp_file.write(code_version.code.encode('utf-8'))
                        temp_file_path = temp_file.name
                    
                    try:
                        # Limit threads used by libs like numpy, numba, etc.
                        env_vars = (
                            f"OMP_NUM_THREADS={num_threads} "
                            f"MKL_NUM_THREADS={num_threads} "
                            f"OPENBLAS_NUM_THREADS={num_threads} "
                            f"BLIS_NUM_THREADS={num_threads} "
                            f"VECLIB_MAXIMUM_THREADS={num_threads} "
                            f"NUMEXPR_NUM_THREADS={num_threads}"
                        )
                        
                        # Use pure asyncio subprocess (no threading!)
                        cmd = f"{env_vars} {python_path} {temp_file_path}"
                        ...
                        try:
                            ...
                            process.kill()
                            await process.wait()
                            stdout, stderr = b"", f"Process timed out after {settings.script_execution_timeout} seconds".encode()
                        
                        execution_end_time = time.time()
                        runtime_ms = int((execution_end_time - execution_start_time) * 1000)
                        total_time_ms = int((execution_end_time - program_start_time) * 1000)
                        ...
                        # Clean status output (using local timing variables for display) - only in debug mode
                        if self.debug:
                            status = "✅" if result.return_code == 0 else "❌"
                            program_name = code_version.name[:25].ljust(25)
                            print(f"   {status} {program_name} | Runtime: {runtime_ms:4}ms | Wait: {wait_time_ms:4}ms | RC: {result.return_code}")
                        
                        return (code_version.code_version_id, result)
                ...
                error_msg = f"Failed to execute {code_version.name}: {type(e).__name__}: {e}"
                if self.debug:
                    print(f"   ❌ {code_version.name[:25].ljust(25)} | ERROR: {error_msg}")
                return ValueError(error_msg)  # Return a specific exception type
        
        # Execute all tasks with controlled concurrency
        if self.debug:
            print(f"⏱️  [TIMING] Starting async execution...")
        tasks = [run_single_async(cv, threads_per_process) for cv in batch]
        ...
        if self.debug:
            ...
            successful_results = []
            for r in results:
                if not isinstance(r, Exception) and isinstance(r, tuple) and len(r) == 2:
                    code_version_id, result = r
                    if hasattr(result, 'runtime_ms') and not isinstance(result, Exception):
                        successful_results.append(r)
            
            if successful_results:
                individual_runtimes = [r[1].runtime_ms for r in successful_results]
                total_individual_time = sum(individual_runtimes)
                avg_runtime = total_individual_time / len(individual_runtimes)
                
                print(f"⏱️  [TIMING] Batch completed in {total_batch_time:.2f}s")
                print(f"   📊 Individual total runtime: {total_individual_time/1000:.2f}s")
                print(f"   📊 Average program runtime: {avg_runtime:.0f}ms") 
                print(f"   💾 Actual concurrent slots used: {actual_concurrent}")
                print(f"   ✅ Successful programs: {len(successful_results)}/{len(results)}")
            else:
                print(f"⏱️  [TIMING] Batch completed in {total_batch_time:.2f}s (with errors)")
    ...
    async def run_workflow(self, code: str, python_path: str, project_absolute_path: str, spinner=None, best_only: bool = False, checkpoint_interval: int = 10, batch_size: int = 1, max_concurrent: int = None, debug: bool = True):
        ...
        try:
            ...
            if best_only:
                ...
                print(f"🎯 Best-Only Mode: Tracking improvements only")
                print()

            # Choose execution mode based on batch_size
            if batch_size > 1:
                # BATCH MODE: Parallel execution
                batch_executor = BatchExecutor(max_concurrent, self.debug_mode)
                await self._run_batch_mode(batch_executor, python_path, project_absolute_path, 
                                         best_tracker, spinner, best_only, batch_size)
            else:
            ...
            if spinner:
                spinner.stop()
            print_workflow_error(f"Workflow error: {e}")
            raise

    async def _run_batch_mode(self, batch_executor: BatchExecutor, python_path: str, 
                             project_absolute_path: str, best_tracker: BestTracker = None, 
                             spinner=None, best_only: bool = False, user_batch_size: int = None):
        """Run workflow in batch mode (parallel execution)"""
        ...
        while not self.workflow.finished and not self.should_stop_workflow:
            ...
            if spinner:
                spinner.text = "Getting batch to run..."
                spinner.start()
            
            try:
                # Pass user's batch size to backend
                if user_batch_size is not None:
                    batch_response = await co_datascientist_api.get_batch_to_run(self.workflow.workflow_id, batch_size=user_batch_size)
                    if self.debug_mode:
                        print(f"🎛️ [USER] Requesting {user_batch_size} programs from engine...")
                else:
                    batch_response = await co_datascientist_api.get_batch_to_run(self.workflow.workflow_id)
                    if self.debug_mode:
                        print(f"🎛️ [DEFAULT] Using engine default batch size...")

                if not batch_response.batch_to_run:
                    # No more batches - workflow finished
                    break
                    
                batch = batch_response.batch_to_run
                batch_id = batch_response.batch_id
                self.workflow = batch_response.workflow
                ...
                if self.debug_mode:
                    print(f"🐛 [DEBUG] Received batch of {len(batch)} programs:")
                    for i, code_version in enumerate(batch):
                        print(f"   {i+1}. {code_version.name} (ID: {code_version.code_version_id[:12]}...)")
                        print(f"      Code length: {len(code_version.code)} chars")
                        print(f"      Idea: {code_version.idea[:50]}..." if len(code_version.idea) > 50 else f"      Idea: {code_version.idea}")
                    
                # Handle spinner for batch execution
                if best_only and best_tracker:
                    ...
                    if self.debug_mode:
                        print(f"⚡ Running batch of {len(batch)} programs in parallel...")
                    if spinner:
                        spinner.text = f"Executing {len(batch)} programs..."
                        spinner.start()
                
                # Execute batch in parallel
                results = await batch_executor.run_batch_parallel(batch, python_path)
                
                # Process results
                if best_only and best_tracker:
                    show_results = await self._handle_batch_best_only_results(
                        results, batch, project_absolute_path, best_tracker, spinner
                    )
                    if show_results and spinner and continuous_spinner_active:
                        spinner.text = "Glowing Up ✨"
                        spinner.start()
                        continuous_spinner_active = True
                else:
                    if spinner:
                        spinner.stop()
                    await self._handle_batch_standard_results(results, batch)
                    print()
                
                # Save the processed batch to generation-based folders
                await self._save_batch_to_generation_folder(batch, results, project_absolute_path)
                
                # Submit results to backend
                batch_results = []
                for result in results:
                    # Handle both successful results (tuples) and exceptions
                    if isinstance(result, Exception):
                        if self.debug_mode:
                            print(f"⚠️  [DEBUG] Skipping result due to exception: {result}")
                        continue
                    elif isinstance(result, tuple) and len(result) == 2:
                        code_version_id, code_result = result
                        if not isinstance(code_result, Exception):
                            batch_results.append({
                                "code_version_id": code_version_id, 
                                "result": code_result.model_dump()
                            })
                        else:
                            if self.debug_mode:
                                print(f"⚠️  [DEBUG] Skipping result with exception in code_result: {code_result}")
                    else:
                        if self.debug_mode:
                            print(f"⚠️  [DEBUG] Unexpected result format: {result}")
                
                if self.debug_mode:
                    print(f"🐛 [DEBUG] Submitting {len(batch_results)} results to backend...")
                batch_response = await co_datascientist_api.finished_running_batch(
                    self.workflow.workflow_id, batch_id, batch_results
                )
                self.workflow = batch_response.workflow
    ...
    async def _run_sequential_mode(self, response, python_path: str, 
        ...
        while not self.workflow.finished and response.code_to_run is not None and not self.should_stop_workflow:
            ...
            response = await co_datascientist_api.finished_running_code(
                self.workflow.workflow_id, response.code_to_run.code_version_id, result
            )
            self.workflow = response.workflow
