...
class InteliaLLMClient:
    ...
    def __init__(
        self,
        pht_api_token: str = None,
        litellm_gen_key: str = None,
    ...
    def call_llm(self, prompt: str, model: str, account_id: int | None = None, key: str | None = None, tags: list | None = None, no_cache: bool = False, metadata: dict | None = None, return_tokens: bool = False) -> Tuple[str, int, int]:
        """
        ...
        if not account_id and not key:
            raise ValueError("Você deve fornecer um account_id ou uma chave (key) para a chamada LLM.")
    ...
    def _get_pht_account_details(self, account_id: int) -> dict[str, Any]:
        ...
        if not self.pht_api_token:
            raise ValueError("pht_api_token não foi configurado. Verifique as credenciais.")
    ...
    def _create_litellm_key(self, account_id: int) -> str:
        ...
        if self.litellm_gen_key is None:
            raise ValueError("litellm_gen_key não foi configurado. Verifique as credenciais.")
        if self.litellm_user_id is None:
            raise ValueError("litellm_user_id não foi configurado. Verifique as credenciais.")
        if self.litellm_team_id is None:
            raise ValueError("litellm_team_id não foi configurado. Verifique as credenciais.")  
