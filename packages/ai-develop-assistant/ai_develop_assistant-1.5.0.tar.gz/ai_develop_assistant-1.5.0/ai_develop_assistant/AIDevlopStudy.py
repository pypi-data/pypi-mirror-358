...
class RequirementStorage:
    ...
    def export_final_document(self):
        ...
        try:
            ...
            # 测试写权限
            test_file = self.storage_dir / "test_write.tmp"
            try:
                test_file.write_text("test")
                test_file.unlink()  # 删除测试文件
            except Exception as perm_error:
                logger.error(f"❌ 存储目录权限不足: {perm_error}")
                return None

            # 准备导出数据，处理不可序列化的对象
            serializable_requirements = self._prepare_serializable_data(current_requirements)
    ...
    def _prepare_serializable_data(self, data):
        """准备可序列化的数据"""
        if isinstance(data, dict):
            result = {}
            for key, value in data.items():
                if key == "structured_requirements":
                    # 转换结构化需求为可序列化格式
                    result[key] = {}
                    for req_id, entity in value.items():
                        if hasattr(entity, 'to_dict'):
                            result[key][req_id] = entity.to_dict()
                        else:
                            result[key][req_id] = entity
                else:
                    result[key] = self._prepare_serializable_data(value)
            return result
        elif isinstance(data, list):
            return [self._prepare_serializable_data(item) for item in data]
        else:
            return data
    ...
    def generate_markdown_report(self, md_file: Path):
        ...
        try:
            with open(md_file, 'w', encoding='utf-8') as f:
                ...
                # 设计需求
                if current_requirements.get("design_requirements"):
                    f.write("## 🎨 设计需求\n\n")
                    for item in current_requirements["design_requirements"]:
                        content = item.get("content", item) if isinstance(item, dict) else item
                        f.write(f"- {content}\n")
                    f.write("\n")

                # 结构化需求分析
                if current_requirements.get("structured_requirements"):
                    f.write("## 📊 需求分析报告\n\n")
                    self._write_structured_analysis(f)
                ...
                # 开发流程建议
                f.write("## 🔄 开发流程建议\n\n")
                self._write_development_process(f)

                # 模块化设计指导
                f.write("## 📦 模块化设计指导\n\n")
                self._write_modularity_guidance(f)

            logger.info(f"✅ Markdown报告已生成: {md_file}")
        except Exception as e:
            logger.error(f"❌ 生成Markdown报告失败: {e}")

    def _write_structured_analysis(self, f):
        """写入结构化需求分析"""
        structured_reqs = current_requirements.get("structured_requirements", {})
        ...
        for req_id, entity in structured_reqs.items():
            req_type = entity.type if hasattr(entity, 'type') else entity.get('type', 'unknown')
        ...
        module_impacts = {}
        for req_id, entity in structured_reqs.items():
            impacts = entity.impacts if hasattr(entity, 'impacts') else entity.get('impacts', [])
            for impact in impacts:
                module_impacts[impact] = module_impacts.get(impact, 0) + 1

        if module_impacts:
            for module, count in sorted(module_impacts.items(), key=lambda x: x[1], reverse=True):
                f.write(f"- **{module}**: {count} 个需求影响\n")
        else:
            f.write("- 暂无模块影响分析数据\n")
        f.write("\n")

    def _write_development_process(self, f):
        """写入开发流程建议"""
        f.write("### 推荐开发流程\n\n")
        f.write("1. **需求确认阶段**\n")
        f.write("   - 与用户确认需求文档的准确性\n")
        f.write("   - 明确项目范围和优先级\n\n")

        f.write("2. **架构设计阶段**\n")
        f.write("   - 基于需求分析进行模块划分\n")
        f.write("   - 设计系统架构和接口规范\n\n")
    ...
    def _write_modularity_guidance(self, f):
        """写入模块化设计指导"""
        f.write("### 模块化设计原则\n\n")
        f.write("1. **单一职责原则**\n")
        f.write("   - 每个模块只负责一个特定的功能领域\n")
        f.write("   - 避免模块功能过于复杂\n\n")

        f.write("2. **低耦合原则**\n")
        f.write("   - 模块间通过明确的接口进行通信\n")
        f.write("   - 减少模块间的直接依赖\n\n")
...
def requirement_clarifier(user_input: str, context: str = "") -> str:
    ...
    if not completeness_check["is_sufficient"]:
        ...
        analysis_prompt += f"\n\n## 🤔 **用户选择**"
        analysis_prompt += f"\n请选择下一步处理方式："
        analysis_prompt += f"\n1. **继续澄清**：我来提供更详细的{next_priority}需求信息"
        analysis_prompt += f"\n2. **AI自主发挥**：让AI基于现有信息自动补充并设计架构"
        analysis_prompt += f"\n\n💡 **建议**：继续澄清可获得更精准的架构设计"
    else:
        # 需求充足，可以进入下一阶段
...
# 智能补充工具
@mcp.tool()
def intelligent_supplement(supplement_type: str = "auto") -> str:
    """智能补充缺失的需求信息

    Args:
        supplement_type: 补充类型 ("auto", "functional", "technical", "ui_design", "project_goals")
    """
    ...
    try:
        ...
        if completeness_check["is_sufficient"]:
            return "✅ 当前需求已经足够完善，无需智能补充。可以直接进行架构设计。"

        # 确定需要补充的分支
        if supplement_type == "auto":
            # 自动选择最需要补充的分支
            target_branch = IntelligentClarificationEngine._calculate_next_priority()
        else:
            target_branch = supplement_type

        # 基于现有需求智能推断缺失信息
        supplemented_requirements = _generate_intelligent_supplements(target_branch)

        # 保存补充的需求
        supplement_count = 0
        for req_content in supplemented_requirements:
            req_id = RequirementManager.add_requirement(
                req_type=target_branch,
                content=req_content,
                priority="medium",
                source="ai_inferred"
            )
        ...
        # 重新检查完整性
        updated_check = _check_requirements_completeness()

        return f"""# 🤖 智能补充完成

## 📝 补充详情
- **目标分支**: {target_branch}
- **补充需求数量**: {supplement_count}
- **补充来源**: AI智能推断

## 📊 补充后状态
- **完成率**: {branch_status['completion_rate']:.0%} → {updated_check['branch_status']['completion_rate']:.0%}
- **平均质量**: {updated_check['quality_check']['avg_quality']:.2f}

## 🔍 补充的需求
{chr(10).join(f"- {req}" for req in supplemented_requirements)}

## 🎯 下一步建议
{'✅ 需求已足够完善，可以进行架构设计' if updated_check['is_sufficient'] else '⚠️ 建议继续澄清或再次智能补充'}

---
💡 **提示**: 智能补充基于AI推断，建议用户确认补充内容的准确性。
"""

    except Exception as e:
        logger.error(f"❌ 智能补充失败: {str(e)}")
        return f"智能补充过程中出现错误: {str(e)}"

def _generate_intelligent_supplements(target_branch: str) -> list:
    """基于现有需求智能生成补充内容"""

    # 分析现有需求内容
    existing_reqs = []
    for req_type, reqs in current_requirements.items():
        if isinstance(reqs, list) and req_type.endswith('_requirements'):
            existing_reqs.extend(reqs)

    # 基于项目特征和现有需求推断缺失内容
    project_context = " ".join(existing_reqs)

    supplements = []

    if target_branch == "project_goals":
        supplements = _supplement_project_goals(project_context)
    elif target_branch == "functional_design":
        supplements = _supplement_functional_requirements(project_context)
    elif target_branch == "technical_preferences":
        supplements = _supplement_technical_requirements(project_context)
    elif target_branch == "ui_design":
        supplements = _supplement_ui_requirements(project_context)

    return supplements

def _supplement_project_goals(context: str) -> list:
    """补充项目目标需求"""
    supplements = []

    if "用户" not in context:
        supplements.append("目标用户群体：普通互联网用户，具备基本的计算机操作能力")

    if "价值" not in context and "解决" not in context:
        supplements.append("核心价值：提供便捷高效的在线服务，解决用户的实际需求")

    if "成功" not in context and "指标" not in context:
        supplements.append("成功指标：用户活跃度、功能使用率、用户满意度")

    return supplements

def _supplement_functional_requirements(context: str) -> list:
    """补充功能需求"""
    supplements = []

    if "登录" not in context and "用户" in context:
        supplements.append("用户认证：支持用户注册、登录、密码重置功能")

    if "管理" in context and "权限" not in context:
        supplements.append("权限管理：不同用户角色具有不同的操作权限")

    if "数据" in context and "搜索" not in context:
        supplements.append("数据检索：支持关键词搜索和筛选功能")

    return supplements

def _supplement_technical_requirements(context: str) -> list:
    """补充技术需求"""
    supplements = []

    if "数据库" not in context:
        supplements.append("数据存储：使用关系型数据库（如MySQL）存储业务数据")

    if "性能" not in context:
        supplements.append("性能要求：页面响应时间不超过2秒，支持并发用户访问")

    if "安全" not in context:
        supplements.append("安全要求：数据传输加密，用户信息安全保护")

    return supplements

def _supplement_ui_requirements(context: str) -> list:
    """补充UI设计需求"""
    supplements = []

    if "响应式" not in context and "移动" not in context:
        supplements.append("响应式设计：支持桌面端和移动端自适应显示")

    if "简洁" not in context and "界面" not in context:
        supplements.append("界面风格：简洁现代的设计风格，良好的用户体验")

    if "交互" not in context:
        supplements.append("交互设计：直观的操作流程，清晰的导航结构")
...
def _show_choice_interface(completeness_check):
    """显示强制用户选择界面"""
    branch_status = completeness_check["branch_status"]
    quality_check = completeness_check["quality_check"]
...
## 📊 质量评估
- **平均质量**: {quality_check['avg_quality']:.0%}
- **需求总数**: {quality_check['total_requirements']}

## 🎯 必须选择处理方式
请明确选择以下操作之一：

1. **继续澄清**: 使用 requirement_clarifier 补充更多需求细节
   - 获得高质量专业架构设计（推荐）

2. **AI自由发挥**: 调用 architecture_designer 并添加参数 design_focus="ai_auto"
   - AI自动补充缺失需求并生成完整架构

3. **快速原型**: 调用 architecture_designer 并添加参数 design_focus="simplified"
   - 基于现有信息生成基础架构

⚠️ **重要**: 必须明确选择其中一种方式才能继续"""

def _auto_supplement_requirements():
    """AI自动补充缺失的需求信息 - 仅补充技术栈，不擅自定义UI"""

    # 分析现有需求内容
    existing_content = []
    for req_type in ["project_overview", "functional_requirements", "technical_requirements"]:
        reqs = current_requirements.get(req_type, [])
        for req in reqs:
            content = req.get("content", req) if isinstance(req, dict) else req
            existing_content.append(content)

    context = " ".join(existing_content)

    # 补充项目目标（如果缺失）
    if not current_requirements.get("project_overview"):
        current_requirements["project_overview"].append("基于现有功能需求推断的项目目标和用户群体")

    # 补充技术需求（如果缺失）
    if not current_requirements.get("technical_requirements"):
        if "web" in context.lower() or "网站" in context:
            current_requirements["technical_requirements"].append("Web技术栈：前端框架+后端API+数据库，支持响应式设计")
        else:
            current_requirements["technical_requirements"].append("现代技术栈，确保性能和可维护性")
...
def architecture_designer(design_focus: str = "full_architecture") -> str:
    ...
    # 基础需求检查
    has_basic_requirements = bool(current_requirements.get("project_overview") or
                                 current_requirements.get("functional_requirements"))

    # 优先检查是否需要用户选择（强制选择机制）
    if not completeness_check["is_sufficient"] and design_focus == "full_architecture":
        return _show_choice_interface(completeness_check)

    # 处理用户明确选择的模式
    if design_focus == "ai_auto":
        # AI自动模式：用户明确选择后执行
        if not has_basic_requirements:
            return "❌ AI自动模式仍需要至少一个项目概述或功能需求作为基础。请使用 requirement_clarifier 添加基本信息。"
        _auto_supplement_requirements()
    elif design_focus == "simplified":
        # 简化模式：用户明确选择后执行
        if not has_basic_requirements:
            return "❌ 简化模式仍需要至少一个项目概述或功能需求。请使用 requirement_clarifier 添加基本信息。"
        # 这个分支现在不应该被执行，因为逻辑已经调整
        return "❌ 内部逻辑错误，请重新调用 architecture_designer"
