...
class SigmaPI:
    ...
    def calculate(
        ...
        # Unscale gradients if a GradScaler is provided
        if scaler is not None:
            scale = scaler.get_scale()
            # Create copies for calculation to not interfere with the scaler's state.
            unscaled_grads = [grad.detach() / scale for grad in model_grads]
            surprise_tensor = get_surprise_from_grads_torch(unscaled_grads)
        else:
