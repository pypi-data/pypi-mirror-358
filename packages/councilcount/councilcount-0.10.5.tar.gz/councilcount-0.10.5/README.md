...
# here is an example
acs_year = '2023' # Note: in R you may need to input years as strings in order for the functions to work
cc$get_available_councilcount_codes(acs_year=acs_year)
```

If you run a function in R that requires a year as an input, and there is error message suggesting that an available year is not available, input the same year as a string and try again. This may solve the problem.
...
First, review the codes available in the existing CouncilCount database and ensure that the code(s) you are looking for is missing. Generate your own census API key [here](https://api.census.gov/data/key_signup.html):
```Python
acs_year = 2011
