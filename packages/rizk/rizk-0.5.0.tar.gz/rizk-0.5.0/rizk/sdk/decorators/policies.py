"""Provides the @add_policies decorator for applying Rizk guardrails."""
...
logger = logging.getLogger("rizk.decorators.policies")


class RizkPolicyViolation(Exception):
    ...
    def __init__(self, message: str, violated_policies: Optional[list[str]] = None):
        """Initializes the RizkPolicyViolation exception.
...
def add_policies(
    ...
    *,  # Make subsequent arguments keyword-only
    ...
    """(DEPRECATED) Decorator to apply Rizk policy guardrails to function inputs and outputs.

    WARNING: This decorator is deprecated and will be removed in a future version.
    Please use the new `@rizk.guardrails` decorator instead, which utilizes framework
    ...
    Args:
        ...
        on_input_violation: Action on input violation: 'default' (return result dict),
                           'exception' (raise RizkPolicyViolation), 'alternative' (return string).
        on_output_violation: Action on output violation: 'alternative' (return string),
                             'exception' (raise RizkPolicyViolation), 'redact' (use transformed_response).
    ...
    Raises:
        RizkPolicyViolation: If `on_input_violation` or `on_output_violation` is 'exception' and a policy is violated.
    """
    logger.warning(
        "The @add_policies decorator is deprecated. Please switch to @rizk.guardrails."
    )
    ...
    def decorator(func: F) -> F:
        ...
        async def async_wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if not input_result["allowed"]:
                ...
                if on_input_violation == "exception":
                    raise RizkPolicyViolation(str(reason), violated)
        ...
        # Please migrate to @rizk.guardrails and use 'async def'.
        ...
        if is_async:
            ...
            raise TypeError(
                "The deprecated @add_policies decorator no longer supports synchronous functions (`def`). Please migrate to @rizk.guardrails and use `async def`."
            )
...
def guardrails(
    ...
    """Decorator to apply Rizk guardrails to individual LLM calls.
    ...
    Args:
        augment_prompt: If True, attempt to augment the system prompt.
        check_output: If True, check the function's string output against policies.
        on_input_violation: Action on input violation: 'exception' (raise RizkPolicyViolation),
...
__all__ = ["add_policies", "RizkPolicyViolation", "guardrails"]
