"""Base OpenTelemetry decorator implementations for the Rizk SDK.

Provides wrappers around Traceloop decorators OR framework-specific adapters
to automatically add Rizk-specific context attributes and apply framework-aware tracing.
"""

import functools
import logging
from typing import Optional, Callable, Any, Dict, Type, TypeVar, cast, Union
...
logger = logging.getLogger("rizk.decorators.base")

# Mapping from Traceloop span kind to Rizk entity type (keep as before)
_SPAN_KIND_TO_RIZK_ENTITY_TYPE: Dict[TraceloopSpanKindValues, str] = {
...
def _add_rizk_context(
    ctx_attributes: Optional[Dict[str, str]],
    span_kind: Optional[
        TraceloopSpanKindValues
    ] = None,  # Made optional, may not always apply
) -> None:
    """Helper to add custom rizk.* attributes to the current OTel context."""
    current_ctx = context.get_current()
    new_values: Dict[str, Any] = {}

    # Add custom context attributes prefixed with 'rizk.'
    if ctx_attributes:
        for key, value in ctx_attributes.items():
            new_values[f"rizk.{key}"] = value

    # Add rizk.entity_type based on span kind if provided
    if span_kind:
        entity_type = _SPAN_KIND_TO_RIZK_ENTITY_TYPE.get(span_kind)
        if entity_type:
            new_values["rizk.entity_type"] = entity_type
...
def _apply_rizk_decorator(
    decorator_type: str,  # e.g., "workflow", "tool"
    func_or_class: Union[F, C],
    ...
    **kwargs: Any,  # Pass other kwargs (like version, and context_attributes/tlp_span_kind for fallback context) through
    ...
    """Core helper function to apply Rizk decorators with framework adaptation."""
    ...
    logger.debug(
        f"Applying Rizk decorator '{decorator_type}' for detected framework '{framework}'. Adapter: {type(adapter).__name__ if adapter else 'None'}"
    )
    ...
    adapted_func_or_class: Union[F, C] = (
    ...
    if adapter and hasattr(adapter, adapter_method_name):
        try:
            ...
            # Separate Rizk-specific context args from args meant for the adapter/traceloop
            adapter_kwargs = {
                k: v
            ...
            logger.warning(
                f"Rizk Adapter '{type(adapter).__name__}' does not implement '{adapter_method_name}' for framework '{framework}'. Falling back if possible."
            )
        except Exception as e:
            logger.error(
                f"Rizk Error applying adapter '{adapter_method_name}' for '{framework}': {e}. Falling back if possible.",
                exc_info=True,
            )
    ...
    if not applied_adapter:
        if fallback_decorator:
            try:
                ...
                # Separate Rizk-specific context args from args meant for the traceloop decorator
                traceloop_kwargs = {
                    k: v
                    for k, v in kwargs.items()
                    if k not in ["context_attributes", "tlp_span_kind"]
                ...
                # Wrap the Traceloop decorator to add Rizk context *before* it runs
                # This is only needed if we didn't use a Rizk adapter
                @functools.wraps(
                    func_or_class
                ...
                def context_wrapper(*args: Any, **wrapper_kwargs: Any) -> Any:
                    ...
                    # Get Rizk context attributes passed to the original decorator call from original kwargs
                    rizk_ctx_attrs = kwargs.get("context_attributes", {})
                    _add_rizk_context(rizk_ctx_attrs, tlp_span_kind)
                ...
                adapted_func_or_class = cast(Union[F, C], context_wrapper)
            ...
            logger.warning(
                f"Rizk Warning: No adapter method '{adapter_method_name}' found for '{framework}' and no Traceloop fallback available for decorator type '{decorator_type}'. Returning original function/class without tracing."
            )
...
# These functions now primarily gather context and call the core _apply_rizk_decorator
...
def workflow_with_context(
    ...
    **other_kwargs: Any,  # Catch any other traceloop args
    ...
    Adds Rizk context attributes (organization_id, project_id) if falling back.
    """
    ctx_attrs = {}
    if organization_id:
    ...
    def decorator(func_or_class: Union[F, C]) -> Union[F, C]:
        # Pass context attributes and span kind for potential fallback use
        return _apply_rizk_decorator(
            "workflow",
            func_or_class,
            name=name,
            version=version,
...
# --- DEPRECATED ORIGINAL DECORATORS (entity_method, entity_class) ---
# These are no longer the primary mechanism. Keep for reference or remove later.
