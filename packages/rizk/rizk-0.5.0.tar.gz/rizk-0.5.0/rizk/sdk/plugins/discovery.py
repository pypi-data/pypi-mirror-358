"""Plugin Discovery System for Rizk SDK.

This module provides automatic discovery of Rizk plugins from various sources:
- Local directories
...
class PluginDiscovery:
    """Discovers Rizk plugins from multiple sources."""
    ...
    def __init__(
        ...
        # Discovery results
        self.discovered_plugins: Dict[str, Type[RizkPlugin]] = {}
        ...
        # Discovery hooks
        self._discovery_hooks: List[Callable[[Dict[str, Type[RizkPlugin]]], None]] = []
    ...
    def _get_default_plugin_dirs(self) -> List[str]:
        ...
        home_dir = Path.home()
        user_plugin_dir = home_dir / ".rizk" / "plugins"
        dirs.append(str(user_plugin_dir))

        # System-wide plugin directory (Unix-like systems)
        if os.name != "nt":  # Not Windows
            dirs.append("/etc/rizk/plugins")
            dirs.append("/usr/local/lib/rizk/plugins")

        # Current working directory plugins
        dirs.append("./rizk_plugins")
        dirs.append("./plugins")

        # Environment variable override
        env_dirs = os.getenv("RIZK_PLUGIN_DIRS")
        if env_dirs:
    ...
    def add_discovery_hook(
        self, hook: Callable[[Dict[str, Type[RizkPlugin]]], None]
    ) -> None:
    ...
    def _scan_installed_packages(self) -> None:
        """Scan installed packages for Rizk plugins."""
        logger.debug("Scanning installed packages for plugins...")

        # Look for packages with 'rizk_plugin_' prefix
        for finder, name, ispkg in pkgutil.iter_modules():
            if name.startswith("rizk_plugin_"):
                try:
                    module = importlib.import_module(name)
                    plugin_name = name.replace("rizk_plugin_", "")
    ...
    def _scan_entry_points(self) -> None:
        """Scan entry points for Rizk plugins."""
        try:
            ...
            try:
                ...
                eps_result = entry_points(group="rizk.plugins")
                ...
                try:
                    ...
                    eps = pkg_resources.iter_entry_points("rizk.plugins")
                except ImportError:
            ...
            for ep in eps:
                try:
                    ...
                    if issubclass(plugin_class, RizkPlugin):
                        plugin_name = ep.name
    ...
    def _extract_plugins_from_module(
        ...
        for attr_name in dir(module):
            ...
            if (
                ...
                and issubclass(attr, RizkPlugin)
                and attr is not RizkPlugin
            ):
    ...
    def _validate_plugins(self) -> None:
        ...
        for plugin_name, plugin_class in self.discovered_plugins.items():
            try:
                # Basic validation
                if not issubclass(plugin_class, RizkPlugin):
                    raise PluginLoadError(
                        f"Plugin {plugin_name} does not inherit from RizkPlugin"
                    )
...
def discover_plugins(force_rediscover: bool = False) -> Dict[str, Type[RizkPlugin]]:
