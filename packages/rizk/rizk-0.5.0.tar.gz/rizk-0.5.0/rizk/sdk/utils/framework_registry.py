...
import weakref
import time
from typing import Dict, Optional, Type, Callable, List, TYPE_CHECKING, Any

# Moved E402 imports to be before other module-level code
from rizk.sdk.adapters.framework_adapter import RizkFrameworkAdapter
from rizk.sdk.adapters.llm_base_adapter import BaseLLMAdapter  # Import LLM base
from rizk.sdk.utils.framework_detection import (
    detect_framework_cached as detect_framework,
)
from rizk.sdk._lazy_imports import LAZY_FRAMEWORK_ADAPTERS, LAZY_LLM_ADAPTERS

# Plugin system imports with fallback
if TYPE_CHECKING:
    from rizk.sdk.plugins.manager import PluginManager, get_global_plugin_manager
    from rizk.sdk.plugins import PluginCapability
    ...
    try:
        from rizk.sdk.plugins.manager import get_global_plugin_manager
        from rizk.sdk.plugins import PluginCapability
...
logger = logging.getLogger("rizk.utils.framework_registry")

# Import the base adapter class types and detection function # This comment is now slightly out of place but fine
# from rizk.sdk.adapters.framework_adapter import RizkFrameworkAdapter # Moved up
# from rizk.sdk.adapters.llm_base_adapter import BaseLLMAdapter # Import LLM base # Moved up
# from rizk.sdk.utils.framework_detection import detect_framework # Moved up
...
class FrameworkRegistry:
    ...
    _adapter_classes: Dict[str, Type[RizkFrameworkAdapter]] = {}
    _adapter_instances: "weakref.WeakValueDictionary[str, RizkFrameworkAdapter]" = (
    ...
    def register_adapter(
        cls, framework_name: str, adapter_instance: RizkFrameworkAdapter
    ) -> None:
    ...
    ) -> Optional[Type[RizkFrameworkAdapter]]:
        """
    ...
    @classmethod
    def get_adapter(cls, framework_name: str) -> Optional[RizkFrameworkAdapter]:
        """
    ...
    @classmethod
    def get_all_adapter_instances(cls) -> Dict[str, RizkFrameworkAdapter]:
    ...
    # Removed get_all_adapters (ambiguous, replaced by get_all_adapter_instances)
...
def register_all_framework_adapters() -> Dict[str, RizkFrameworkAdapter]:
    """
    ...
    for name, lazy_adapter in LAZY_FRAMEWORK_ADAPTERS.items():
        ...
        if name in ["openai_agents", "agents_sdk"]:
            # Always try OpenAI agents as it's core (both names refer to same adapter)
...
def register_all_llm_adapters() -> Dict[str, BaseLLMAdapter]:
    ...
    if openai_imported:
        ...
        for adapter_name in legacy_adapters:
            try:
                module_map = {
                    "openai_completion": "rizk.sdk.adapters.openai_completion_adapter.OpenAICompletionAdapter",
                    "openai_responses": "rizk.sdk.adapters.openai_responses_adapter.OpenAIResponsesAdapter",
                }
...
# Register all adapters when this module is imported - Keep this behavior?
# Consider if explicit registration call is better from Rizk.init()
