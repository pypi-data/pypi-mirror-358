...
logger = logging.getLogger("rizk.utils.framework_detection")
...
class FrameworkDetectionCache:
    """Thread-safe cache for framework detection results."""
        ...
        with self._lock:
            if cache_key in self._cache:
                ...
                if time.time() - timestamp < self.ttl_seconds:
                    self._hits += 1
                    logger.debug(f"Cache HIT: {cache_key} -> {framework}")
                    return framework
                else:
    ...
    def set(
        ...
        with self._lock:
            ...
            logger.debug(f"Cache SET: {cache_key} -> {framework}")
...
def get_detection_cache_stats() -> Dict[str, Any]:
    ...
    return _get_detection_cache().get_stats()


def clear_detection_cache() -> None:
    """Clear the framework detection cache."""
    _get_detection_cache().clear()
    logger.info("Framework detection cache cleared")
