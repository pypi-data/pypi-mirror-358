"""Built-in Analytics Processors for Rizk SDK.
...
class ConsoleAnalyticsProcessor(AnalyticsProcessor):
    ...
    def __init__(
        ...
        self.analytics_logger = logging.getLogger("rizk.analytics.console")
...
class RizkHubProcessor(AnalyticsProcessor):
    """Analytics processor that sends data to Rizk Hub API."""

    def __init__(
        self,
        api_key: Optional[str] = None,
        api_endpoint: str = "https://api.rizk.tools",
        batch_size: int = 50,
        ...
        """Initialize Rizk Hub processor.

        Args:
            api_key: Rizk API key for authentication
            api_endpoint: Rizk Hub API endpoint
        ...
        """
        self.api_key = api_key or os.getenv("RIZK_API_KEY")
        ...
        if not self.api_key:
            logger.warning(
                "RizkHubProcessor: No API key provided. Data will not be sent to Rizk Hub."
            )

        self._start_auto_flush()
        logger.info(f"RizkHubProcessor initialized: {self.api_endpoint}")
    ...
    def _should_include_event(self, event: AnalyticsEvent) -> bool:
        """Determine if an event should be sent to Rizk Hub."""
        if not self.api_key:
            return False
    ...
    def _should_include_metric(self, metric: Metric) -> bool:
        """Determine if a metric should be sent to Rizk Hub."""
        if not self.api_key:
            return False
    ...
    def _flush_events(self) -> None:
        """Flush events buffer to Rizk Hub."""
        ...
        try:
            ...
            logger.error(f"Failed to send events to Rizk Hub: {e}")
    ...
    def _flush_metrics(self) -> None:
        """Flush metrics buffer to Rizk Hub."""
        ...
        try:
            ...
            logger.error(f"Failed to send metrics to Rizk Hub: {e}")
            self.stats["send_errors"] = (self.stats.get("send_errors", 0) or 0) + 1
    ...
    async def _send_events_async(self, events: List[AnalyticsEvent]) -> None:
        """Send events to Rizk Hub API asynchronously."""
        if not events:
            return
        ...
        headers = {
            ...
            "User-Agent": "rizk-sdk-python/0.1.0",
        }
                ...
                try:
                    async with session.post(
                        ...
                        if response.status == 200:
                            logger.debug(
                                f"Successfully sent {len(events)} events to Rizk Hub"
                            )
                            return
                        elif response.status == 401:
                            logger.error(
                                "Authentication failed when sending to Rizk Hub"
                            )
                            return  # Don't retry auth failures
                        else:
                            response_text = await response.text()
                            logger.warning(
                                f"Rizk Hub returned status {response.status}: {response_text}"
                            )

                except asyncio.TimeoutError:
                    logger.warning(
                        f"Timeout sending events to Rizk Hub (attempt {attempt + 1})"
                    )
                except Exception as e:
                    logger.warning(
                        f"Error sending events to Rizk Hub (attempt {attempt + 1}): {e}"
                    )
    ...
    async def _send_metrics_async(self, metrics: List[Metric]) -> None:
        """Send metrics to Rizk Hub API asynchronously."""
        if not metrics:
            return
            ...
            for attempt in range(self.max_retries):
                try:
                    async with session.post(
                        ...
                        if response.status == 200:
                            logger.debug(
                                f"Successfully sent {len(metrics)} metrics to Rizk Hub"
                            )
                            return
                        elif response.status == 401:
                            logger.error(
                    ...
                    logger.warning(
                        f"Timeout sending metrics to Rizk Hub (attempt {attempt + 1})"
                    )
                except Exception as e:
                    logger.warning(
                        f"Error sending metrics to Rizk Hub (attempt {attempt + 1}): {e}"
                    )
    ...
    def close(self) -> None:
        ...
        logger.info("RizkHubProcessor closed")
