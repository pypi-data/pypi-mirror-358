"""Rizk SDK Plugin Architecture.

This module provides a comprehensive plugin system for extending the Rizk SDK
...
if TYPE_CHECKING:
    from ..adapters.base import BaseAdapter as RizkFrameworkAdapter
...
class RizkPlugin(ABC):
    """Base class for all Rizk plugins.
...
class FrameworkPlugin(RizkPlugin):
    """Plugin for custom framework adapters.

    Framework plugins provide adapters for new AI/ML frameworks
    that are not natively supported by Rizk SDK.
    """

    @abstractmethod
    def get_adapter_class(self) -> Type["RizkFrameworkAdapter"]:
        """Return the adapter class for this framework.

        Returns:
            Adapter class that implements RizkFrameworkAdapter interface
        """
        pass
...
class GuardrailsPlugin(RizkPlugin):
...
class AnalyticsPlugin(RizkPlugin):
...
class LLMServicePlugin(RizkPlugin):
...
class PluginRegistryProtocol(Protocol):
    ...
    def register_plugin(self, plugin: RizkPlugin) -> None:
        """Register a plugin."""
        ...
    ...
    def get_plugin(self, plugin_name: str) -> Optional[RizkPlugin]:
