"""Automatic Performance Instrumentation for Rizk SDK.

This module provides transparent performance tracking for all Rizk components:
- Guardrails pipeline timing
- Framework adapter performance
- LLM fallback latency
- Overall Rizk overhead
...
def init_performance_instrumentation(app_name: str = "rizk") -> None:
...
def timed_operation(
    ...
    span_name = f"rizk.{component}.{operation_name}"
        ...
        try:
            ...
            if tags:
                for key, value in tags.items():
                    span.set_attribute(f"rizk.{key}", value)

            span.set_attribute("rizk.component", component)
            span.set_attribute("rizk.operation", operation_name)
            ...
            span.set_attribute("rizk.duration_ms", duration_ms)

            # Send metric to OTel pipeline (automatically forwarded to collectors)
            metric_name = f"rizk.{component}.{operation_name}.duration_ms"
            try:
                if _meter:
...
# Pre-configured decorators for specific Rizk components
...
class PerformanceTimer:
    ...
    def _record_phase_metric(self, phase_name: str, duration_ms: float) -> None:
        ...
        try:
            metric_name = (
                f"rizk.{self.component}.{self.operation}.{phase_name}.duration_ms"
            )
            histogram = _meter.create_histogram(
                metric_name,
