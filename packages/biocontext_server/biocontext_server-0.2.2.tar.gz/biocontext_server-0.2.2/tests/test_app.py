...
async def test_setup_development_environment(mock_mcp_app, monkeypatch):
    ...
    mock_mcp_app.import_server.assert_any_call(slugify(core_mcp.name), core_mcp)
    mock_mcp_app.import_server.assert_any_call(slugify(mock_openapi_mcps[0].name), mock_openapi_mcps[0])
