...
def status():
    ...
    if api_key:
        ...
        try:
            ...
            if not working_model:
                typer.echo("❌ API key test failed - no compatible models found")
                typer.echo("💡 Run 'bashbuddy setup' to reconfigure")
        except Exception as e:
            typer.echo(f"❌ API key test failed: {e}")
            typer.echo("💡 Run 'bashbuddy setup' to reconfigure")
    else:
        ...
        try:
            with open(profile_path, "r") as f:
                ...
                if "GEMINI_API_KEY" in content:
                    typer.echo("📝 ✅ PowerShell profile contains API key")
                else:
                    typer.echo("📝 ❌ PowerShell profile does not contain API key")
        except Exception:
            typer.echo("📝 ⚠️  Could not read PowerShell profile")
    else:
        typer.echo("📝 ❌ PowerShell profile not found")
