...
def rot_with_pixel_scale(telescope_latitude_rad, 
    ...
    for i in range(len(current_alt)):
        ...
        # sinHA = -np.cos(current_alt[i]) * np.sin(current_az[i]) / np.cos(dec)
        # cosHA = (np.sin(current_alt[i]) - np.sin(telescope_latitude_rad) * np.sin(dec)) / (np.cos(telescope_latitude_rad) * np.cos(dec))
        ...
        # sinHA_star = -np.cos(current_alt_star[i]) * np.sin(current_az_star[i]) / np.cos(dec_star)
        # cosHA_star= (np.sin(current_alt_star[i]) - np.sin(telescope_latitude_rad) * np.sin(dec_star)) / (np.cos(telescope_latitude_rad) * np.cos(dec_star))
        # HA_star = np.arctan2(sinHA_star, cosHA_star)
        ...
        if current_alt_star[0] < np.max(current_alt_star) and current_alt_star[-1] < np.max(current_alt_star):
           ...
           ra_star = SI[i].value*15*np.pi/180 * u.rad  - HA_star *u.rad
        # ra_dec_history.append([ra.value * np.cos(dec), dec])
        # ra_dec_star_history.append([ra_star.value * np.cos(dec_star), dec_star])
        ra_dec_history.append([ra.value , dec])
        ra_dec_star_history.append([ra_star.value , dec_star])
    return ([ev_x, ev_y], ev_alt,altaz_star_history, pos_star, ra_dec_star_history, altaz_history, ra_dec_history)