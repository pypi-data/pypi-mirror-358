...
def relation2id(config):
    ...
    描述：从 config.rel_data_path 读取关系文件，每行格式为 "relation_name id"，生成映射字典。
    """
    relation2id_dict = {}
    logger.info("加载关系映射...")
    try:
...
def word2id(config):
    ...
    输出：词到ID的字典 {word: id}
    描述：通过 config.get_word2id() 获取缓存的词表映射，避免重复加载。
    """
    return config.get_word2id()
...
def pos(num, config):
    ...
    输入：
        num: 相对位置差
        config: Config 对象，提供 pos_range
    输出：映射后的非负位置ID
    描述：根据 config.pos_range（默认 max_length-1）将位置差映射到 [0, 2*pos_range]，
          num < -pos_range 返回 0，num > pos_range 返回 2*pos_range，其他情况返回 num + pos_range。
    """
    pos_range_num = config.pos_range
...
def pos_padding(pos_ids, config):
    ...
    输入：
        pos_ids: 位置序列
        config: Config 对象，提供 max_length 和 pos_padding_value
    ...
    return pos_seq + [config.pos_padding_value] * (config.max_length - len(pos_seq))
...
def build_vocabulary(config):
    ...
    输入：
        config: Config 对象，提供 train_data_path
    输出：字符列表
    描述：读取 train_data_path 中的文本，提取唯一字符，用于生成词表，不涉及标签或位置编码。
    """
    logger.info(f"构建词汇表从: {config.train_data_path}")
    chars = set()
    try:
        ...
        logger.error(f"训练数据集文件 {config.train_data_path} 未找到！")
        raise
...
def get_vocab(config):
    ...
    输出：生成词表文件
    描述：调用 build_vocabulary 获取字符列表，保存到 vocab_data_path。
    """
    data_list = build_vocabulary(config)
    logger.info(f"生成词表，保存到: {config.vocab_data_path}")
    with open(config.vocab_data_path, 'w', encoding='utf-8') as fw:
        fw.write('\n'.join(data_list))
        fw.flush()
    logger.info(f"词表已保存到 {config.vocab_data_path}")

def process_single_sample(sample, config):
    """
    功能：处理单条无标签样本数据，转换为张量格式。
    输入：
        sample: 字典，包含 "text" (句子), "ent1" (实体1), "ent2" (实体2)，无 "label" 字段
        config: Config 对象，提供 max_length 和 vocab_data_path
    ...
    # 使用现有逻辑处理
    word2id_dict = word2id(config)
    sents_ids = [sent_padding(sent, word2id_dict, config) for sent in sents]
