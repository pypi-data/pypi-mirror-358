...
def collate_fn(batch, baseconfig):
    ...
    pos_size = baseconfig.pos_size if hasattr(baseconfig, 'pos_size') else 150
    tag_size = len(relation2id(baseconfig))
    for i, sent_id in enumerate(sents_ids):
        if max(sent_id) >= vocab_size:
            logger.warning(f"批次 {i} 句子索引超出范围: {max(sent_id)} >= {vocab_size}")
    for i, (p1, p2) in enumerate(zip(pos_e1_ids, pos_e2_ids)):
        if max(p1) >= pos_size or max(p2) >= pos_size:
            logger.warning(f"批次 {i} 位置索引超出范围: pos1_max={max(p1)}, pos2_max={max(p2)}, pos_size={pos_size}")
    if max(labels) >= tag_size:
        logger.warning(f"批次标签索引超出范围: {max(labels)} >= {tag_size}")
    try:
        ...
        logger.info(f"批次张量: sents_shape={sents_tensor.shape}, pos1_max={pos_e1_tensor.max().item()}, pos2_max={pos_e2_tensor.max().item()}, labels_max={labels_tensor.max().item()}")
