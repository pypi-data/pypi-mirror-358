...
class client:
    ...
    def __init__(self, email=None, password=None, APIKey=None, bearer_token=None, environment=None, endpoint=None, local=False, interactive=True, verbose=None):
        ...
        if self.__bearer_token:
            try:
                ...
                workspace = self.workspaces[0]
                self.workspace = workspace['workspaceId'] # Default to first workspace
        ...
        elif email or (APIKey is None and os.environ.get('RENDEREDAI_API_KEY') is None):
            if not email:
            ...
            self.sign_in_apikey()
            if self.user: self.auth_method = 'apikey'
            if not self.user:
                if self.interactive: 
    ...
    from .volumes       import get_volumes, get_volumes, create_volume, edit_volume, delete_volume, get_volume_data, download_volume_data, upload_volume_data, delete_volume_data, mount_volumes
