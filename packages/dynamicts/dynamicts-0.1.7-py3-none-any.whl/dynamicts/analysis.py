...
class UnivariateAnalysis:
    ...
    def __init__(self, df: pd.DataFrame, target_col: str, index_col: str = "date", output_filepath : str = "output_filepath"):
    ...
    def plot_distribution(self):
        ...
        plt.tight_layout()
        log_plot_to_html_report(fig, title=f"Distribution of {self.target_col}", data_filepath=self.filepath)
        plt.close(fig)
        return fig
    ...
    def check_distribution_stats(self):
        ...
        log_message_to_html_report(full_msg, data_filepath=self.filepath, title=f"Distribution Stats: {self.target_col}")
    ...
    def check_missing_values(self):
        ...
        log_message_to_html_report(msg.strip(), data_filepath=self.filepath, title=f"Missing Value Analysis for '{self.target_col}'")
    ...
    def detect_outliers(self, method="both", plot=True):
        ...
        log_message_to_html_report(msg.strip(), data_filepath=self.filepath, title=f"Outlier Detection ({method_label})")
        ...
        if plot:
            ...
            log_plot_to_html_report(fig, title=f"{method_label} Outlier Detection for {self.target_col}", data_filepath=self.filepath)
            plt.close(fig)
    ...
    #     log_plot_to_html_report(fig, title=f"Rolling Statistics of {self.target_col}", data_filepath=self.filepath)
    #     plt.close(fig)
    ...
    #     """
    #     log_message_to_html_report(msg.strip(),data_filepath=self.filepath, title="Rolling Statistics Summary")
    ...
    @staticmethod
    def run_univariate_analysis(df : pd.DataFrame, output_filepath: str, target_col: str, index_col: str = "date"):
        try:
            print(f"\nRunning Univariate Time Series Analysis on '{target_col}' ")
            analysis = UnivariateAnalysis(df = df, output_filepath=output_filepath, target_col=target_col, index_col=index_col)
            results = {}
...
if __name__ == "__main__":
    ...
    UnivariateAnalysis.run_univariate_analysis(df=df,
        output_filepath="test2",
