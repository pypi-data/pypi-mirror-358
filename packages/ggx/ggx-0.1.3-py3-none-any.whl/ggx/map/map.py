...
class Map(GameClient):
    ...
    async def get_map_chunk(self, kingdom, x, y):
    ...
    async def map_scanner(
        ...
        try:
            ...
            for x, y in self._spiral_coords(max_radius=max_radius, cx=castle_x, cy=castle_y):
                await self.get_map_chunk(kingdom, x, y)
    ...
    def periodic_map_scanner(
        ...
        async def _scan_and_reschedule():
            
            try:
                await self.map_scanner(kingdom, max_radius, castle_x, castle_y, handler)
            
            except Exception as e:
                logger.error(e)
            
            finally:
                loop.call_later(interval, lambda: loop.create_task(_scan_and_reschedule()))
                
            if run_immediately:
                loop.create_task(_scan_and_reschedule())
