...
def run_create():
    ...
    new_target = input("Enter target creation folder (or press Enter to keep current): ").strip()
    if not new_target:
    ...
    config["creation_directory"] = new_target
    with open(jpc_config_file, "w") as f:
        json.dump(config, f, indent=2)

    target_base = Path(new_target)
    if not target_base.exists():
        print(f"✅ Creating folder: {target_base}")
    ...
    package_root = target_base / package_name
    print(f"\n✅ Generating Jadio package at:\n{package_root}\n")
    package_root.mkdir(parents=True, exist_ok=True)

    # Build skeleton
    src_dir = package_root / "src" / package_name
    cli_dir = src_dir / "cli"
