...
def test():
    from convert_stream import (
        FileDocumentPdf, CompressDocument, DocumentPdf, LibraryPDF,
        PdfStream, ImageStream, PageDocumentPdf,
    )
    from soup_files import (
        Directory, File, UserFileSystem, InputFiles
    )

    outdir: Directory = UserFileSystem().userDownloads.concat('output', create=True)
    indir: Directory = UserFileSystem().userDownloads.concat('TESTE-PDF', create=True)
    base = indir.join_file('base.pdf')
