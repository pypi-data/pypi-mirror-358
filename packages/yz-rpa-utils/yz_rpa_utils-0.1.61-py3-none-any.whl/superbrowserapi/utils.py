...
def find_process_ids(process_names: List[str]) -> List[int]:
    ...
    for proc in psutil.process_iter(['pid', 'name']):
        try:
            ...
            # 检查进程名是否在目标列表中（不区分大小写）
            if any(target_name.lower() in proc_name for target_name in process_names):
...
if __name__ == "__main__":
    # 示例1: 通过进程名列表终止多个进程（带强制终止）
    success, failed = terminate_processes_by_names(
    ...
    if not success:
        logger.error(f"未能终止的进程: {failed}")
        # 对未能终止的进程进行强制处理
        for pid in failed:
            terminate_process_tree_force(pid)

    # 示例2: 强制终止特定浏览器
    terminate_browser_processes(['chrome', 'edge'], force=True)

    # 示例3: 强制终止特定进程
    terminate_process_tree_force(1234)  # 替换为实际PID
