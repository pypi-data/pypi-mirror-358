...
class Button:
    def __init__(self, x, y, width=100, height=100, border_size=5,
        ...
        # Internal flags to handle click edge detection.
    ...
    def on_click(self, func):
        """
        Decorator for setting the click callback.
        Usage:
            @btn.on_click
    ...
    def on_toggle_on(self, func):
        """
        Decorator for setting the toggle-on callback.
        Usage:
            @btn.on_toggle_on
            def my_toggle_on():
                print("Toggled on!")
    ...
    def on_toggle_off(self, func):
        """
        Decorator for setting the toggle-off callback.
        Usage:
            @btn.on_toggle_off
            def my_toggle_off():
                print("Toggled off!")
    ...
    def tick(self, screen):
        ...
        It draws the button and checks for clicks.
        """
        # Draw the button and its border.
        pygame.draw.rect(screen, self.colour, (self.x, self.y, self.width, self.height))
        pygame.draw.rect(screen, self.border_colour, (self.x, self.y, self.width, self.height),
                         self.border_size)
        ...
        if (self.x <= mouse_pos[0] <= self.x + self.width and
            ...
            # If the mouse is pressed and it wasn't pressed in the previous tick,
            # we consider this a click (edge detection).
            if mouse_pressed and not self._was_clicked:
                self._was_clicked = True

                # Fire the on_click callback if one is registered.
                if self._click_callback:
                    self._click_callback()

                # Toggle the state and call the corresponding toggle callback.
                if self.toggled:
                    self.toggled = False
            ...
            # If the mouse is outside the button, also reset the click flag.
            self._was_clicked = False
