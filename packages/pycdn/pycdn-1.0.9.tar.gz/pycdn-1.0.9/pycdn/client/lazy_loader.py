...
class LazyAttribute:
    ...
    def __call__(self, *args, **kwargs) -> Any:
        ...
        try:
            ...
            if self._instance:
                # Instance method call - encode the instance creation and method call together
                # We'll create a special function call that first creates the instance, then calls the method
                
                # Apply automatic encryption to sensitive data
                encrypted_init_args, encrypted_init_kwargs = encryption.process_arguments(
                    self._instance._init_args, 
                    self._instance._init_kwargs
                )
                encrypted_method_args, encrypted_method_kwargs = encryption.process_arguments(
                    args, 
                    kwargs
                )
                
                # Create a special payload that includes both instance creation and method call
                instance_call_data = {
                    "type": "instance_method",
                    "class_name": self._instance._class_name,
                    "init_args": encrypted_init_args,
                    "init_kwargs": encrypted_init_kwargs,
                    "method_path": self._attribute_path,
                    "method_args": encrypted_method_args,
                    "method_kwargs": encrypted_method_kwargs
                }
                
                serialized_args = serialize_args(instance_call_data)
                
                # Use a special function name to indicate this is an instance method call
                method_name = f"__instance_call__"
            else:
                # Module function call - apply automatic encryption
                encrypted_args, encrypted_kwargs = encryption.process_arguments(args, kwargs)
                serialized_args = serialize_args(*encrypted_args, **encrypted_kwargs)
