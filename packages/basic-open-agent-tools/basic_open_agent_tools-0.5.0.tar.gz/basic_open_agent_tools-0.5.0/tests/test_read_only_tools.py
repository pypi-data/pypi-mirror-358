...
class TestReadOnlyTools:
    ...
    def test_read_only_tools_exclude_write_operations(self):
        ...
        write_operations = [
            'write_file_from_string', 'append_to_file', 'create_directory',
            'delete_file', 'delete_directory', 'move_file', 'copy_file',
            'write_csv_file', 'write_yaml_file', 'write_toml_file', 
            'write_ini_file', 'write_binary_file', 'create_zip_archive',
            'create_tar_archive', 'serialize_object', 'compress_json_data'
        ]
        
        for write_op in write_operations:
            assert write_op not in tool_names, f"Write operation {write_op} found in read-only tools"
    ...
    def test_read_only_tools_include_expected_functions(self):
        ...
        expected_read_only = [
            'file_exists', 'directory_exists', 'read_file_to_string',
            'list_directory_contents', 'validate_json_string',
            'read_csv_file', 'read_yaml_file', 'validate_schema',
            'clean_whitespace', 'extract_sentences', 'flatten_dict',
            'validate_binary_format', 'list_archive_contents'
        ]
        
        for expected in expected_read_only:
            assert expected in tool_names, f"Expected read-only tool {expected} not found"
    ...
    def test_load_all_read_only_tools_can_be_merged(self):
        ...
        assert len(merged) > len(read_only_tools)  # Should be larger due to additional write tools
    ...
    def test_list_all_available_tools_includes_read_only(self):
        ...
        assert len(all_tools['read_only']) == 56  # Should match our read-only tool count