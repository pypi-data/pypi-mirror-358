...
from doclayout_yolo_slim.utils import (
    ASSETS,
    AUTOINSTALL,
    LINUX,
    LOGGER,
...
def check_file(file, suffix="", download=True, hard=True):
    ...
    if (
        ...
        if Path(file).exists():
            ...
            LOGGER.error(f"Failed to download {clean_url(url)}")
        return file
    else:  # search
...
def check_yolo(verbose=True, device=""):
    ...
    from doclayout_yolo_slim.utils.torch_utils import select_device
...
def collect_system_info():
    ...
    from doclayout_yolo_slim.utils import ENVIRONMENT, is_git_dir
    from doclayout_yolo_slim.utils.torch_utils import get_cpu_info
