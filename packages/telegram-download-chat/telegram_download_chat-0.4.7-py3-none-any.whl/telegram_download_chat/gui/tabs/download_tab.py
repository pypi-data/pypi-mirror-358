...
from PySide6.QtWidgets import (
    ...
    QPushButton, QSpinBox, QDateEdit, QLabel, QTreeView, QFrame,
    QSizePolicy, QMessageBox, QCheckBox, QFileDialog, QProgressBar
)
from PySide6.QtCore import Qt, QDate, QModelIndex, Signal, QSize, QPropertyAnimation, QPoint, QPointF, QRectF, QTimer
from PySide6.QtGui import QStandardItemModel, QStandardItem, QIcon, QPainter, QPainterPath, QTransform, QPixmap, QMovie
...
class DownloadTab(QWidget):
    ...
    def _setup_settings_container(self, parent_layout):
        ...
        self.user_edit.setPlaceholderText("@username or user_id (e.g., 12345 or user12345)")
        ...
        self.settings_item = QStandardItem("Settings ▶")  # Add arrow indicator (collapsed state)
    ...
    def _save_settings(self):
        ...
        try:
            ...
            settings.update({
                ...
                'until': self.until_edit.date().toString("yyyy-MM-dd") if self.until_edit.date().isValid() else "",
    ...
    def start_download(self):
        ...
        chat = self.chat_edit.text().strip()
        if not chat:
            QMessageBox.warning(self, "Error", "Please enter a chat ID, username, or link.")
            return
    ...
    def update_progress(self, current: int, total: int):
        ...
        if total > 0:
            ...
            self.progress.setFormat(f"Downloading... {current}/{total} ({current/total*100:.1f}%)")
        else:
            # Indeterminate mode
    ...
    def save_settings(self, settings: Dict[str, Any]):
        ...
        settings['until'] = self.until_edit.date().toString("yyyy-MM-dd") if self.until_edit.date().isValid() else ""
