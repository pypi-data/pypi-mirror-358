from canproc.pipelines.loaders import FileLoader, create_loader, default_loader
...
class Pipeline:
        ...
        if from_file:
            ...
            if "loader" in var[name]:
                ...
                kwargs = {
                    ...
                    "parallel": self.config["setup"]["file_format"] == "netcdf",
                    "chunks": {"time": 96},
                }
                if "chunks" in var[name]:
                    kwargs["chunks"] = var[name].pop("chunks")

                loader = default_loader(filename, kwargs=kwargs)
        else:
            loader = None

        self.variables[name] = Variable(name, loader=loader, **kwargs)
    ...
    def _write_and_tag(self, var: str | dict, stage: str):
        ...
        if write:
            try:
                posix_path = self.variables[name].write(
                    output_dir=self.directories[stage], reuse_from_tag=reuse_tag, **kwargs
                )
            except KeyError:
                pass
        ...
        if (
            self.merged_netcdf
            and stage in self.config["setup"]["output_filenames"]
            and posix_path is not None
        ):
