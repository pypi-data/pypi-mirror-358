...
class AsyncMemory(MemoryBase):
    ...
    async def add(
        ...
        vector_store_task = asyncio.create_task(self._add_to_vector_store(messages, metadata, filters, infer, prompt))
    ...
    async def _add_to_vector_store(self, messages, metadata, filters, infer, prompt=None):
        if not infer:
            returned_memories = []
            for message in messages:
                if message["role"] != "system":
