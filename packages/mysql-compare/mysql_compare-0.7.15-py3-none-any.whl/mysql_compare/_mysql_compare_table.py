...
def mysql_query(con: MySQLConnection, query_statement: str, query_params) -> list[list[bytes]]:
    with con.cursor(raw=True) as cur:
...
def find_missing_in_b2(a: list[list[bytes]], b: list[list[bytes]]):
    b_set = {tuple(sublist) for sublist in b}
    return [sublist for sublist in a if tuple(sublist) not in b_set]


def mysql_query_row(con: MySQLConnection, database, table, table_keys, diff_row) -> list[bytes] | None:
    whereval = []
    params: list = []
...
class MysqlCompareTable:
    ...
    def running_compare(self):
        with connect(**self.src_dsn) as source_con, connect(**self.dst_dsn) as target_con:
            ...
            query_statement_cols = [col[0] for col in source_table_struct]

            while True:
                query_statement_src, query_params_src = get_query_full_table_statement_params(self.src_db, self.src_tab, self.table_keys, query_statement_cols, self.batch_size, self.proc_keyval)
                query_statement_dst, query_params_dst = get_query_full_table_statement_params(self.dst_db, self.dst_tab, self.table_keys, query_statement_cols, self.batch_size, self.proc_keyval)
                ...
                _diff_rows = find_missing_in_b2(source_rows, target_rows)
                ...
                _source_rows = dict(zip(query_statement_cols, source_rows[-1]))
                self.proc_keyval = extract_keyvals(_source_rows, self.table_keys)
                ...
                if self.exact and len(_diff_rows) >= 1:
                    ...
                    for diff_row in _diff_rows:
                        ...
                        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                            _diff_row = dict(zip(query_statement_cols, diff_row))
                            src_row = executor.submit(mysql_query_row, source_con, self.src_db, self.src_tab, self.table_keys, _diff_row).result()
                            dst_row = executor.submit(mysql_query_row, target_con, self.dst_db, self.dst_tab, self.table_keys, _diff_row).result()
                            if src_row is not None and src_row != dst_row:
                                self.diff_rows_cnt += 1
                                self.logger.debug(f"batch_id[{_batch_id}] diff row: src[{_diff_row}]")
                                self.logger.debug(f"batch_id[{_batch_id}] diff row: dst[{dst_row}]")
                                if self.is_repair >= 1:
                                    _src_row = dict(zip(query_statement_cols, src_row))
                                    mysql_repair_row(self.logger, target_con, self.dst_db, self.dst_tab, _src_row)
