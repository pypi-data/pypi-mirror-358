...
class StrUtil:
    ...
    def encrypt(src: str, key: str) -> str:
        '''key: 32byte'''
        nonce, cipher = StrUtil.__make_key(key=key)
        encrypted = cipher.encrypt(nonce=nonce, data=src.encode(), associated_data=StrUtil.ASSOCIATED)
        encrypted = base64.urlsafe_b64encode(encrypted).decode().replace('=', '')
        return encrypted

    @staticmethod
    def decrypt(src: str, key: str) -> str | None:
        encrypted = StrUtil.__b64decode_padded(src)
        nonce, cipher = StrUtil.__make_key(key=key)
        decrypted = cipher.decrypt(nonce=nonce, data=encrypted, associated_data=StrUtil.ASSOCIATED)
...
if __name__ == '__main__':
    ...
    encrypted = StrUtil.encrypt('암호화', KEY)
    print(f'암호화: {encrypted}')
    decrypted = StrUtil.decrypt(encrypted, KEY)
