...
def show_image_popup(img: Image.Image, title="QR Code"):
    root = tk.Tk()
    root.title(title)
...
def show_qr(uri: str, title="QR Code"):
    """
...
def config_to_v2ray_outbound(cfg: dict) -> dict:
    """
    Given a parsed config dict (output of parse_uri),
    return the matching v2ray-core outbound JSON block.
    """
    protocol = cfg["type"]
    if protocol == "vless":
        return {
            ...
            "settings": {
                "vnext": [{
                    ...
                    "users": [{
                        "id": cfg["uuid"],
                        "encryption": cfg.get("params", {}).get("encryption", ["none"])[0]
                    }]
                }]
            },
            "streamSettings": {
                "network": cfg.get("params", {}).get("type", ["tcp"])[0],
                "security": cfg.get("params", {}).get("security", ["none"])[0],
                "tcpSettings": {
                    "header": {"type": cfg.get("params", {}).get("headerType", ["none"])[0]}
                },
                "wsSettings": {
                    "path": cfg.get("params", {}).get("path", [""])[0],
                    "headers": {"Host": cfg.get("params", {}).get("host", [""])[0]}
                }
            }
        }
