...
from P2Ray.menu import Menu, Option
...
class CLIApp:
    def __init__(self) -> None:
        ...
        self.arc_manager = ConfigManager("archive_db.json")
        self.running = True
    ...
    def create_main_menu(self) -> Menu:
        return Menu("P2Ray Configuration Manager", [
            
            Option("Add Configs",      "Import server URIs",                self.add_configs),
            Option("List Configs",     "View & sort your configs",          self.list_configs),
            Option("Share",            "Share URI by ID",                   self.share_uri),
            Option("Share All",        "Share All URIs",                    self.share_all),
            Option("Test All",         "Run latency test on all configs",   self.test_all),
            Option("Log Con",          "Logging the connection",            self.log_connectivity),
            Option("Archive Config",   "Move a config to archive DB",       self.archive_config),
            Option("Unarchive",        "Restore from archive DB",           self.unarchive_config),
            Option("Remove Config",    "Permanently delete by ID",          self.remove_config),
            Option("connect",          "",                                  self.connect),
            Option("disconnect",       "",                                  self.disconnect),
            Option("Toggle Sys Proxy", "",                                  self.toggle_system_proxy),
    ...
    def _connect(self, cfg_id) -> None:
        ...
        v2_conf = {
            ...
            "outbounds": [config_to_v2ray_outbound(cfg._parsed)],
        }
    ...
    def _interactive_list(self, mgr: ConfigManager, title: str) -> None:
        ...
        valid_fields    = [key   for alias, key, _ in FIELDS] # All legal sort keys
        headers  = [
            styled_header(title, alias, fg_color="h cyan")
            for alias, key, title in FIELDS
        ]
