...
```bash
claude mcp add {project_name} -s user "/path/to/run_server.py"
```
...
1. Run this test file immediately: python scripts/test.py
2. If import errors occur, run diagnostics first: python scripts/diagnose.py
3. Fix any failing tests in server.py
4. After tests pass, run: python scripts/verify.py
...
TROUBLESHOOTING:
- Import errors? Run: python scripts/diagnose.py
- Tests fail? Check if server.py has print() statements (use logger instead)
- Still issues? The scripts/diagnose.py script will identify common problems
...
claude mcp add {project_name} -s user "py {project_path}\\run_server.py"

# Alternative methods:
claude mcp add {project_name} -s user "{project_path}\\run_server.bat"
claude mcp add {project_name} -s user "python {project_path}\\run_server.py"
...
```bash
# Primary method (Global - recommended):
claude mcp add {project_name} -s user "{project_path}/run_server.py"

# Alternative methods:
claude mcp add {project_name} -s user "python3 {project_path}/run_server.py"
claude mcp add {project_name} -s user "{project_path}/run_server.sh"
...
class ImplementationVerifier:
    ...
    def __init__(self):
        ...
        # Only match TODO/FIXME etc when they appear in comments
        self.todo_patterns = [
            r'#.*\bTODO:', r'#.*\bFIXME:', r'#.*\bXXX:', r'#.*\bHACK:', r'#.*\bNOTE:',
            r'#.*\bTODO\s*\(', r'#.*\bFIXME\s*\(', r'#.*\bXXX\s*\(',
            r'""".*\bTODO:.*"""', r"'''.*\bTODO:.*'''",  # In docstrings
            r'""".*\bFIXME:.*"""', r"'''.*\bFIXME:.*'''"
        ]
    ...
    def verify_all(self):
        ...
        if not issues and not missing_resources:
            ...
            print("\\n5. After fixing, run tests again: python scripts/test.py")
            print("\\n6. Then run this verification again: python scripts/verify.py")
...
# Verification template for scripts directory
VERIFICATION_TEMPLATE_FOR_SCRIPTS = r'''#!/usr/bin/env python3
"""
Implementation Verification for {project_name}
Checks for incomplete implementations and missing requirements
...
class ImplementationVerifier:
    ...
    def __init__(self, project_dir_name: str):
        self.project_dir = Path(__file__).parent.parent / project_dir_name
        self.issues = []
        self.warnings = []
        self.checked_files = []
    ...
    def check_resources(self) -> List[str]:
        ...
        # Check server.py for path references
        server_file = self.project_dir / "server.py"
        if server_file.exists():
            content = server_file.read_text()
    ...
    def verify_all(self):
        ...
        # Check server.py in project directory
        server_file = self.project_dir / "server.py"
        if not server_file.exists():
            print(f"❌ ERROR: server.py not found in {{self.project_dir}}!")
            return
            
        print(f"\\nChecking {{server_file}} for incomplete implementations...")
        print("-" * 70)
...
if __name__ == "__main__":
    verifier = ImplementationVerifier("{project_dir_name}")
    verifier.verify_all()
'''
