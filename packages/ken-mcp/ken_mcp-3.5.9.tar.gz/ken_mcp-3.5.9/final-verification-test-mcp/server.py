...
mcp = FastMCP(
    name="final-verification-test-mcp",
    instructions="""
    MCP server for: Create a simple test MCP with a single ping tool that returns pong
...
- What response format makes sense"""
    ...
    try:
        ...
        # Simulate successful data fetch for testing
        item = {"id": data_id, "data": "test_data", "status": "active"}
...
def help(topic: Optional[str] = None) -> str:
    """Get help with general operations"""
    if topic == "ping":
        return "Use the fetch_data tool to ping the system. It returns 'pong' data."
    elif topic == "process":
        return "Use the process_data tool to process information."
    elif topic == "store":
        return "Use the store_result tool to store data."
    else:
        return "Available tools: fetch_data (ping), process_data (process), store_result (store). Use help with topic name for specific help."
