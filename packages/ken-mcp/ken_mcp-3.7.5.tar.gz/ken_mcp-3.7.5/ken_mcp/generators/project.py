...
def create_diagnostic_script_to_scripts(
    ...
    """Create diagnostic script in scripts directory with correct paths
    ...
    Args:
        ...
        python_version: Python version requirement
    """
    from ken_mcp.templates.diagnostics import get_diagnostic_script_for_scripts
    
    diagnostic_content = get_diagnostic_script_for_scripts(
        project_name, 
        project_path.name, 
        python_version
    )
    diagnostic_path = scripts_path / "diagnose.py"
    diagnostic_path.write_text(diagnostic_content)
    make_executable(diagnostic_path)