...
class SiteApiService(BaseApiService):
    ...
    def get_sites(self) -> List[SiteDTO]:
        ...
        print(f"[DEBUG] Found {len(results)} sites in V8 API response")
        
        sites = [self._to_site_dto(item) for item in results]
        self.logger.info(f"Converted {len(sites)} sites to DTOs")
        print(f"[DEBUG] Converted {len(sites)} sites to DTOs: {[site.name for site in sites]}")

        for site in sites:
            print(f"[DEBUG] Processing site: fid={site.fid}, name='{site.name}'")
            try:
                buildings = self.api_service.building_service.get_buildings(site.fid)
                print(f"[DEBUG] Got {len(buildings)} buildings for site fid={site.fid}, name='{site.name}'")
                for b in buildings:
                    print(f"[DEBUG]   Building: fid={b.fid}, name='{b.name}'")
                site.buildings = buildings
                self.logger.info(
                    f"Attached {len(buildings)} buildings to site {site.fid}"
                )
                print(f"[DEBUG] Attached {len(buildings)} buildings to site fid={site.fid}, name='{site.name}'")
            except Exception as e:
                self.logger.error(
                    f"Failed to fetch buildings for site {site.fid}: {str(e)}"
                )
                print(f"[DEBUG] Failed to fetch buildings for site fid={site.fid}, name='{site.name}': {str(e)}")
    ...
    def get_site_by_fid(self, site_fid: str) -> SiteDTO:
        ...
        data = self._make_request("GET", endpoint)
        result = data.get("result", data)
        return self._to_site_dto(result)
