...
def get_scalecrop_filter(path, pass_num: int):
    ...
    size = 456 if pass_num == 1 else 512

    filters = []

    # scale the smallest dimension to size, then center-crop the larger to size
    if w < h:
        # scale width to size, maintain aspect, then crop height
        filters.append(f"scale={size}:-1,crop={size}:{size}:0:(ih-{size})/2")
    else:
        # scale height to size, maintain aspect, then crop width
        filters.append(f"scale=-1:{size},crop={size}:{size}:(iw-{size})/2:0")
...
def vp9_pass1(input_path: str, vid_bps: float):
    ...
    """
    vf = get_scalecrop_filter(input_path, 1)
...
def vp9_pass2(input_path: str, output_path: str, vid_bps: float):
    ...
    """
    vf = get_scalecrop_filter(input_path, 2)
    stream = ffmpeg.input(input_path).video
    # build output keyword arguments, adding filter if needed
    output_kwargs = {'c:v': 'libvpx-vp9', 'b:v': str(vid_bps), 'pix_fmt': 'yuv420p'}
    if vf:
        output_kwargs['vf'] = vf
    stream.output(
        output_path,
...
def convert_optimize(input_path: str, target_size_kb: float = 255, accuracy_kbps: float = 0.5, progress_callback=None):
    """
