...
class NormalizationHandler:
    ...
    def swap_column_values(
        ...
        key_df = self.__table_state[key_table]

        suffix = f"_{key_table}"
        exchange_df_columns = set(exchange_df.columns)

        suffixed_key_column = (
            key_column + suffix if key_column in exchange_df_columns else key_column
        )
        suffixed_new_column = (
            new_column + suffix if new_column in exchange_df_columns else key_column
        )

        exchange_df = (
            exchange_df.merge(
                key_df[[key_column, new_column]],
                left_on=exchange_column,
                right_on=key_column,
                suffixes=(f"", f"_{key_table}"),
                how="left",
            )
            .drop(columns=list(set([key_column, suffixed_key_column, exchange_column])))
            .rename(
                columns={
                    suffixed_new_column: exchange_column,
                }
            )
        )
