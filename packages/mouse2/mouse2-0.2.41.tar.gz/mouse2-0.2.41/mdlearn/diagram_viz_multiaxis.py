...
if __name__ == "__main__":
    ...
    for i_plot, config_filename in enumerate(args.config):
        ...
        if i_plot > 0:
            current_ax = axes[0].twiny()
            current_ax.spines['top'].set_position(('outward', (i_plot-1)*40))


        current_ax.scatter(points1y, points1x, s = 15, marker = 'o', 
                 facecolors = pp['colors']['1'], edgecolors = pp['colors']['1'], 
                 label = pp['label'] + ' stable')
        current_ax.scatter(points2y, points2x, s = 75,
                    marker = 'x', facecolors = pp['colors']['2'], alpha = 0.33,
